...
export default function POSTerminalLayout({
  ...
  const { itemCount, cashDrawer, fetchStoreSettings, storeSettingsLoaded } =
    usePOSStore()
  ...
  // ── Load real store settings (tax rate, currency) from Medusa once ────────
  useEffect(() => {
  ...
  // ── Auth check ──────────────────────────────────────────────────────────
  useEffect(() => {
    // Check via authStore — middleware already guards the pos-auth cookie,
    // but if client-side state is missing, send them back to login
  ...
  const handleLogout = () => {
    // Log out via authStore — clears only the pos-auth cookie now
    // (previously cleared dashboard/website sessions too)
    logout('pos')
  ...
  return (
    <AuthProvider surface='pos'>
      ...
      >
        ...
        <main
          className='flex-1 min-h-0 overflow-hidden transition-opacity duration-200 flex flex-col'
        ...
        {showStaff && authUser?.role === 'admin' && (
          <StaffManagement onClose={() => setShowStaff(false)} />
        )}
      </div>
    </AuthProvider>
  )
