...
export async function GET(req: NextRequest) {
  try {
    ...
    const ordersRes = await fetch(`${MEDUSA_URL}/admin/orders?limit=10&order=-created_at`, {
      headers: h
    }).catch(() => null);
    if (ordersRes?.ok) {
      const {
      ...
      const newOrders = (orders ?? []).filter((o: any) => new Date(o.created_at) > new Date(since));
    ...
    const productsRes = await fetch(`${MEDUSA_URL}/admin/products?limit=100&fields=*variants,*variants.inventory_items`, {
      headers: h
    }).catch(() => null);
    if (productsRes?.ok) {
      const {
    ...
    const customersRes = await fetch(`${MEDUSA_URL}/admin/customers?limit=5&order=-created_at`, {
      headers: h
    }).catch(() => null);
    if (customersRes?.ok) {
      const {
      ...
      const newCustomers = (customers ?? []).filter((c: any) => new Date(c.created_at) > new Date(since));
