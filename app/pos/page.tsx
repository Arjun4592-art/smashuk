...
function PinDots({
  ...
  return <div className='flex items-center justify-center gap-3'>
      {[0, 1, 2, 3, 4, 5].map(i => <div key={i} className={`w-3 h-3 rounded-full border-2 transition-all duration-150 ${success ? 'bg-[#008060] border-[#008060]' : error ? 'bg-red-500 border-red-500' : i < pin.length ? 'bg-[#202223] border-[#202223]' : 'bg-transparent border-[#C9CCCF]'}`} />)}
    </div>;
}
function PinPad({
  onPress,
  ...
  return <div className='grid grid-cols-3 gap-2'>
      {keys.map((k, i) => k === '' ? <div key={i} /> : <button key={i} onClick={() => k === '⌫' ? onDelete() : onPress(k)} disabled={disabled} className={`h-12 rounded-lg text-[15px] font-semibold transition-all select-none
...
export default function POSLoginPage() {
  ...
  return <div className='min-h-screen flex items-center justify-center p-4 bg-[#F6F6F7]'>
      <div className='w-full max-w-sm'>
        <div className='text-center mb-6'>
          ...
          <p className='text-sm text-[#6D7175]'>
            Choose your account then enter your 6-digit PIN
          </p>
        </div>

        <div className='bg-white rounded-xl border border-[#E1E3E5] shadow-sm p-5'>
          {loadingStaff ? <div className='py-8 flex flex-col items-center gap-2'>
            ...
            </div> : loadError ? <p className='text-sm text-center text-red-600 py-4'>{loadError}</p> : staffList.length === 0 ? <p className='text-sm text-center text-[#6D7175] py-4'>
              No active staff found. Please add staff from the dashboard.
            </p> : <>
              <div className='flex flex-col gap-2 mb-4'>
                {staffList.map(staff => <button key={staff.id} onClick={() => handleSelect(staff)} disabled={verifying} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-all text-left cursor-pointer ${selected?.id === staff.id ? 'border-[#008060] bg-[#008060]/5' : 'border-[#E1E3E5] bg-white hover:bg-[#F6F6F7]'}`}>
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center text-[13px] font-bold shrink-0 ${selected?.id === staff.id ? 'bg-[#008060] text-white' : 'bg-[#F1F2F3] text-[#6D7175]'}`}>
                      {staff.initials}
                    </div>
                    ...
                    {selected?.id === staff.id && <div className='w-2 h-2 rounded-full bg-[#008060] shrink-0' />}
                  </button>)}
              </div>
              ...
              <div className='h-5 flex items-center justify-center mb-3'>
                {pinError && <p className='text-xs font-medium text-red-600 text-center px-4'>
                    {pinError}
                  </p>}
                {pinSuccess && <p className='text-xs font-medium text-[#008060]'>
