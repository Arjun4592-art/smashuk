...
export async function GET(req: NextRequest) {
  ...
  // Customer lookup is a routine POS checkout action for every cashier,
  // not a manager-only action, so POS sessions are allowed here.
...
export async function POST(req: NextRequest) {
  // Creating a walk-in/new customer at checkout is a routine POS action.
  const authHeader = await getAdminAuthHeader(req, {
    allowPos: true,
  })
  if (!authHeader) {
  ...
  try {
    ...
    return NextResponse.json(
      await safeJson(res, 'app/api/admin/customers/route.ts'),
    )
  } catch (err: any) {
