...
function StatusPill({
  ...
  return <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${isRedeemed ? 'bg-gray-100 text-gray-500' : 'bg-green-50 text-green-700'}`}>
...
export default function GiftCardsPage() {
  ...
  const activeValue = giftCards.filter(g => g.status === 'pending').reduce((s, g) => s + g.value, 0);
  return <div className='p-6 max-w-6xl mx-auto'>
      <div className='flex items-center justify-between mb-6'>
        ...
        <button onClick={() => setShowModal(true)} className='flex items-center gap-2 bg-[#0A1F44] hover:bg-[#E8553A] text-white font-semibold text-sm px-4 py-2.5 rounded-xl transition-colors'>
      ...
      <div className='flex gap-3 mb-4'>
        <input type='text' value={search} onChange={e => setSearch(e.target.value)} placeholder='Search by code…' className='flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#E8553A]' />
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as any)} className='border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#E8553A]'>
      ...
      <div className='bg-white border border-[#E1E3E5] rounded-2xl overflow-hidden'>
        {loading ? <div className='p-12 flex justify-center'>
            <SpinnerIcon />
          </div> : giftCards.length === 0 ? <div className='p-12 flex flex-col items-center gap-3 text-center'>
            <GiftIcon />
            ...
            <tbody>
              {giftCards.map(gc => <tr key={gc.id} className='border-b border-gray-50 hover:bg-gray-50/50'>
          ...
          <div className='bg-white rounded-2xl w-full max-w-md p-6'>
            ...
            <div className='space-y-4'>
              <div>
                ...
                <input type='number' min='1' step='0.01' value={form.value} onChange={e => setForm(f => ({
              ...f,
              value: e.target.value
            }))} placeholder='50.00' className='w-full mt-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#E8553A]' />
              </div>
              <div>
                <label className='text-xs font-semibold text-gray-500 uppercase'>
                  Code (optional — auto-generated if left blank)
                </label>
                <input type='text' value={form.code} onChange={e => setForm(f => ({
              ...f,
              code: e.target.value.toUpperCase()
            }))} placeholder='e.g. THANKYOU25' className='w-full mt-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#E8553A] font-mono' />
              </div>
              <div>
                <label className='text-xs font-semibold text-gray-500 uppercase'>
                  Expires (optional)
                </label>
                <input type='date' value={form.expires_at} onChange={e => setForm(f => ({
              ...f,
              ...
              ...f,
              note: e.target.value
            }))} placeholder='e.g. Goodwill gesture — delayed order #482' rows={2} className='w-full mt-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#E8553A] resize-none' />
              </div>
            </div>
            ...
            <div className='flex gap-3 mt-6'>
              ...
              <button onClick={handleCreate} disabled={saving} className='flex-1 bg-[#0A1F44] hover:bg-[#E8553A] text-white rounded-xl py-2.5 text-sm font-semibold transition-colors disabled:opacity-60 flex items-center justify-center gap-2'>
                {saving && <SpinnerIcon />}
