...
interface Props {
  onClose: () => void
  // Everything needed to build the receipt email — same data already shown
  // on the receipt screen / sent to Print, so this stays consistent with
  // what the customer already saw at the counter.
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            disabled={sending}
            className='w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#F6F6F7] disabled:opacity-40'
            style={{ color: '#6D7175' }}
          >
            <svg
              width='16'
        ...
        <div className='p-5'>
          ...
          <input
            ...
            disabled={sending}
            className='w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors focus:border-[#008060] disabled:opacity-60'
            style={{
          ...
          />
          {error && (
            <p className='text-xs mt-1.5' style={{ color: '#D82C0D' }}>
              {error}
            </p>
          )}
        </div>
