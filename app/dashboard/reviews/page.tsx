...
// Reviews shown on the website homepage (ReviewsSlider) used to be 6
// hardcoded fake customers with fake quotes — no backend, no way to
// change them without editing code. This page manages the real thing:
// reviews live in Medusa's store.metadata.reviews (app/api/admin/reviews),
// and only ones marked "Published" show up on the public site.
...
function StarIcon({ filled }: { filled: boolean }) {
  return (
    <svg width='16' height='16' viewBox='0 0 24 24' fill={filled ? '#FBBF24' : 'none'} stroke={filled ? '#FBBF24' : '#C4C8CC'} strokeWidth='1.5'>
...
function ChatIcon() {
  return (
    <svg width='32' height='32' viewBox='0 0 24 24' fill='none' stroke='#C4C8CC' strokeWidth='1.5' strokeLinecap='round' strokeLinejoin='round'>
...
// ─── Stat Card ──────────────────────────────────────────────────
function StatCard({ label, value, icon, colorClasses }: { label: string; value: string | number; icon: React.ReactNode; colorClasses: string }) {
  return (
...
export default function ReviewsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.success(`Review by "${name}" deleted`, { id: toastId })
    } catch (err: any) {
    ...
    <div className='space-y-5'>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-3 gap-4'>
        ...
        <StatCard
          ...
          colorClasses='bg-[#FBBF24]/10 text-[#B45309]'
        />
      </div>
      ...
      <div className='bg-white border border-[#E1E3E5] rounded-2xl overflow-hidden'>
        {loading ? (
          <div className='flex items-center justify-center py-16 text-[#8C9196] gap-2 text-[13px]'>
          ...
          <div className='flex flex-col items-center justify-center py-16 gap-3'>
            ...
            <button
              onClick={openAdd}
              className='inline-flex items-center gap-2 px-4 py-2 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-medium rounded-lg transition-colors border-none cursor-pointer'
            >
          ...
          <div className='divide-y divide-[#F1F2F3]'>
            {reviews.map((r) => (
              <div key={r.id} className='flex items-start gap-4 p-4 hover:bg-[#FAFBFB] transition-colors'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  <p className='text-[11px] text-[#B5B9BD] mt-1.5'>
                    Added {new Date(r.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                  </p>
                </div>
                <div className='flex items-center gap-3 shrink-0'>
                  <Toggle
                    ...
                    title={r.published ? 'Live on website — click to hide' : 'Hidden — click to publish'}
                  />
                  <button
                    onClick={() => setDeleteId(r.id)}
                    className='w-8 h-8 rounded-lg flex items-center justify-center text-[#8C9196] hover:bg-[#FEF2F2] hover:text-[#D82C0D] transition-colors border-none bg-transparent cursor-pointer'
                    title='Delete'
                  >
                    <TrashIcon />
                  </button>
            ...
            <div className='space-y-4'>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className='w-full px-3 py-2 border border-[#E1E3E5] rounded-lg text-[13px] focus:outline-none focus:ring-2 focus:ring-[#008060]/20 focus:border-[#008060] resize-none'
              ...
              <div className='flex gap-2.5 pt-2'>
                ...
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className='flex-1 py-2.5 bg-[#008060] hover:bg-[#006e52] disabled:opacity-60 text-white text-[13px] font-semibold rounded-lg transition-all duration-150 border-none cursor-pointer shadow-sm shadow-[#008060]/20 flex items-center justify-center gap-2'
                >
