...
// BUG FIX: this bar used to hardcode a fixed 11-brand list and fake stats
// ("11+ Premium Brands", "500+ Products") that never matched what was
// actually in stock. Curated copy below (description blurb + 2-letter
// initial fallback + logo path) is still hand-written since that's
// presentation, not data — but the LIST and STATS now come from
// useBrandStats(), which aggregates real product.metadata.brand values
// from Medusa. A brand only appears here if it genuinely has products.
const BRAND_INFO: Record<string, { description: string; initial: string }> = {
  Yonex: { description: 'Badminton & Tennis', initial: 'YX' },
  Victor: { description: 'Badminton', initial: 'VC' },
  'Li-Ning': { description: 'Badminton', initial: 'LN' },
  Babolat: { description: 'Tennis & Badminton', initial: 'BB' },
  Head: { description: 'Tennis & Squash', initial: 'HD' },
  Wilson: { description: 'Tennis & Squash', initial: 'WL' },
  'K-Swiss': { description: 'Tennis Shoes', initial: 'KS' },
  Adidas: { description: 'Tennis & Padel', initial: 'AD' },
  Bullpadel: { description: 'Padel', initial: 'BP' },
  Tecnifibre: { description: 'Padel & Tennis', initial: 'TF' },
  Dunlop: { description: 'Padel & Squash', initial: 'DL' },
}
...
// Renders the brand's logo image if it exists in /public/brands/<slug>.png,
// and falls back to the initials badge if the file is missing/fails to load.
function BrandLogo({
  logo,
  initial,
  name,
...
export default function BrandsBar() {
  ...
  // No brands in the catalog yet (or still loading) — don't render fake
  // placeholders, just skip the section.
