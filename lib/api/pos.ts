...
function extractPrice(variant: any, productMetadata: any): number {
  const gbpPrice = variant?.prices?.find((pr: any) => pr.currency_code === 'gbp')?.amount;
...
function extractStock(variant: any): number {
  ...
  if (invItem) {
    ...
    if (Array.isArray(levels) && levels.length > 0) {
      return levels.reduce((sum: number, l: any) => sum + (l.available_quantity ?? l.stocked_quantity ?? 0), 0);
    }
...
export async function fetchPOSProducts(): Promise<POSProduct[]> {
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(errorData.error || `HTTP ${res.status}: Products fetch failed`);
    }
    ...
    const mapped = (data.products as any[]).map(mapProductToPOS).filter((p: POSProduct | null): p is POSProduct => p !== null);
    return mapped;
  } catch (err: unknown) {
...
export async function fetchPOSCustomers(search?: string): Promise<POSCustomer[]> {
  try {
...
export async function createPOSOrder(payload: CreatePOSOrderPayload): Promise<any> {
  try {
...
export async function fetchPOSOrderHistory(limit = 150): Promise<PosOrderRecord[]> {
  try {
    ...
    throw new Error(err instanceof Error ? err.message : 'Failed to load order history');
  }
}
export async function markPOSOrderReturned(medusaOrderId: string, reason: string, items: {
  item_id: string;
  quantity: number;
}[]): Promise<{
  ...
  try {
    ...
    throw new Error(err instanceof Error ? err.message : 'Failed to process return');
  }
}
export async function fetchDefaultRegion(): Promise<string | null> {
  try {
    ...
    throw new Error(err instanceof Error ? err.message : 'Failed to fetch region');
  }
}
...
export async function validateCoupon(code: string): Promise<CouponValidationResult> {
  try {
    const res = await fetch(`/api/pos/coupons/validate?code=${encodeURIComponent(code)}`);
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
    ...
    throw new Error(err instanceof Error ? err.message : 'Failed to validate coupon');
  }
}
...
export async function validateGiftCard(code: string): Promise<GiftCardValidationResult> {
  try {
    const res = await fetch(`/api/pos/gift-cards/validate?code=${encodeURIComponent(code)}`);
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
    ...
    throw new Error(err instanceof Error ? err.message : 'Failed to validate gift card');
  }
}
...
export async function emailPOSReceipt(payload: EmailReceiptPayload): Promise<{
  ...
  try {
    ...
    throw new Error(err instanceof Error ? err.message : 'Failed to email receipt');
  }
}
