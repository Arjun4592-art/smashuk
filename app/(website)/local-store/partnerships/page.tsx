...
export const metadata = {
  ...
  description: 'Partner with us for exclusive club benefits including free demo days, bulk discounts on equipment, custom printing, and player sponsorships for tennis, badminton, and padel clubs.',
  keywords: 'club partnership programme, university sports partnership, racket club sponsorship, custom kit printing club, shuttlecock partnership programme, tennis ball club supply, sports club discounts uk'
};
const BENEFITS = [{
  icon: '🎾',
  title: 'Free Demo Days',
  desc: 'We bring the latest rackets to your club nights:',
  items: ['Tennis, badminton & padel demos', 'All major brands in stock', 'Expert fitting & advice on-site', 'Try before you buy', 'Member-only pricing'],
  ctas: [{
    label: 'Find Out More',
  ...
  items: ['Tennis balls & shuttlecocks', 'Training equipment', 'Court accessories', 'Volume pricing available'],
  ctas: [{
  ...
  items: ['Custom logo printing', 'Team apparel & bags', 'Personalised racket bags', 'Fast turnaround times'],
  ctas: [{
    label: 'Find Out More',
  ...
  items: ['Equipment sponsorships', 'Tournament support', 'Junior development programmes', 'Performance discounts'],
  ctas: [{
    label: 'Find Out More',
  ...
  return <div className='bg-[#F5F3EF] min-h-screen'>
      ...
      <section className='reveal max-w-5xl mx-auto px-4 md:px-6 py-16'>
        ...
        <div className='grid md:grid-cols-2 gap-6'>
          {BENEFITS.map(b => <div key={b.title} className='bg-white border border-[#0A1F44]/8 rounded-2xl p-8'>
              ...
              <ul className='space-y-1.5 mb-5'>
                {b.items.map(item => <li key={item} className='text-gray-500 text-xs flex gap-2'>
                    <span className='text-[#E8553A]'>•</span>
                    {item}
                  </li>)}
              </ul>
              <div className='flex flex-col gap-1.5'>
                {b.ctas.map(c => <Link key={c.href} href={c.href} className='text-xs font-montserrat font-bold text-[#E8553A] hover:text-[#D4441F]'>
                    {c.label} →
                  </Link>)}
              </div>
            </div>)}
      ...
      <section className='reveal bg-white border-y border-[#0A1F44]/8'>
        <div className='max-w-5xl mx-auto px-4 md:px-6 py-16'>
          ...
          <div className='grid sm:grid-cols-2 md:grid-cols-4 gap-px bg-[#0A1F44]/6 border border-[#0A1F44]/6 rounded-2xl overflow-hidden'>
            {WHY_US.map(w => <div key={w.title} className='bg-white p-6 hover:bg-[#F5F3EF] transition-colors text-center'>
