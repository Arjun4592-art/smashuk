...
export const STORE_PRODUCT_FIELDS = '+description,+metadata,*variants,*variants.prices,*variants.calculated_price,*variants.inventory_quantity,*variants.options,*variants.images,*options,*options.values,*categories,*images,*tags';
export const STORE_PRODUCT_LISTING_FIELDS = '+description,+metadata,*variants.prices,*variants.inventory_quantity,*categories,*images,*tags';
export function normalizeProduct(p: any): Product {
  const variant = p.variants?.[0];
  const calcAmount = variant?.calculated_price?.calculated_amount;
  const gbpPrices = (variant?.prices ?? []).filter((pr: any) => pr.currency_code === 'gbp').map((pr: any) => pr.amount);
  ...
  const metaCompareAt = p.metadata?.compare_at_price ? Number(p.metadata.compare_at_price) : undefined;
  const metaOriginal = p.metadata?.originalPrice ? Number(p.metadata.originalPrice) / 100 : undefined;
  const legacySecondGbpPrice = gbpPrices.length > 1 ? Math.max(...gbpPrices) : undefined;
  ...
  return {
    ...
    price: rawPrice,
    originalPrice,
    images: p.images?.map((img: any) => img.url) ?? (p.thumbnail ? [p.thumbnail] : ['/placeholder.png']),
    ...
    inStock: p.variants?.some((v: any) => (v.inventory_quantity ?? 0) > 0) ?? false,
    ...
    variants: p.variants ?? [],
    options: p.options ?? [],
    tierPricing: Array.isArray(p.metadata?.tier_pricing) ? p.metadata.tier_pricing : [],
    crossSells: Array.isArray(p.metadata?.cross_sells) ? p.metadata.cross_sells : []
  };
}
...
const SYSTEM_METADATA_KEYS = new Set(['brand', 'sport', 'badge', 'tags', 'specs', 'specifications', 'string_upgrade_available', 'sale_price', 'regular_price', 'compare_at_price', 'cost_price', 'low_stock_alert', 'taxable', 'originalPrice', 'rating', 'reviewCount', 'metaTitle', 'metaDescription', 'metaKeywords', 'ogImage', 'tier_pricing', 'cross_sells', 'service_type', 'service_sport', 'posRole', 'pin', 'shift', 'isActive', 'totalSales', 'totalOrders', 'phone', 'role', 'stripe_customer_id']);
...
function extractSpecs(metadata: any): {
  ...
  return Object.entries(metadata).filter(([key, value]) => !SYSTEM_METADATA_KEYS.has(key) && value !== undefined && value !== null && value !== '').map(([key, value]) => ({
...
function buildProductUrl(params: Record<string, string | undefined>, regionId?: string | null, light?: boolean) {
  const isServer = typeof window === 'undefined';
  const base = isServer ? `${process.env.NEXT_PUBLIC_MEDUSA_BACKEND_URL ?? 'http://localhost:9000'}/store/products` : '/api/store/products';
  const sp = new URLSearchParams();
  Object.entries(params).forEach(([k, v]) => {
    if (v !== undefined && v !== '') sp.set(k, v);
  });
  if (isServer) {
    sp.set('fields', light ? STORE_PRODUCT_LISTING_FIELDS : STORE_PRODUCT_FIELDS);
...
async function fetchProducts(params: Record<string, string | undefined>, light?: boolean) {
  const isServer = typeof window === 'undefined';
  ...
  if (!res.ok) {
    console.warn(`${label} FAILED after ${Date.now() - t0}ms — HTTP ${res.status}`);
...
export async function getAllProducts(params?: {
  ...
  for (let i = 0; i < offsets.length; i += CONCURRENCY) {
    ...
    const batchResults = await Promise.all(batch.map(offset => getProducts({
      ...params,
...
export async function getAllProductsProgressive(params: {
  q?: string;
  category_id?: string[];
} | undefined, handlers: {
  onFirstBatch: (result: {
  ...
  for (let i = 0; i < offsets.length; i += CONCURRENCY) {
    ...
    if (signal?.cancelled) return;
    handlers.onMore(([] as any[]).concat(...batchResults.map(p => p.products)));
  }
  handlers.onDone?.();
}
...
export async function getSiteReviews(): Promise<import('@/types').SiteReview[]> {
...
export async function createCart() {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.message ?? data.error ?? 'Failed to create cart');
  return data.cart;
}
export async function addToCart(cartId: string, variantId: string, quantity = 1, metadata?: Record<string, any>) {
...
export async function requestReturn(orderId: string, items: {
  item_id: string;
  quantity: number;
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(data.message ?? data.error ?? 'Failed to submit return request');
  return data;
}
async function checkoutAction(action: string, cartId: string, extra?: Record<string, any>) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(data?.error ?? data?.message ?? `Checkout step "${action}" failed`);
  }
  return data;
}
export async function addShippingAddress(cartId: string, address: {
  ...
  const data = await res.json();
  if (!res.ok) {
    throw new Error(data?.error ?? data?.message ?? 'Failed to save shipping address');
  }
  return data.cart;
}
...
export async function updateAddress(addressId: string, address: Partial<CustomerAddress>) {
  const res = await fetch('/api/store/addresses', {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json'
