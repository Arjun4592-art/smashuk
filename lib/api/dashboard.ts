...
export async function getOrders(params?: {
  ...
  return {
    orders: data.orders.map((o: any) => ({
      ...
      customer: o.customer ? `${o.customer.first_name ?? ''} ${o.customer.last_name ?? ''}`.trim() || o.customer.email || o.email || 'Guest' : o.email ?? 'Guest',
      email: o.email ?? '',
      ...
      paymentMethod: (() => {
        ...
        if (posMeta) {
          return posMeta.charAt(0).toUpperCase() + posMeta.slice(1).toLowerCase();
        }
        const providerId: string = (o.payment_collections ?? []).flatMap((pc: any) => pc.payments ?? []).find(Boolean)?.provider_id ?? '';
      ...
      source: (o.metadata?.source === 'pos' ? 'pos' : o.metadata?.source === 'dashboard' ? 'dashboard' : 'website') as 'website' | 'pos' | 'dashboard',
...
export async function processOrderReturn(id: string, items: {
  item_id: string;
  quantity: number;
...
export async function rejectOrderReturn(id: string, returnId: string, note?: string) {
  return mutate(`/api/admin/orders/${id}`, 'PATCH', {
...
export async function getProducts(params?: {
  ...
  return {
    products: data.products.map((p: any) => {
      const gbpPrice = p.variants?.[0]?.prices?.find((pr: any) => pr.currency_code === 'gbp')?.amount;
      ...
      const stock = p.variants?.reduce((sum: number, v: any) => {
        ...
        return sum + levels.reduce((s: number, lvl: any) => s + (lvl.available_quantity ?? lvl.stocked_quantity ?? 0), 0);
      }, 0) ?? 0;
      return {
        id: p.id,
        name: p.title,
...
async function findGlobalOption(title: string, preferredId?: string): Promise<{
  id: string;
  values: {
    id: string;
    value: string;
...
export async function upsertOptionValues(title: string, values: string[], preferredId?: string): Promise<{
  optionId: string;
  valueIds: string[];
  ...
  if (missing.length > 0) {
    const updated = await mutate(`/api/admin/product-options/${existing.id}`, 'POST', {
      title,
  ...
  if (missingAfterUpsert.length > 0) {
    throw new Error(`Failed to resolve option value(s) [${missingAfterUpsert.join(', ')}] for option "${title}" after upsert.`);
  }
...
export async function linkOptionsToProduct(productId: string, options: {
  id: string;
  value_ids: string[];
}[], alreadyLinkedOptionIds: Set<string>, removeOptionIds: string[] = []) {
  const add = options.filter(o => !alreadyLinkedOptionIds.has(o.id));
  const update = options.filter(o => alreadyLinkedOptionIds.has(o.id)).map(o => ({
    product_option_id: o.id,
    value_ids: o.value_ids
  }));
  if (add.length === 0 && update.length === 0 && removeOptionIds.length === 0) return;
...
export async function upsertProductTags(values: string[]): Promise<{
  ...
  const byValue = new Map<string, string>((data.product_tags ?? []).map((t: any) => [t.value.toLowerCase(), t.id]));
  const ids: string[] = [];
...
export async function deleteProductVariant(productId: string, variantId: string) {
  return mutate(`/api/admin/products/${productId}/variants/${variantId}`, 'DELETE');
}
...
export async function deleteProductOption(productId: string, optionId: string) {
  return mutate(`/api/admin/products/${productId}/options/${optionId}`, 'DELETE');
}
export async function upsertProductOption(productId: string, title: string, values: string[], existingOption?: {
  id: string;
  values: {
    id: string;
    value: string;
  ...
  if (!existingOption) {
    const created = await mutate(`/api/admin/products/${productId}/options`, 'POST', {
      title,
      values
    });
    const opt = created.product_option;
    const byValue = new Map<string, string>((opt.values ?? []).map((v: any) => [v.value.toLowerCase(), v.value]));
    return {
      optionId: opt.id,
      canonicalValues: values.map(v => byValue.get(v.toLowerCase()) ?? v)
    };
  }
  const byValue = new Map<string, string>(existingOption.values.map(v => [v.value.toLowerCase(), v.value]));
  const missing = values.filter(v => !byValue.has(v.toLowerCase()));
  if (missing.length > 0) {
    const allValues = [...existingOption.values.map(v => v.value), ...missing];
    try {
      const updated = await mutate(`/api/admin/products/${productId}/options/${existingOption.id}`, 'POST', {
        title,
        values: allValues
      });
      const updatedByValue = new Map<string, string>((updated.product_option?.values ?? []).map((v: any) => [v.value.toLowerCase(), v.value]));
      return {
        optionId: existingOption.id,
        canonicalValues: values.map(v => updatedByValue.get(v.toLowerCase()) ?? v)
      };
    } catch (err: any) {
      if (/already exists|conflict|duplicate/i.test(err?.message ?? '')) {
        ...
        const opt = created.product_option;
        const freshByValue = new Map<string, string>((opt.values ?? []).map((v: any) => [v.value.toLowerCase(), v.value]));
        return {
          optionId: opt.id,
          canonicalValues: values.map(v => freshByValue.get(v.toLowerCase()) ?? v),
...
export async function getCustomers(params?: {
  ...
  return {
    customers: data.customers.map((c: any) => ({
      ...
      lastOrder: c.orders?.[0] ? new Date(c.orders[0].created_at).toLocaleDateString('en-GB', {
        day: 'numeric',
        month: 'short',
        year: 'numeric'
      }) : '—',
      ...
      avatar: `${c.first_name?.[0] ?? ''}${c.last_name?.[0] ?? ''}`.toUpperCase() || 'U'
    })),
    count: data.count
  };
}
export async function getInventory(params?: {
  ...
  return data.products.flatMap((p: any) => (p.variants ?? []).map((v: any) => {
    const levels = v.inventory_items?.flatMap((ii: any) => ii.inventory?.location_levels ?? []) ?? [];
    const stock = levels.reduce((s: number, lvl: any) => s + (lvl.available_quantity ?? lvl.stocked_quantity ?? 0), 0);
    const reserved = levels.reduce((s: number, lvl: any) => s + (lvl.reserved_quantity ?? 0), 0);
    const incoming = levels.reduce((s: number, lvl: any) => s + (lvl.incoming_quantity ?? 0), 0);
    const gbpPrice = v.prices?.find((pr: any) => pr.currency_code === 'gbp')?.amount;
...
export async function getDashboardStats(range?: string): Promise<DashboardStats> {
  const data = await api<DashboardStats>('/api/admin/stats', range ? {
    range
  } : undefined);
  return {
...
export async function getDiscounts(params?: {
  ...
  return {
    discounts: data.promotions.map((d: any) => {
      ...
      const isQuantityDiscount = d.application_method?.type === 'percentage' && !!d.is_automatic && !!quantityRule;
      return {
        id: d.id,
        code: d.code,
        type: isQuantityDiscount ? 'buy_x_get_y' : d.application_method?.type ?? 'percentage',
        value: d.application_method?.value ?? 0,
        minQuantity: quantityRule?.values?.[0]?.value ?? null,
        minOrderAmount: d.rules?.find((r: any) => r.attribute === 'subtotal')?.values?.[0] ?? 0,
        maxUses: d.usage_limit ?? null,
        usedCount: d.usage_count ?? 0,
        startsAt: d.starts_at ? new Date(d.starts_at).toISOString().split('T')[0] : '',
        expiresAt: d.ends_at ? new Date(d.ends_at).toISOString().split('T')[0] : null,
