...
function buildReceiptHtml(p: ReceiptPayload): string {
  ...
  const discountRow = p.discountAmount > 0 ? `<tr><td style="padding:4px 0;color:#008060;">Discount</td><td style="padding:4px 0;text-align:right;color:#008060;">-${fmt(p.discountAmount)}</td></tr>` : '';
  const paymentLine = p.payMethod === 'split' && p.splitPayments?.length ? p.splitPayments.map(s => `${PAY_LABELS[s.method] || s.method}: ${fmt(s.amount)}`).join(' · ') : PAY_LABELS[p.payMethod] || p.payMethod;
  return `
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!result.sent) {
      return NextResponse.json({
        error: result.error === 'SMTP not configured' ? 'Email is not set up yet — add SMTP_HOST/SMTP_USER/SMTP_PASS/SMTP_FROM to .env.local (see SETUP.md).' : result.error ?? 'Failed to send email'
      }, {
        status: 502
      });
    }
