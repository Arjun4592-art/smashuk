...
import { updateProduct, upsertOptionValues, linkOptionsToProduct, deleteProductVariant, upsertProductTags } from '@/lib/api/dashboard';
...
export default function EditProductPage({
    ...
    async function loadProduct() {
      try {
        ...
        setForm({
          ...
          comparePrice: p.metadata?.compare_at_price ? String(p.metadata.compare_at_price) : '',
          costPrice: p.metadata?.cost_price ? String(p.metadata.cost_price) : '',
          ...
          tags: Array.isArray(p.tags) && p.tags.length > 0 ? p.tags.map((t: any) => t.value).join(', ') : p.metadata?.tags ?? '',
          metaTitle: '',
          metaDescription: '',
          metaKeywords: ''
        });
        const KNOWN_KEYS = new Set(['brand', 'sport', 'badge', 'tags', 'specs', 'specifications', 'sale_price', 'regular_price', 'compare_at_price', 'cost_price', 'low_stock_alert', 'taxable', 'tier_pricing', 'cross_sells', 'string_upgrade_available', 'originalPrice', 'rating', 'reviewCount', 'metaTitle', 'metaDescription', 'metaKeywords', 'ogImage', 'service_type', 'service_sport']);
        if (Array.isArray(p.metadata?.cross_sells)) {
          setCrossSells(p.metadata.cross_sells.map((c: any, i: number) => ({
            id: String(i),
        ...
        if (Array.isArray(p.metadata?.specs) && p.metadata.specs.length > 0) {
          ...
          const legacy = Object.entries(p.metadata).filter(([k, v]) => !KNOWN_KEYS.has(k) && v !== undefined && v !== null && v !== '').map(([k, v]) => ({
        ...
        if (p.variants && p.variants.length > 1) {
          setVariants(p.variants.map((v: any) => ({
            ...
            imageUrls: Array.isArray(v.metadata?.variant_images) ? v.metadata.variant_images : []
          })));
        ...
        if (Array.isArray(p.options)) {
          ...
          Promise.all(rawOptions.map(async (o: {
            id: string;
            title: string;
          }) => {
            try {
              const res = await fetch(`/api/admin/product-options/${o.id}?fields=id,title,values.id,values.value`);
              const data = await res.json();
              const values = (data.product_option?.values ?? []).map((v: any) => v.value);
            ...
            setExistingOptions(prev => prev.map(o => {
  ...
  const updateForm = (key: string, value: string | boolean) => setForm(prev => ({
    ...prev,
    [key]: value
  }));
  ...
  const removeVariant = (vid: string) => setVariants(prev => {
    const target = prev.find(v => v.id === vid);
    if (target?.medusaId) {
      variantsToDeleteRef.current = [...variantsToDeleteRef.current, target.medusaId];
    }
    return prev.filter(v => v.id !== vid);
  });
  const updateVariant = (vid: string, key: Exclude<keyof Variant, 'options' | 'imageUrls'>, value: string) => setVariants(prev => prev.map(v => v.id === vid ? {
    ...v,
    [key]: value
  } : v));
  ...
  const toggleVariantImage = (vid: string, url: string) => setVariants(prev => prev.map(v => v.id === vid ? {
    ...v,
  ...
  const searchCrossSellProducts = async (q: string) => {
    ...
    try {
      ...
      const data = await res.json();
      setCrossSeachResults((data.products ?? []).filter((p: any) => p.id !== id).map((p: any) => ({
        id: p.id,
        title: p.title
      })));
    } catch {
  ...
  const buildPayload = (saveStatus: 'published' | 'draft') => {
    ...
    const canonicalValue = (title: string, typed: string) => optionValueCasingRef.current.get(`${title.toLowerCase()}::${typed.toLowerCase()}`) ?? typed;
  ...
  const syncOptionsForVariants = async () => {
    ...
    for (const [title, valueSet] of neededByTitle) {
      ...
      const existingOption = existingOptions.find(o => o.title.toLowerCase() === title.toLowerCase());
      const union = Array.from(new Set([...(existingOption?.values ?? []), ...requested]));
      const {
        optionId,
        valueIds,
        canonicalValues
      } = await upsertOptionValues(title, union, existingOption?.id);
      linkTargets.push({
        id: optionId,
        title,
        value_ids: valueIds
      ...
      requested.forEach(typed => {
        const idx = union.findIndex(v => v.toLowerCase() === typed.toLowerCase());
        const canonical = idx >= 0 ? canonicalValues[idx] : undefined;
        if (canonical) {
          optionValueCasingRef.current.set(`${title.toLowerCase()}::${typed.toLowerCase()}`, canonical);
        }
      });
      setExistingOptions(prev => {
        const others = prev.filter(o => o.id !== optionId && o.title.toLowerCase() !== title.toLowerCase());
        return [...others, {
          id: optionId,
          title,
          values: canonicalValues
    ...
    const removeOptionIds = existingOptions.filter(o => !neededTitles.has(o.title)).map(o => o.id);
    const defaultVariantSurvives = variants.some(v => v.medusaId === defaultVariantId && filledOptions(v).length > 0);
    ...
    if (removeNow.length > 0) {
      setExistingOptions(prev => prev.filter(o => !removeNow.includes(o.id)));
    }
  };
  const restoreDeletedDefaultVariant = async () => {
    try {
      ...
      if (restoredId) {
        ...
        setVariants(prev => prev.map((v, i) => i === 0 && filledOptions(v).length === 0 ? {
          ...v,
          medusaId: restoredId
        } : v));
      }
      toast.error('Save failed — your original SKU/price/stock have been restored. Please try adding the variant again.');
    } catch (restoreErr) {
      console.error('[restore deleted default variant]', restoreErr);
      toast.error('Save failed and the original variant could not be restored automatically — please refresh and check this product before saving again.');
    } finally {
  ...
  const handleSaveInner = async (saveStatus: 'published' | 'draft') => {
    ...
    try {
      ...
      if (staleOptionIdsRef.current.length > 0) {
        ...
        try {
          await linkOptionsToProduct(id, [], new Set(existingOptions.map(o => o.id)), toRemove);
      ...
      if (inventoryDebug.length > 0) {
        const hasFailure = inventoryDebug.some(line => /FAILED|SKIPPED|THREW|No stock location|still NOT FOUND/.test(line));
        if (hasFailure) {
  ...
  return <div className='max-w-275 mx-auto space-y-5'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex items-center gap-2'>
          <button onClick={() => handleSave('draft')} disabled={saving} className='px-4 py-2 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[13px] font-medium text-[#202223] rounded-lg transition-colors disabled:opacity-50 cursor-pointer'>
            Save as Draft
          </button>
          <button onClick={() => handleSave('published')} disabled={saving} className='px-4 py-2 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-semibold rounded-lg transition-colors disabled:opacity-50 cursor-pointer flex items-center gap-2'>
            {saving ? <>
      ...
      {}
      {saveSuccess && <div className='flex items-center gap-3 px-4 py-3 bg-[#F2F7F5] border border-[#008060]/20 rounded-xl text-[13px] text-[#008060]'>
          <svg width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2'>
      ...
      {}
      {saveError && <div className='flex items-start gap-3 px-4 py-3 bg-[#FFF4F4] border border-[#D82C0D]/20 rounded-xl text-[13px] text-[#D82C0D]'>
      ...
      <div className='grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-5'>
            ...
            <div className='p-6'>
              ...
              {activeTab === 'general' && <div className='space-y-5'>
                  <div>
                    ...
                    <input type='text' value={form.name} onChange={e => updateForm('name', e.target.value)} className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea value={form.description} onChange={e => updateForm('description', e.target.value)} rows={5} className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all resize-none' />
                  </div>
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    ...
                    if (e.target.value === '__add_new__') {
                      ...
                      updateForm('brand', e.target.value);
                          ...
                          {}
                          {(form.brand && !brandOptions.includes(form.brand) ? [form.brand, ...brandOptions] : brandOptions).map(b => <option key={b} value={b}>
                              {b}
                            </option>)}
                    ...
                    if (e.target.value === '__add_new__') {
                      ...
                      updateForm('sport', e.target.value);
                          ...
                          {(form.sport && !sportOptions.includes(form.sport) ? [form.sport, ...sportOptions] : sportOptions).map(s => <option key={s} value={s}>
                              {s}
                            </option>)}
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    <div>
                      ...
                      <input type='text' value={form.sku} onChange={e => updateForm('sku', e.target.value)} className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                    </div>
                    <div>
                      <label className='block text-[12.5px] font-medium text-[#202223] mb-1.5'>
                        Barcode
                      </label>
                      <input type='text' value={form.barcode} onChange={e => updateForm('barcode', e.target.value)} className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input type='text' value={form.tags} onChange={e => updateForm('tags', e.target.value)} className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
              ...
              {activeTab === 'pricing' && <div className='space-y-5'>
                  <div className='grid grid-cols-2 gap-4'>
                    <div>
                      ...
                      <div className='relative'>
                        ...
                        <input type='number' value={form.price} onChange={e => updateForm('price', e.target.value)} placeholder='0.00' className='w-full pl-8 pr-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                      </div>
                    </div>
                    <div>
                      <label className='block text-[12.5px] font-medium text-[#202223] mb-1.5'>
                      ...
                      <div className='relative'>
                        ...
                        <input type='number' value={form.comparePrice} onChange={e => updateForm('comparePrice', e.target.value)} placeholder='0.00' className='w-full pl-8 pr-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    <div>
                      ...
                      <div className='relative'>
                        ...
                        <input type='number' value={form.costPrice} onChange={e => updateForm('costPrice', e.target.value)} placeholder='0.00' className='w-full pl-8 pr-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                      </div>
                    </div>
                  ...
                  <div className='border border-[#E1E3E5] rounded-lg overflow-hidden'>
                    <div className='flex items-center justify-between px-4 py-3 bg-[#F6F6F7] border-b border-[#E1E3E5]'>
                      ...
                      <button onClick={() => setTierPricing(prev => [...prev, {
                    ...
                    {tierPricing.length === 0 ? <div className='px-4 py-6 text-center'>
                      ...
                      setTierPricing(updated);
                        ...
                        setTierPricing(updated);
                            ...
                            <button onClick={() => setTierPricing(prev => prev.filter((_, j) => j !== i))} className='w-8 h-8 flex items-center justify-center text-[#8C9196] hover:text-[#D82C0D] hover:bg-[#FFF4F4] rounded-lg bg-transparent border-none cursor-pointer text-base'>
                              ✕
                            </button>
                          </div>)}
                        {}
                  ...
                  {form.trackInventory && <>
                      <div className='grid grid-cols-2 gap-4'>
                        <div>
                          ...
                          <input type='number' value={form.stock} onChange={e => updateForm('stock', e.target.value)} placeholder='0' className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                        </div>
                        <div>
                          <label className='block text-[12.5px] font-medium text-[#202223] mb-1.5'>
                            Low Stock Alert
                          </label>
                          <input type='number' value={form.lowStockAlert} onChange={e => updateForm('lowStockAlert', e.target.value)} placeholder='5' className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                        </div>
                      </div>
                      <div>
                        <label className='block text-[12.5px] font-medium text-[#202223] mb-1.5'>
                        ...
                        <input type='number' value={form.weight} onChange={e => updateForm('weight', e.target.value)} placeholder='e.g. 500' className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                      </div>
                    </>}
                </div>}
                      ...
                      return <div key={opt.id} className='flex items-center gap-1.5'>
                        ...
                        <div className='grid grid-cols-3 gap-3'>
                          {(['sku', 'price', 'stock'] as const).map(field => <div key={field}>
                              ...
                              <input type={['price', 'stock'].includes(field) ? 'number' : 'text'} value={variant[field]} onChange={e => updateVariant(variant.id, field, e.target.value)} className='w-full px-3 py-2 border border-[#E1E3E5] rounded-lg text-[12.5px] text-[#202223] outline-none focus:border-[#008060] transition-all' />
                            </div>)}
                        </div>
              ...
              {activeTab === 'cross-sell' && <div className='space-y-5'>
                  ...
                  searchCrossSellProducts(e.target.value);
                }} placeholder='Search for a product (e.g. Socks, Grip, Shuttle)...' className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] placeholder-[#8C9196] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                    ...
                    {crossSellResults.length > 0 && <div className='absolute z-10 top-full left-0 right-0 mt-1 bg-white border border-[#E1E3E5] rounded-lg shadow-lg overflow-hidden'>
              ...
              {activeTab === 'seo' && <div className='space-y-5'>
                  ...
                  <div>
                    ...
                    <input type='text' value={form.metaTitle} onChange={e => updateForm('metaTitle', e.target.value)} maxLength={60} className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all' />
                  </div>
                  <div>
                    <label className='block text-[12.5px] font-medium text-[#202223] mb-1.5'>
                      Meta Description{' '}
                    ...
                    <textarea value={form.metaDescription} onChange={e => updateForm('metaDescription', e.target.value)} maxLength={160} rows={3} className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all resize-none' />
                  </div>
                </div>}
            </div>
          </div>
        ...
        <div className='space-y-4'>
          ...
          <div className='bg-white border border-[#E1E3E5] rounded-xl p-5'>
            ...
            <div className='space-y-2'>
              {(['published', 'draft'] as const).map(s => <label key={s} className='relative flex items-center gap-3 p-3 border border-[#E1E3E5] rounded-lg cursor-pointer hover:bg-[#F6F6F7] transition-colors'>
                  <input type='radio' name='status' value={s} checked={status === s} onChange={() => setStatus(s)} className='accent-[#008060] w-4 h-4' />
                  <div>
                    <p className='text-[13px] font-medium text-[#202223] capitalize'>
                      {s}
                    </p>
                    <p className='text-[11.5px] text-[#6D7175]'>
                      {s === 'published' ? 'Visible on storefront' : 'Hidden from storefront'}
                    </p>
                  </div>
                </label>)}
            </div>
          ...
          <div className='bg-white border border-[#E1E3E5] rounded-xl p-5'>
            ...
            <div className='space-y-3'>
              ...
              label: 'Images',
              value: images.filter(i => i.url).length > 0 ? `${images.filter(i => i.url).length} images` : '—'
          ...
          {}
          <div className='space-y-2'>
            <button onClick={() => handleSave('published')} disabled={saving} className='w-full py-2.5 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-semibold rounded-lg transition-colors disabled:opacity-50 cursor-pointer border-none'>
              {saving ? 'Saving...' : 'Save Changes'}
            </button>
            <button onClick={() => handleSave('draft')} disabled={saving} className='w-full py-2.5 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[13px] font-medium text-[#202223] rounded-lg transition-colors disabled:opacity-50 cursor-pointer'>
              Save as Draft
            </button>
