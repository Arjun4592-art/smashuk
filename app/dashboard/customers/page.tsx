    ...
    <div className='bg-white border border-[#E1E3E5] rounded-2xl p-5 flex items-center gap-4 hover:shadow-[0_4px_20px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 transition-all duration-200'>
      ...
      <div>
        <p className='font-sora text-[22px] font-bold text-[#202223] leading-tight tracking-tight'>
          {loading ? (
            <span className='inline-block w-16 h-6 bg-[#F1F1F1] rounded animate-pulse' />
          ) : (
            value
          )}
        </p>
...
function CustomersContent() {
  ...
  const vipCustomers = customers.filter(
    (c: (typeof customers)[number]) => c.totalSpent > 300,
  ).length

  // Segments (definitions shared with the Add Discount "quick add from
  // segment" picker via lib/customer-segments.ts)
  const SEGMENTS = CUSTOMER_SEGMENTS
  const [selectedSegment, setSelectedSegment] = useState<string | null>(null)
  const activeSegment = SEGMENTS.find((s) => s.id === selectedSegment) ?? null
  const segmentCustomers = activeSegment
    ? customers.filter(activeSegment.filter)
    : []
  const handleExportSegment = () => {
    ...
    const rows = segmentCustomers.map((c: (typeof customers)[number]) => ({
      Name: c.name,
      Email: c.email,
      Phone: c.phone,
      City: c.city,
    ...
    <div className='space-y-5'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex items-center gap-2.5'>
          ...
          <button
            ...
            className='inline-flex items-center gap-2 px-4 py-2 bg-[#008060] hover:bg-[#006e52] active:bg-[#005c45] text-white text-[13px] font-medium rounded-lg transition-all duration-150 border-none cursor-pointer shadow-sm shadow-[#008060]/20'
          >
            <PlusIcon />
            Add Customer
          </button>
      ...
      <div className='grid grid-cols-2 xl:grid-cols-4 gap-4'>
        ...
        <StatCard
          ...
          value={
            customers.filter(
              (c: (typeof customers)[number]) => c.status === 'active',
            ).length
          }
        ...
        <div className='flex items-center border-b border-[#E1E3E5] px-4 gap-0.5'>
          {[
            ...
            <button
              ...
              className={`px-4 py-3 text-[13px] font-medium whitespace-nowrap border-b-2 transition-all duration-150 bg-transparent border-l-0 border-r-0 border-t-0 cursor-pointer ${view === tab.id ? 'border-b-[#008060] text-[#008060]' : 'border-b-transparent text-[#8C9196] hover:text-[#202223]'}`}
            >
              {tab.label}
                  ...
                  {loading ? (
                    ...
                    paginated.map((customer: (typeof customers)[number]) => (
                      ...
                      >
                        ...
                        <td className='px-4 py-3.5'>
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold capitalize ${STATUS_STYLES[customer.status] ?? 'bg-gray-100 text-gray-600'}`}
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full shrink-0 ${STATUS_DOT[customer.status] ?? 'bg-gray-400'}`}
                            />
                            {customer.status}
                          </span>
                        </td>
            ...
            <div className='grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3'>
              {SEGMENTS.map((seg) => {
                ...
                return (
                  <button
                    key={seg.id}
                    onClick={() => setSelectedSegment(isActive ? null : seg.id)}
                    className={`text-left bg-white border rounded-xl p-4 flex items-start gap-3 cursor-pointer transition-all duration-150 hover:shadow-[0_4px_16px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 ${isActive ? 'border-[#008060] ring-2 ring-[#008060]/15' : 'border-[#E1E3E5]'}`}
                  >
                    <div
                      className={`w-9 h-9 ${style.bg} rounded-lg flex items-center justify-center ${style.text} shrink-0`}
                    >
                      <SegmentIcon />
                    </div>
                    <div className='min-w-0'>
              ...
              <div className='border border-[#E1E3E5] rounded-xl overflow-hidden'>
                <div className='flex items-center justify-between px-4 py-3 bg-[#FAFAFA] border-b border-[#E1E3E5]'>
                  ...
                  <button
                    ...
                    className='inline-flex items-center gap-2 px-3 py-1.5 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[12.5px] font-medium text-[#202223] rounded-lg transition-colors duration-150 cursor-pointer shadow-sm'
                  >
                    <ExportIcon />
                    Export
                  </button>
                      ...
                      <thead>
                        <tr className='border-b border-[#E1E3E5]'>
                          {[
                            ...
                            <th
                              key={i}
                              className='px-4 py-2.5 text-[11px] font-semibold text-[#9CA3AF] uppercase tracking-wider text-left'
                            >
                              {h}
                            </th>
                          ))}
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
          ...
          <div className='relative bg-white rounded-2xl shadow-[0_24px_64px_rgba(0,0,0,0.15)] w-full max-w-md overflow-hidden'>
            ...
            <div className='px-6 py-5 space-y-3'>
              {addError && (
                <div className='px-3 py-2 rounded-lg text-xs bg-[#FFF4F4] text-[#D82C0D]'>
                  {addError}
                </div>
              )}
              <div className='grid grid-cols-2 gap-3'>
                <div>
                  <label className='block text-[11px] font-medium text-[#6D7175] mb-1'>
                    First name
                  </label>
                  <input
            ...
            <div className='flex items-center justify-end gap-2.5 px-6 py-4 bg-[#FAFAFA] border-t border-[#E1E3E5]'>
              <button
                ...
                disabled={addSaving}
                className='px-4 py-2 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[13px] font-medium text-[#202223] rounded-lg cursor-pointer disabled:opacity-50'
              >
                Cancel
              </button>
              <button
                ...
                className='px-5 py-2 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-semibold rounded-lg disabled:opacity-50 cursor-pointer border-none shadow-sm shadow-[#008060]/20'
              >
      ...
      {editingCustomer && (
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
          <div
            className='absolute inset-0 bg-black/40 backdrop-blur-[2px]'
            onClick={() => !editSaving && setEditingCustomer(null)}
          />
          <div className='relative bg-white rounded-2xl shadow-[0_24px_64px_rgba(0,0,0,0.15)] w-full max-w-md overflow-hidden'>
            <div className='flex items-center justify-between px-6 py-4 border-b border-[#E1E3E5]'>
              <h2 className='font-sora text-[16px] font-semibold text-[#202223]'>
