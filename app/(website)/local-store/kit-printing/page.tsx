...
export const metadata = {
  ...
  description: 'Professional kit printing and design services for clubs and universities. Branded and non-branded options available at competitive prices. Custom logos, team wear, and bulk discounts.',
  keywords: 'club kit printing, custom team kit, racket club merchandise, university sports kit printing uk'
};
const WHAT_WE_OFFER = [{
  icon: '🎨',
  title: 'Custom Design',
...
const SERVICES = [{
  ...
  items: ['Match day kits', 'Training wear', 'Hoodies & tracksuits', 'Team bags', 'Bulk order discounts']
}, {
  icon: '🎾',
  title: 'Sports Clubs',
  desc: 'Professional kits for tennis, badminton & padel clubs:',
  items: ['Club polo shirts', 'Performance wear', 'Warm-up jackets', 'Custom racket bags', 'Personalisation options']
}];
const PRICING = [{
  ...
  points: ['Club + sponsor logos', 'Custom design process', 'Premium materials'],
  accent: true
}];
const HOW_IT_WORKS = [{
  n: '01',
  ...
  n: '03',
  title: 'Approve',
  body: 'Review and approve your design.'
}, {
  n: '04',
  title: 'Production',
  body: 'We print and deliver your kits.'
}];
function Eyebrow({
  children
}: {
  ...
  return <div className='bg-[#F5F3EF] min-h-screen'>
      ...
      <section className='reveal max-w-5xl mx-auto px-4 md:px-6 py-16'>
        ...
        <div className='grid sm:grid-cols-2 md:grid-cols-4 gap-px bg-[#0A1F44]/6 border border-[#0A1F44]/6 rounded-2xl overflow-hidden'>
          {WHAT_WE_OFFER.map(f => <div key={f.title} className='bg-white p-6 hover:bg-[#F5F3EF] transition-colors'>
      ...
      <section className='reveal bg-[#0A1F44]'>
        <div className='max-w-5xl mx-auto px-4 md:px-6 py-16'>
          ...
          <div className='grid md:grid-cols-2 gap-6'>
            {SERVICES.map(s => <div key={s.title} className='bg-white/5 border border-white/10 rounded-2xl p-8'>
                ...
                <ul className='space-y-1.5'>
                  {s.items.map(item => <li key={item} className='text-white/60 text-xs flex gap-2'>
      ...
      <section className='reveal max-w-5xl mx-auto px-4 md:px-6 py-16'>
        ...
        <div className='grid md:grid-cols-2 gap-6'>
          {PRICING.map(p => p.accent ? <div key={p.title} className='bg-[#E8553A] rounded-2xl p-8 relative overflow-hidden'>
                ...
                <ul className='space-y-1.5'>
                  {p.points.map(pt => <li key={pt} className='text-white/80 text-xs flex gap-2'>
                      <span className='text-white'>✓</span>
                      {pt}
                    </li>)}
                </ul>
                <Link href='/contact' className='mt-6 inline-block bg-white text-[#E8553A] font-montserrat font-bold text-sm px-6 py-3 rounded-full hover:bg-white/90 transition-colors'>
                  Request a Quote →
                </Link>
              </div> : <div key={p.title} className='bg-white border border-[#0A1F44]/8 rounded-2xl p-8'>
                ...
                <ul className='space-y-1.5'>
                  {p.points.map(pt => <li key={pt} className='text-gray-500 text-xs flex gap-2'>
                      <span className='text-[#E8553A]'>✓</span>
                      {pt}
                    </li>)}
                </ul>
