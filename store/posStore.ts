...
import { fetchCashDrawerState, openCashDrawerRemote, addCashMovementRemote, closeCashDrawerRemote } from '@/lib/api/pos-cash-drawer';
...
export type AuditAction = 'login' | 'logout' | 'pin_change' | 'sale' | 'void' | 'return';
...
interface POSState {
  ...
  updateTerminalSetting: (key: 'soundOnScan' | 'autoPrintReceipt' | 'showStockCount' | 'taxInclusivePricing', value: boolean) => void;
  openSession: (cashierId: string, cashierName: string, openingCash: number) => void;
  ...
  completeOrder: (orderId: string, cashier: string, medusaOrderId?: string) => void;
  ...
  addStaff: (staff: Omit<POSStaffMember, 'id' | 'createdAt' | 'totalSales' | 'totalOrders'>) => void;
...
function computePOSTotals(items: POSCartItem[], customDiscount: number, couponDiscount: number, giftCardAmount: number = 0) {
  const subtotal = items.reduce((sum, item) => sum + item.product.price * item.quantity - (item.discount ?? 0), 0);
...
export const usePOSStore = create<POSState>()(persist((set, get) => ({
  ...
  retryLoadProducts: async () => {
    ...
    if (retries >= 3) {
      set({
        medusaError: 'Max retries reached. Check your Medusa server connection.'
      });
      return;
    }
  ...
  fetchStoreSettings: async () => {
    try {
      ...
      set(computePOSTotals(items, customDiscount, couponDiscount, giftCardAmount));
    } catch (err) {
      console.warn('[POS] fetchStoreSettings failed, keeping defaults:', err);
    }
  },
  updateTerminalSetting: (key, value) => set({
    [key]: value
  } as Pick<POSState, typeof key>),
  logStaffLogin: (staffId, staffName) => set(state => ({
    auditLog: [{
      id: makeAuditId(),
      staffId,
      staffName,
  ...
  logStaffLogout: (staffId, staffName) => set(state => ({
    auditLog: [{
      id: makeAuditId(),
      staffId,
      staffName,
  ...
  openSession: (cashierId, cashierName, openingCash) => set({
    session: {
      id: `session-${Date.now()}`,
      cashierId,
      cashierName,
  ...
  closeSession: closingCash => set(state => ({
  ...
  addItem: (product, quantity = 1, variant) => {
    set(state => {
      ...
      if (existingIndex >= 0) {
        newItems = state.items.map((item, i) => i === existingIndex ? {
          ...item,
      ...
      return {
        items: newItems,
        ...computePOSTotals(newItems, state.customDiscount, state.couponDiscount, state.giftCardAmount)
      };
    });
  },
  removeItem: (productId, variantId) => {
    set(state => {
      const newItems = state.items.filter(item => !(item.product.id === productId && item.variant?.id === variantId));
      return {
        items: newItems,
        ...computePOSTotals(newItems, state.customDiscount, state.couponDiscount, state.giftCardAmount)
      };
  ...
  updateQuantity: (productId, quantity, variantId) => {
    ...
    set(state => {
      const newItems = state.items.map(item => item.product.id === productId && item.variant?.id === variantId ? {
        ...item,
        quantity
      } : item);
      return {
  ...
  setCustomDiscount: discount => set(state => ({
    customDiscount: discount,
    ...computePOSTotals(state.items, discount, state.couponDiscount, state.giftCardAmount)
  })),
  applyCoupon: (code, discount) => set(state => ({
    couponCode: code,
    couponDiscount: discount,
    ...computePOSTotals(state.items, state.customDiscount, discount, state.giftCardAmount)
  })),
  removeCoupon: () => set(state => ({
    couponCode: null,
    couponDiscount: 0,
    ...computePOSTotals(state.items, state.customDiscount, 0, state.giftCardAmount)
  })),
  applyGiftCard: (code, cardBalance) => set(state => ({
    giftCardCode: code,
    ...computePOSTotals(state.items, state.customDiscount, state.couponDiscount, cardBalance)
  })),
  removeGiftCard: () => set(state => ({
    giftCardCode: null,
    ...computePOSTotals(state.items, state.customDiscount, state.couponDiscount, 0)
  })),
  setPaymentMethod: method => set({
    paymentMethod: method
  }),
  setCustomer: (name, phone) => set({
    customerName: name,
    customerPhone: phone
  }),
  setNote: note => set({
  ...
  setFulfillmentType: type => set({
    fulfillmentType: type,
    ...(type === 'pickup' ? {
      shippingAddress: null
    } : {})
  }),
  setShippingAddress: addr => set({
    shippingAddress: addr
  }),
  applyFixedDiscount: amount => set(state => ({
    customDiscount: amount,
    ...computePOSTotals(state.items, amount, state.couponDiscount, state.giftCardAmount)
  })),
  applyPercentageDiscount: percent => set(state => {
  ...
  saveCart: name => set(state => {
    ...
    return {
      savedCarts: [...state.savedCarts, saved]
    };
  }),
  loadCart: id => set(state => {
  ...
  openCashDrawer: (openingCash, cashier) => {
    ...
    openCashDrawerRemote(openingCash, cashier).catch(err => {
      ...
      set({
        cashDrawerSyncError: err instanceof Error ? err.message : 'Failed to sync till open to server'
      });
    });
  },
  closeCashDrawer: closingCash => {
    ...
    const cashIn = state.cashDrawer.movements.filter(m => m.type === 'in').reduce((s, m) => s + m.amount, 0);
    const cashOut = state.cashDrawer.movements.filter(m => m.type === 'out').reduce((s, m) => s + m.amount, 0);
    const expectedCash = state.cashDrawer.openingCash + (state.session?.cashSales ?? 0) + cashIn - cashOut;
    ...
    closeCashDrawerRemote(closingCash, roundedExpected, variance).catch(err => {
      console.error('[posStore] closeCashDrawer sync failed:', err);
      set({
        cashDrawerSyncError: err instanceof Error ? err.message : 'Failed to sync till close to server'
      });
    });
  },
  addCashMovement: (amount, type, reason) => {
    ...
    addCashMovementRemote(amount, type, reason).catch(err => {
      ...
      set({
        cashDrawerSyncError: err instanceof Error ? err.message : 'Failed to sync cash movement to server'
      });
    });
  },
  ...
  completeOrder: (orderId, cashier, medusaOrderId) => {
    const state = get();
    get().deductStock(state.items.map(i => ({
  ...
  processReturn: (orderId, returnItems) => {
    set(state => ({
      completedOrders: state.completedOrders.map(order => order.id === orderId ? {
        ...order,
        returned: true
      } : order)
    }));
    get().restoreStock(returnItems.map(ri => ({
  ...
  addStaff: staff => set(state => ({
    staffList: [...state.staffList, {
      ...
      totalSales: 0,
      totalOrders: 0,
      initials: staff.initials || staff.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)
    }]
  })),
  updateStaff: (id, data) => set(state => ({
    staffList: state.staffList.map(s => s.id === id ? {
      ...s,
      ...data
    } : s)
  })),
  ...
  incrementStaffSales: (name, amount) => set(state => ({
    staffList: state.staffList.map(s => s.name === name ? {
      ...s,
      totalSales: s.totalSales + amount,
      totalOrders: s.totalOrders + 1
    } : s)
  })),
  addRevenueEntry: entry => set(state => {
    const newEntry: RevenueEntry = {
      ...entry,
      timestamp: new Date().toISOString()
    };
    const updatedStaff = entry.cashier ? state.staffList.map(s => s.name === entry.cashier ? {
      ...s,
  ...
  recordCashSale: amount => set(state => ({
    session: state.session ? {
      ...state.session,
      cashSales: state.session.cashSales + amount
    } : null
  })),
  getTotalRevenue: () => get().revenueLog.reduce((s, e) => s + e.amount, 0),
  getOnlineRevenue: () => get().revenueLog.filter(e => e.source === 'online').reduce((s, e) => s + e.amount, 0),
  getPOSRevenue: () => get().revenueLog.filter(e => e.source === 'pos').reduce((s, e) => s + e.amount, 0),
  clearSalesHistory: () => set({
    completedOrders: [],
    revenueLog: []
  }),
  deductStock: items => set(state => ({
    products: state.products.map(p => {
      const found = items.find(i => i.productId === p.id);
      return found ? {
        ...p,
        stock: Math.max(0, p.stock - found.quantity)
      } : p;
    })
  })),
