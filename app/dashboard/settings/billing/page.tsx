...
const Icons = {
  card: <svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
      <rect x='1' y='4' width='22' height='16' rx='2' ry='2' />
      <line x1='1' y1='10' x2='23' y2='10' />
    </svg>,
  external: <svg width='13' height='13' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
...
export default function BillingPage() {
  ...
  useEffect(() => {
    Promise.all([fetch('/api/admin/stripe/account', {
      credentials: 'include'
    }).then(r => r.json()), fetch('/api/admin/stripe/payouts', {
      credentials: 'include'
    }).then(r => r.json())]).then(([accountData, payoutsData]) => {
  ...
  return <div className='space-y-5 max-w-3xl'>
      ...
      {loading ? <div className='flex justify-center py-16'>
        ...
        </div> : !account?.connected ? <div className='bg-white border border-[#E1E3E5] rounded-xl p-6 flex items-start gap-3'>
          ...
          <div className='bg-white border border-[#E1E3E5] rounded-xl p-6'>
            <div className='flex items-center justify-between'>
              <div className='flex items-center gap-3'>
                ...
                <div>
                  <div className='flex items-center gap-2'>
                    ...
                    {account.mode === 'test' && <span className='text-[11px] px-2 py-0.5 rounded-full bg-[#FFC453]/20 text-[#916A00] font-medium'>
                        Test mode
                      </span>}
                  </div>
              ...
              <a href='https://dashboard.stripe.com' target='_blank' rel='noopener noreferrer' className='flex items-center gap-1.5 px-3.5 py-2 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[13px] font-medium text-[#202223] rounded-lg transition-colors'>
          ...
          <div className='grid grid-cols-2 gap-4'>
            <div className='bg-white border border-[#E1E3E5] rounded-xl p-5'>
              ...
              {account.balance?.available.map(b => <p key={b.currency} className='text-[24px] font-semibold text-[#202223] font-sora'>
                  {fmt(b.amount, b.currency)}
                </p>)}
            </div>
            <div className='bg-white border border-[#E1E3E5] rounded-xl p-5'>
              <p className='text-[12px] text-[#8C9196] uppercase tracking-wide mb-1.5'>Pending balance</p>
              {account.balance?.pending.map(b => <p key={b.currency} className='text-[24px] font-semibold text-[#202223] font-sora'>
                  {fmt(b.amount, b.currency)}
                </p>)}
            </div>
          </div>
          ...
          <div className='bg-white border border-[#E1E3E5] rounded-xl overflow-hidden'>
            ...
            {payouts.length === 0 ? <p className='text-[13px] text-[#8C9196] px-5 py-8 text-center'>
                No payouts yet.
              </p> : <div className='divide-y divide-[#F1F1F1]'>
                {payouts.map(p => <div key={p.id} className='px-5 py-3.5 flex items-center justify-between'>
                    <div>
