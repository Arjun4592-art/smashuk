...
//
// Website customer: email/password login.
// Gets a JWT from Medusa and sets the smashuk-auth + smashuk-token cookies.
// The client never receives the token — only the user object.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Step 2: Customer profile lo (JWT se)
    ...
    // Step 3: Set HttpOnly cookies — the client never gets the token
    ...
    setSurfaceCookies(
      ...
      token,
      'lax', // lax — so Google OAuth redirect also works
    )
    return response
  } catch (err: any) {
