...
// UI-facing labels — the actual stored role is only ever 'staff' or 'admin'.
// Keep these matching the role value itself (no "Manager"/"Cashier"/"Owner"
// aliases) so the same person doesn't see a different title on the
// dashboard vs the POS terminal.
...
// API staff object → StaffMember
function toStaff(u: any): StaffMember {
  // Handle both direct API response (has metadata) and
  // already-mapped staff objects from our API route
  const name =
  ...
  return {
    ...
    // POS permission tier only — this is never the dashboard-login gate.
    role: (['admin', 'staff'].includes(u.role ?? '')
      ? u.role
      : ['admin', 'staff'].includes(u.metadata?.posRole ?? '')
        ? u.metadata.posRole
...
export default function StaffPage() {
  const { auditLog, clearAuditLog } = usePOSStore()

  // ── Server-side activity (login/logout — cross-device, see
  // lib/api/staff-activity.ts). Merged with the local `auditLog` below,
  // which still owns sale/void/pin_change entries for now.
  ...
  const fetchServerActivity = useCallback(async () => {
    try {
      ...
      // Non-fatal — Activity Log just falls back to local-only entries.
  ...
  // ── Auth helper ───────────────────────────────────────────────
  // Dashboard JWT is in an HttpOnly cookie — the browser sends it
  // automatically on same-origin requests, no header needed here.
  const authHeaders = () => ({
    'Content-Type': 'application/json',
  })

  // ── Fetch staff ───────────────────────────────────────────────
  // FIX: data.staff (not data.users) — our API route returns { staff, count }
  const fetchStaff = useCallback(async () => {
    ...
    try {
      ...
      const list = data.staff ?? [] // ← FIXED: was data.users
      let staff = list.map(toStaff) as StaffMember[]

      // Real "Orders" / "Total Sales" columns: each POS sale is tagged with
      // metadata.cashier (the staff member's name — see app/api/pos/orders
      // /route.ts). Pull recent orders and tally them per cashier, same
      // name-match already used on the POS Analytics page, instead of
      // leaving these at whatever placeholder totalSales/totalOrders value
      // (usually 0) happened to be on the staff record itself.
      try {
        ...
        for (const o of orders as any[]) {
          ...
          if (!key) continue
          const existing = totalsByName.get(key) ?? { orders: 0, sales: 0 }
          existing.orders += 1
        ...
        staff = staff.map((s) => {
          ...
          return totals
            ? { ...s, totalOrders: totals.orders, totalSales: totals.sales }
            : s
        })
      } catch (ordersErr) {
        // Non-fatal — staff list still loads, just without real sales
        // totals if the orders fetch failed for some reason.
        console.warn(
          '[Staff] failed to compute per-staff sales totals:',
          ordersErr,
        )
      }
      ...
      toast.error('Failed to load staff', { description: err.message })
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const filtered = staffList.filter((s) => {
    ...
    const matchRole =
      roleFilter === 'All' || s.role === ROLE_FILTER_TO_INTERNAL[roleFilter]
    return matchSearch && matchRole
  })

  // ── Combined audit log — local (sale/void/pin_change) + server
  // (login/logout, cross-device). Server logout entries don't carry a
  // staff_name (see lib/api/auth/logout/route.ts), so resolve it from
  // staffList here; falls back to the raw id if that staff was since removed.
  const combinedAuditLog = [
    ...auditLog,
    ...serverActivity.map((e) => ({
      ...
      staffName:
        e.staffName ||
        staffList.find((s) => s.id === e.staffId)?.name ||
        e.staffId,
      action: e.action,
  ...
  const filteredAudit = combinedAuditLog.filter((e) => {
    const matchSearch =
      e.staffName.toLowerCase().includes(auditSearch.toLowerCase()) ||
      (e.detail?.toLowerCase().includes(auditSearch.toLowerCase()) ?? false)
  ...
  const handleSave = async () => {
    if (!form.name) return
    if (!editingId && form.pin.length !== 6) return // new staff must set a PIN
    if (form.pin && form.pin.length !== 6) return // partial PIN typed while editing
    setSaving(true)
    setSaveError(null)

    const toastId = toast.loading(
      editingId ? 'Updating staff...' : 'Adding staff member...',
    )
    ...
    try {
      if (editingId) {
        ...
        const data = await res.json()
        // PATCH route may return data.user or data.staff
  ...
  const handleUpdatePin = async () => {
    ...
    const staffName =
      staffList.find((s) => s.id === pinStaffId)?.name ?? 'Staff member'
    ...
    try {
      ...
      setStaffList((prev) =>
        prev.map((s) => (s.id === pinStaffId ? { ...s, hasPin: true } : s)),
      )
  ...
  // ── Audit log clear ───────────────────────────────────────────
  // Clears BOTH the local sale/void/pin_change entries and the
  // server-tracked login/logout entries — previously this only cleared
  // the local ones, so login/logout kept showing up after "clearing".
  const handleClearAudit = async () => {
    if (!window.confirm('Clear all audit logs? This cannot be undone.')) return
    clearAuditLog()
    try {
      const res = await fetch('/api/admin/staff-activity', { method: 'DELETE' })
      if (!res.ok)
        throw new Error(
      ...
      // Local entries are already gone at this point — surface the partial
      // failure clearly rather than silently leaving stale server entries.
      toast.error(
        'Local log cleared, but server-side entries failed to clear',
        {
          description: err.message,
        },
      )
    ...
    <div className='space-y-5'>
      ...
      <div className='grid grid-cols-2 xl:grid-cols-4 gap-4'>
        {STATS.map((stat) => (
          ...
          >
            <div
              className={`w-10 h-10 ${stat.bg} ${stat.color} rounded-lg shrink-0 flex items-center justify-center`}
            >
              {stat.icon}
            </div>
            <div>
        ...
        <div className='flex items-center border-b border-[#E1E3E5] px-4'>
          {[
            ...
            <button
              ...
              className={`px-4 py-3 text-[13px] font-medium whitespace-nowrap border-b-2 transition-all bg-transparent border-l-0 border-r-0 border-t-0 cursor-pointer ${activeTab === tab.id ? 'border-b-[#008060] text-[#008060]' : 'border-b-transparent text-[#6D7175] hover:text-[#202223]'}`}
            >
              {tab.label}
            </button>
          ))}
          ...
          <>
            <div className='flex items-center gap-3 px-4 py-3 border-b border-[#E1E3E5] flex-wrap'>
              ...
              <select
                ...
                className='px-3 py-2 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] bg-white outline-none cursor-pointer'
              >
                {ROLES.map((r) => (
            ...
            <div className='flex items-center border-b border-[#E1E3E5] overflow-x-auto scrollbar-none px-4'>
              {ROLES.map((r) => (
                <button
                  ...
                  className={`px-4 py-2.5 text-[12.5px] font-medium whitespace-nowrap border-b-2 transition-all bg-transparent border-l-0 border-r-0 border-t-0 cursor-pointer ${roleFilter === r ? 'border-b-[#008060] text-[#008060]' : 'border-b-transparent text-[#6D7175] hover:text-[#202223]'}`}
                >
                  {r}{' '}
                  <span className='ml-1 text-[10.5px] text-[#8C9196]'>
                    {r === 'All'
                      ? staffList.length
                      : staffList.filter(
                          (s) => s.role === ROLE_FILTER_TO_INTERNAL[r],
                        ).length}
                  </span>
                </button>
              ))}
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={7} className='px-4 py-16 text-center'>
                        ...
                        <button
                          onClick={openAdd}
                          className='mt-3 px-4 py-2 bg-[#008060] text-white text-[13px] rounded-lg hover:bg-[#006e52] border-none cursor-pointer'
                        >
                          Add Staff Member
                        </button>
                      </td>
                    ...
                    filtered.map((staff) => (
                      ...
                      >
                        <td className='px-4 py-3'>
                          <div className='flex items-center gap-3'>
                            <div
                              className={`w-9 h-9 rounded-full flex items-center justify-center text-[13px] font-bold shrink-0 ${staff.isActive ? 'bg-[#008060] text-white' : 'bg-[#F1F2F3] text-[#6D7175]'}`}
                            >
                              {staff.initials}
                            </div>
                            <div>
                        ...
                        <td className='px-4 py-3'>
                          <span
                            className={`px-2.5 py-1 rounded-full text-[11px] font-semibold capitalize ${ROLE_STYLES[staff.role] ?? 'bg-gray-100 text-gray-600'}`}
                          >
                        ...
                        <td className='px-4 py-3'>
                          <button
                            ...
                            className={`relative w-9 h-5 rounded-full transition-colors border-none cursor-pointer ${staff.isActive ? 'bg-[#008060]' : 'bg-[#8C9196]'}`}
                          >
                            <span
                              className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${staff.isActive ? 'right-0.5' : 'left-0.5'}`}
                            />
                          </button>
                        </td>
                        <td className='px-4 py-3'>
        ...
        {activeTab === 'audit' && (
          <>
            <div className='flex items-center gap-3 px-4 py-3 border-b border-[#E1E3E5] flex-wrap'>
              ...
              {combinedAuditLog.length > 0 && (
                <button
                  onClick={handleClearAudit}
                  className='px-3 py-2 border border-[#D82C0D]/30 text-[#D82C0D] hover:bg-[#D82C0D]/5 text-[12.5px] font-medium rounded-lg cursor-pointer bg-transparent transition-colors'
                >
                  Clear Log
                </button>
              )}
            ...
            <div className='overflow-x-auto'>
              <table className='w-full'>
                <thead>
                  <tr className='border-b border-[#E1E3E5] bg-[#F6F6F7]/50'>
                    {['Staff Member', 'Action', 'Detail', 'Time'].map(
                      (h, i) => (
                        <th
                          key={h}
                          className={`px-4 py-3 text-[12px] font-semibold text-[#6D7175] uppercase tracking-wide ${i === 3 ? 'text-right' : 'text-left'}`}
                        >
                          {h}
                        </th>
                      ),
                        ...
                        >
                          <td className='px-4 py-3'>
                            <div className='flex items-center gap-2.5'>
                              <div
                                className={`w-8 h-8 rounded-full flex items-center justify-center text-[12px] font-bold shrink-0 ${staff?.isActive ? 'bg-[#008060] text-white' : 'bg-[#F1F2F3] text-[#6D7175]'}`}
                              >
                                {staff?.initials ??
                                  entry.staffName.slice(0, 2).toUpperCase()}
                              </div>
                              <div>
                                <p className='text-[13px] font-medium text-[#202223]'>
                                  {entry.staffName}
                          ...
                          <td className='px-4 py-3'>
                            <span
                              className={`px-2.5 py-1 rounded-full text-[11px] font-semibold capitalize ${ACTION_STYLES[entry.action] ?? 'bg-[#F6F6F7] text-[#6D7175]'}`}
                            >
                ...
                <div>
                  ...
                  <div className='relative'>
                    <input
                      ...
                      placeholder='••••••'
                      maxLength={6}
                      className='w-full px-3.5 py-2.5 pr-10 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] placeholder-[#8C9196] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all'
                    />
                    <button
                      type='button'
                      onClick={() => setShowPinValue((v) => !v)}
                      className='absolute right-2.5 top-1/2 -translate-y-1/2 text-[#8C9196] hover:text-[#202223] bg-transparent border-none cursor-pointer p-0.5'
                      tabIndex={-1}
                    >
                      {showPinValue ? (
                        <svg
          ...
          <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-95 p-6'>
            ...
            <div className='relative mb-4'>
              <input
                ...
                maxLength={6}
                className='w-full px-3.5 py-2.5 pr-10 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] placeholder-[#8C9196] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all text-center tracking-widest text-[18px]'
              />
              <button
                type='button'
                onClick={() => setShowNewPinValue((v) => !v)}
                className='absolute right-2.5 top-1/2 -translate-y-1/2 text-[#8C9196] hover:text-[#202223] bg-transparent border-none cursor-pointer p-0.5'
                tabIndex={-1}
              >
                {showNewPinValue ? (
            ...
            <div className='flex gap-3'>
              <button
                onClick={() => setShowPinModal(false)}
                className='flex-1 py-2.5 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[13px] font-medium text-[#202223] rounded-lg cursor-pointer'
              >
                Cancel
              </button>
              <button
                ...
                className='flex-1 py-2.5 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-semibold rounded-lg border-none cursor-pointer disabled:opacity-50'
              >
                Update PIN
              </button>
            </div>
          ...
          <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-95 p-6'>
            ...
            <div className='flex gap-3'>
              ...
              <button
                ...
                className='flex-1 py-2.5 bg-[#D82C0D] hover:bg-[#be2209] text-white text-[13px] font-semibold rounded-lg border-none cursor-pointer'
              >
                Remove
              </button>
            </div>
