...
> = {
  admin: { label: 'Admin', bg: '#FFF3CD', color: '#B7791F' },
  staff: { label: 'Staff', bg: '#E3F1EB', color: '#008060' },
}

export default function POSProfileModal({ user, onClose, onLogout }: Props) {
  const { staffList, updateStaffPin } = usePOSStore()
          ...
          <>
            ...
            <div className='p-5'>
              <div className='flex flex-col items-center text-center mb-4'>
                ...
                <span
                  className='mt-1.5 text-xs font-medium px-2.5 py-0.5 rounded-full'
                ...
                >
                  {staff.email && (
                    <div className='flex justify-between text-xs'>
                      <span style={{ color: '#8C9196' }}>Email</span>
                      <span style={{ color: '#202223' }}>{staff.email}</span>
                    </div>
                  )}
                  {staff.phone && (
                    <div className='flex justify-between text-xs'>
                      <span style={{ color: '#8C9196' }}>Phone</span>
                      <span style={{ color: '#202223' }}>{staff.phone}</span>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className='px-5 pb-5 space-y-2'>
              <button
                ...
                className='w-full py-2.5 rounded-lg text-sm font-medium border transition-colors hover:bg-[#F6F6F7]'
              ...
              <button
                onClick={onLogout}
                className='w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold transition-colors hover:bg-[#FFF4F4]'
                style={{ border: '1px solid #FECACA', color: '#D82C0D' }}
              >
                <svg
                  width='14'
                  height='14'
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      placeholder='••••'
                      maxLength={4}
                      className='w-full px-3 py-2.5 rounded-lg border text-sm outline-none focus:border-[#008060]'
                      style={{
