...
export default function CartDrawer({ open, onClose }: CartDrawerProps) {
  const { items, subtotal, shipping, tax, taxRate, total, removeItem, updateQuantity } =
    useCartStore()
  ...
  return (
    <>
      {/* ── Backdrop ── */}
      <div
        className={`fixed inset-0 z-70 bg-[#0A1F44]/60 backdrop-blur-sm transition-opacity duration-300 ${
      ...
      <div
        ...
        className={`fixed top-0 right-0 bottom-0 z-80 w-full max-w-[420px] bg-[#F2F4F7] flex flex-col transition-transform duration-300 ease-in-out ${
