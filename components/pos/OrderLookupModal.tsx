...
export default function OrderLookupModal({ onClose }: Props) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `/api/pos/orders/lookup?q=${encodeURIComponent(query.trim())}`,
        {
          credentials: 'include',
        },
      )
  ...
  const handleAction = async (
    ...
    try {
      ...
      if (action === 'pickup') {
        // Uses existing lookup PATCH — marks fulfillment as delivered + captures COD
        ...
        // dispatch — creates fulfillment (stops at fulfilled, not delivered)
        ...
        (prev ?? []).map((o) =>
          o.id === order.id
            ? {
                ...o,
                fulfillmentStatus:
                  action === 'pickup' ? 'delivered' : 'fulfilled',
  ...
  // What action (if any) should we show for this order?
  // pickup + not_fulfilled / fulfilled → show "Confirm Pickup" (fulfilled means ready, not collected)
  // pickup + delivered               → done, no action
  // delivery + not_fulfilled         → show "Mark as Dispatched"
  // delivery + fulfilled / delivered → done, no action
  function getAction(o: LookupOrder): 'pickup' | 'dispatch' | 'done' {
    ...
    if (o.isPickup) {
      ...
      return 'pickup' // not_fulfilled OR fulfilled (awaiting pickup) — both need confirmation
    } else {
      if (s === 'not_fulfilled' || s === 'requires_action' || !s)
        return 'dispatch'
      return 'done' // fulfilled / delivered / shipped
    }
  }
  ...
  return (
    ...
    >
      <div
        className='w-full max-w-md rounded-xl overflow-hidden max-h-[85vh] flex flex-col'
        style={{
          background: '#FFFFFF',
      ...
      >
        ...
        <div className='p-5 space-y-3'>
          <p className='text-[12px]' style={{ color: '#6D7175' }}>
          ...
          <div className='flex gap-2'>
            <input
              ...
              placeholder='SR-1042, phone, or email'
              autoFocus
              className='flex-1 px-3 py-2.5 rounded-lg border text-sm outline-none focus:border-[#008060]'
          ...
          {error && (
            <p className='text-[12.5px]' style={{ color: '#D82C0D' }}>
              {error}
            </p>
          )}
        </div>
            ...
            {results.map((o) => {
              ...
              return (
                <div
                  ...
                  style={{ borderColor: '#E1E3E5', background: '#FAFAFA' }}
                >
                  {/* Order number + delivery type badge */}
                  ...
                  {/* Customer */}
                  <p className='text-[12.5px]' style={{ color: '#202223' }}>
                    {o.customerName}
                    {o.phone && ` · ${o.phone}`}
                  </p>

                  {/* Items */}
                  <p className='text-[11.5px]' style={{ color: '#8C9196' }}>
                  ...
                  {action === 'done' ? (
                    ...
                    <button
                      onClick={() => handleAction(o, 'pickup')}
                      disabled={actioningId === o.id}
                      className='w-full mt-1 py-2.5 rounded-lg text-[12.5px] font-semibold disabled:opacity-50 flex items-center justify-center gap-1.5'
                      style={{ background: '#008060', color: '#FFFFFF' }}
                    >
                      {actioningId === o.id
                        ? 'Completing…'
                    ...
                    <button
                      onClick={() => handleAction(o, 'dispatch')}
                      disabled={actioningId === o.id}
                      className='w-full mt-1 py-2.5 rounded-lg text-[12.5px] font-semibold disabled:opacity-50 flex items-center justify-center gap-1.5'
                      style={{ background: '#2C6ECB', color: '#FFFFFF' }}
                    >
                      {actioningId === o.id
                        ? 'Updating…'
                        : '📦 Mark as Dispatched'}
