...
export default function POSBottomTabs({
  ...
  return (
    // Hidden on laptop (lg+), visible on mobile + iPad
    <nav
    ...
    >
      {TABS.map(({ id, label, Icon }) => {
        ...
        return (
          <button
            key={id}
            onClick={() => setTab(id)}
            className='flex-1 flex flex-col items-center justify-center gap-1 pt-2 pb-2 relative'
            style={{ minHeight: 56 }}
          >
            {/* Active top bar */}
            {isActive && (
            ...
            {/* Cart badge */}
            {id === 'billing' && itemCount > 0 && (
              <span
                className='absolute rounded-full text-white font-bold flex items-center justify-center'
                style={{
                  top: 4,
                  right: 'calc(50% - 22px)',
                  minWidth: 16,
      ...
      <button
        onClick={onMore}
        className='flex-1 flex flex-col items-center justify-center gap-1 pt-2 pb-2'
        style={{ minHeight: 56 }}
      >
        <svg
          width='24'
