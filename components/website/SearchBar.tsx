...
export default function SearchBar({
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        const region = await medusaStore.store.region
          .list()
          .then((r: any) => r.regions[0])
        // BUG FIX: the Medusa SDK's `product.list()` return type wasn't
        // narrow enough for TS to carry through `.map(normalizeProduct)`,
        // so the `.filter((p) => ...)` callback below was implicitly-`any`
        // (TS7006), failing `next build` under strict mode.
        ...
        // matches ShopClient.tsx / HomePageProducts.tsx.
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <div
        className={`flex items-center gap-2 bg-white border rounded-xl px-4 py-2.5 transition-all duration-200 ${focused ? 'border-[#E8553A] shadow-[0_0_0_3px_rgba(232,85,58,0.08)]' : 'border-[#E5E7EB] hover:border-[#D1D5DB]'}`}
      >
        {isSearching ? (
          <svg
            className='w-[18px] h-[18px] text-[#E8553A] animate-spin shrink-0'
            viewBox='0 0 24 24'
            fill='none'
          >
            <circle
          ...
          <SearchIcon
            size={18}
            className={`shrink-0 transition-colors ${focused ? 'text-[#E8553A]' : 'text-[#9CA3AF]'}`}
          />
        )}
        ...
        <input
          ...
          className='flex-1 outline-none text-sm text-[#0A1F44] placeholder-[#9CA3AF] font-lato bg-transparent'
          autoComplete='off'
          spellCheck={false}
        />
        ...
        <button
          ...
          className='bg-[#E8553A] hover:bg-[#D4441F] text-white text-[11px] font-black px-3 py-1.5 rounded-lg transition-colors font-montserrat shrink-0 whitespace-nowrap'
        >
          Search
        </button>
      </div>
      ...
      {showDropdown && (
        <div className='absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-[0_16px_48px_rgba(0,0,0,0.12)] border border-[#E5E7EB] z-50 overflow-hidden'>
          {hasQuery && results.length > 0 && (
            <>
              ...
              {results.map((product, i) => (
                ...
                >
                  ...
                  <div className='text-right shrink-0'>
                    {product.badge && (
                      <p className='text-[9px] font-black text-[#E8553A] font-montserrat uppercase tracking-wide mb-0.5'>
                        {product.badge}
                      </p>
                    )}
                    ...
                    {product.originalPrice && (
                      <p className='text-[10px] text-[#9CA3AF] line-through font-lato'>
              ...
              <div className='border-t border-[#F2F4F7] px-4 py-3'>
                <button
                  onClick={() => handleSearch()}
                  className='flex items-center gap-1.5 text-[13px] text-[#E8553A] font-bold font-montserrat hover:gap-2.5 transition-all group'
                >
              ...
              <div className='flex flex-wrap gap-1.5 justify-center'>
                {SPORTS.map((sport) => (
                  <Link
                    ...
                    className='flex items-center gap-1 px-3 py-1.5 bg-[#F2F4F7] hover:bg-[#E8553A]/8 hover:text-[#E8553A] rounded-full text-[12px] text-[#4B5563] transition-colors font-lato border border-transparent hover:border-[#E8553A]/20'
                  >
                ...
                <div>
                  ...
                  <div className='space-y-0.5'>
                    {recentSearches.map((term) => (
                      <div
                        key={term}
                        className='flex items-center gap-2 px-2 py-1.5 rounded-xl hover:bg-[#F2F4F7] transition-colors group cursor-pointer'
                      ...
                      >
                        ...
                        <button
                          ...
                          className='opacity-0 group-hover:opacity-100 transition-opacity text-[#9CA3AF] hover:text-[#0A1F44]'
                        >
                          <CloseIcon size={12} />
                        </button>
                      </div>
                ...
                <div className='flex flex-wrap gap-1.5'>
                  {POPULAR_SEARCHES.map((term) => (
                    <button
                      key={term}
                      onClick={() => handleSearch(term)}
                      className='flex items-center gap-1.5 px-3 py-1.5 bg-[#F2F4F7] hover:bg-[#E8553A]/8 hover:text-[#E8553A] border border-transparent hover:border-[#E8553A]/20 rounded-full text-[12px] text-[#4B5563] transition-all font-lato'
                    >
                ...
                <div className='grid grid-cols-2 gap-1.5'>
                  {SPORTS.map((sport) => (
                    <Link
                      ...
                      href={`/shop?sport=${sport.slug}`}
                      onClick={() => setFocused(false)}
                      className='flex items-center gap-2 px-3 py-2 bg-[#F2F4F7] hover:bg-[#E8553A]/8 hover:text-[#E8553A] rounded-xl text-[12px] text-[#0A1F44] font-lato transition-all border border-transparent hover:border-[#E8553A]/20 group'
                    >
                      ...
                      <ArrowRightIcon
                        size={11}
                        className='ml-auto text-[#D1D5DB] group-hover:text-[#E8553A] group-hover:translate-x-0.5 transition-all'
                      />
                    </Link>
                  ))}
                </div>
          ...
          {hasQuery && results.length > 0 && (
            <div className='border-t border-[#F2F4F7] px-4 py-2 flex items-center gap-4'>
              {[
                ['↑↓', 'navigate'],
                ['↵', 'select'],
                ['Esc', 'close'],
                ...
                <span
                  key={key}
                  className='text-[10px] text-[#9CA3AF] font-lato flex items-center gap-1'
                >
