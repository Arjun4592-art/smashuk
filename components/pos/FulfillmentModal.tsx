...
export default function FulfillmentModal({ onClose, onSave }: Props) {
  ...
  // Shipped orders need a real email — it's how the customer gets tracking
  // updates and their receipt, and there's no other point in this flow
  // where a walk-in/guest customer's email gets captured.
  const addressValid =
    !!addr.first_name &&
    !!addr.address_1 &&
    !!addr.city &&
    !!addr.postal_code &&
    EMAIL_RE.test(addr.email)
