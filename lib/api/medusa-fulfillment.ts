...
export async function markOrderDelivered(orderId: string, fetcher: Fetcher) {
  ...
  if (
    ...
    throw new Error(
      'Order must be fulfilled before it can be marked as delivered.',
    )
  }
  ...
  if (fulfillments.length === 0) {
    throw new Error(
      'No active fulfillment found on this order to mark as delivered.',
    )
  }
  const results = []
  for (const f of fulfillments) {
    ...
    if (!deliverRes.ok) {
      throw new Error(
        deliverData?.message ??
          `Failed to mark fulfillment ${f.id} as delivered (${deliverRes.status})`,
      )
    }
...
export async function shipOrder(orderId: string, fetcher: Fetcher) {
  ...
  if (
    ...
    throw new Error(
      'Order must be marked as fulfilled/ready before it can be dispatched.',
    )
  }
  const fulfillments = (order.fulfillments ?? []).filter(
  ...
  for (const f of fulfillments) {
    ...
    if (!shipRes.ok) {
      throw new Error(
        shipData?.message ??
          `Failed to dispatch fulfillment ${f.id} (${shipRes.status})`,
      )
    }
