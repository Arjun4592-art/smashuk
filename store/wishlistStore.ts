...
interface WishlistState {
  ...
  clear: () => void
  // Pulls the saved wishlist from Medusa (customer.metadata.wishlist) and
  // merges it with whatever's currently local, so the wishlist follows the
  // customer across devices/browsers instead of being tied to one
  // localStorage. No-ops silently if the customer isn't logged in.
  hydrateFromServer: () => Promise<void>
}

// Debounced so rapid toggling (e.g. clicking several hearts quickly)
// doesn't fire a network request per click.
let saveTimer: ReturnType<typeof setTimeout> | null = null
function scheduleSave(items: Product[]) {
  if (saveTimer) clearTimeout(saveTimer)
  saveTimer = setTimeout(() => {
    saveWishlist(items.map((p) => p.id)).catch((err) =>
      ...
      hydrateFromServer: async () => {
        try {
          ...
          set((state) => {
            ...
            // Only keep items that are actually still on the server-saved
            // wishlist (drops anything a guest-mode toggle added locally
            // before the server list was known).
