...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const parcel2goProviderId =
      providers.find((p) => /parcel ?2 ?go/i.test(p.id))?.id ?? null
    function providerLabel(providerId: string | null | undefined): string {
      if (!providerId) return 'Unknown'
      if (/parcel ?2 ?go/i.test(providerId)) return 'Parcel2Go'
    ...
    for (const opt of options) {
      ...
      zonesById.get(zone.id).options.push({
        ...
        provider_mismatch:
          !isPickup &&
          !!parcel2goProviderId &&
          opt.provider_id !== parcel2goProviderId,
        is_pickup: isPickup,
      })
    }
    return NextResponse.json({
      zones: Array.from(zonesById.values()),
      parcel2go_provider_id: parcel2goProviderId,
    })
  } catch (err: any) {
    return NextResponse.json(
      {
  ...
  try {
    ...
    if (!resolvedProviderId) {
      ...
      if (is_pickup) {
        ...
        const parcel2go = providers.find((p) => /parcel ?2 ?go/i.test(p.id))
        if (!parcel2go) {
          return NextResponse.json(
            {
              error:
                'No Parcel2Go fulfillment provider is registered on the backend yet, so a ' +
        ...
        resolvedProviderId = parcel2go.id
      }
    }
    const res = await fetch(`${MEDUSA_URL}/admin/shipping-options`, {
      ...
      body: JSON.stringify({
        ...
        // Tells the Parcel2Go provider which of its two options this is; the
        // service slug itself comes from PARCEL2GO_SERVICE_ID_* in the backend .env.
        data: is_pickup
          ? undefined
          : {
              id: /express|next.?day|24/i.test(name)
                ? 'parcel2go-express'
                : 'parcel2go-standard',
            },
        rules: is_pickup
