...
// components/dashboard/BlogPostForm.tsx
//
// Redesigned as a WordPress-style post editor:
// - Left column: title, permalink, content editor, excerpt
// - Right sidebar: Publish box (Draft/Published + Publish button),
//   Category box, Featured Image box, SEO box (search preview + meta
//   title/description/keywords, all saved on the post so the public
//   blog pages can use them for <head> tags)
//
// SEO fields (seo_title / seo_description / seo_keywords) are sent to
// the same /api/admin/blogs endpoint alongside the existing fields, so
// the Medusa blog-post module needs matching columns for them to persist
// — if the backend doesn't have those columns yet, Medusa will either
// ignore the extra keys or error depending on how strict the module's
// validator is. Ask me to add the backend migration if posts don't save.
...
export default function BlogPostForm({ postId }: Props) {
  ...
  const inputClass =
    'w-full px-3 py-2 text-[13px] border border-[#E1E3E5] rounded-lg outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/15 transition-all'
  ...
  return (
    <div className='p-6 max-w-6xl'>
      {/* Top bar */}
      <div className='flex items-center justify-between mb-6 sticky top-0 bg-[#F6F6F7] z-10 -mx-6 px-6 py-3'>
        <div>
        ...
        <button
          ...
          className='px-3.5 py-2 text-[13px] font-medium text-[#202223] bg-white border border-[#E1E3E5] rounded-lg cursor-pointer hover:bg-[#F6F6F7]'
        >
          Cancel
        </button>
      </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start'>
        ...
        <div className='flex flex-col gap-4 min-w-0'>
          <div className='bg-white border border-[#E1E3E5] rounded-xl p-4'>
            <input
              className='w-full text-[24px] font-semibold text-[#202223] placeholder-[#C4C7CA] outline-none border-none mb-2'
              value={title}
              onChange={(e) => {
            ...
            <div className='flex flex-wrap items-center gap-1.5 text-[12.5px] text-[#6D7175]'>
              ...
              {editingSlug ? (
                <>
                  ...
                  <input
                    autoFocus
                    className='px-2 py-1 border border-[#E1E3E5] rounded-md text-[12.5px] outline-none focus:border-[#008060]'
                    value={slug}
                    onChange={(e) => setSlug(slugify(e.target.value))}
                    onBlur={() => setEditingSlug(false)}
                    onKeyDown={(e) =>
                      e.key === 'Enter' && setEditingSlug(false)
                    }
                  />
                  <button
                    type='button'
                    onClick={() => setEditingSlug(false)}
                    className='px-2.5 py-1 text-[12px] font-medium text-white bg-[#008060] rounded-md cursor-pointer hover:bg-[#006e52]'
                  >
                    OK
                  </button>
                </>
                ...
                <>
                  ...
                  <button
                    ...
                    className='px-2 py-0.5 text-[12px] font-medium text-[#2C6ECB] bg-transparent border-none cursor-pointer hover:underline'
                  >
                    Edit
                  </button>
                </>
          ...
          <SidebarCard title='Publish'>
            <div className='flex flex-col gap-3'>
              <div className='flex items-center justify-between text-[13px]'>
                ...
                <span
                  className={`font-medium px-2 py-0.5 rounded-full text-[11.5px] ${
                    status === 'published'
                      ? 'bg-[#E3F5E9] text-[#008060]'
              ...
              <div className='flex gap-2 pt-1 border-t border-[#F1F1F1]'>
                <button
                  ...
                  disabled={saving}
                  className='flex-1 px-3.5 py-2 text-[13px] font-medium text-[#202223] bg-white border border-[#E1E3E5] rounded-lg cursor-pointer hover:bg-[#F6F6F7] disabled:opacity-60'
                >
                  Save Draft
                </button>
                <button
                  ...
                  disabled={saving}
                  className='flex-1 px-3.5 py-2 text-[13px] font-medium text-white bg-[#008060] rounded-lg cursor-pointer hover:bg-[#006e52] disabled:opacity-60'
                >
                  {saving
                    ? 'Saving...'
                    : status === 'published'
            ...
            <div className='flex flex-col gap-4'>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder='badminton racket, string tension'
                />
              </div>

              {/* Schema — auto-generated, read-only. No manual JSON entry:
                  a hand-typed schema field is the #1 way plugins like Yoast
                  let users accidentally break a page (one typo in raw JSON
                  and the <script> tag either breaks the page or gets
                  silently ignored by Google). This is built from the
                  fields above every time the post saves, so it can never
                  go invalid. */}
              <div>
                <label className={labelClass}>
