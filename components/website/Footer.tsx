...
const PARTNERSHIP_LINKS = [
  ...
  { label: 'Club Kit Printing', href: '/local-store/kit-printing' },
  { label: 'Sponsorship Programme', href: '/local-store/sponsorship' },
]
    ...
    <footer className='bg-[#2C2C3E]'>
      ...
      <div className='bg-[#E8553A]'>
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-4'>
          ...
          <Link
            href='/register'
            className='inline-flex items-center gap-2 bg-white text-[#E8553A] font-montserrat font-black text-sm px-7 py-3 rounded-full hover:bg-[#FDF0ED] transition-colors whitespace-nowrap shadow-lg shrink-0'
          >
            Create Free Account
            ...
            <div className='flex items-center gap-2 mb-7'>
              {SOCIAL_LINKS.map((social) => (
                <a
                  ...
                  className='w-9 h-9 rounded-xl bg-white/8 hover:bg-[#E8553A] flex items-center justify-center text-white/40 hover:text-white transition-all duration-200'
                >
                  {social.icon}
                </a>
              ))}
              ...
              {SPORTS.map((sport) => (
                <li key={sport.slug}>
                  <Link
                    ...
                    className='flex items-center gap-2.5 text-[13px] text-white/50 hover:text-[#E8553A] transition-colors font-lato group'
                  >
            ...
            <ul className='space-y-3'>
              {QUICK_LINKS.map((link) => (
                <li key={link.href + link.label}>
                  <Link
                    href={link.href}
                    className='text-[13px] text-white/50 hover:text-[#E8553A] transition-colors font-lato flex items-center gap-2 group'
                  >
            ...
            <ul className='space-y-3.5 mb-7'>
              ...
              <li className='flex items-center gap-3'>
                ...
                <a
                  ...
                  className='text-[13px] text-white/50 hover:text-[#E8553A] transition-colors font-lato'
                >
                  {contact.phone}
                </a>
              </li>
            ...
            <div className='flex items-center gap-4 flex-wrap justify-center'>
              {POLICY_LINKS.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className='text-[11px] text-white/30 hover:text-white/60 transition-colors font-lato'
                >
                  {link.label}
                </Link>
              ))}
