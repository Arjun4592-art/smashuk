...
export default function ProductReviews({
  ...
  useEffect(() => {
    fetch(`/api/store/products/${productId}/reviews`).then(r => r.json()).then(d => setReviews(d.reviews ?? [])).catch(() => setReviews([])).finally(() => setLoading(false));
  }, [productId]);
  const count = reviews.length || initialCount;
  const avg = reviews.length > 0 ? reviews.reduce((s, r) => s + r.rating, 0) / reviews.length : initialRating;
  ...
  if (loading) {
    return <div className='space-y-4 animate-pulse'>
        {[...Array(3)].map((_, i) => <div key={i} className='bg-gray-100 rounded-2xl h-28' />)}
      </div>;
  }
  return <div className='space-y-6'>
      ...
      <div className='bg-[#F8F9FB] rounded-2xl p-6 flex flex-col sm:flex-row items-center gap-6'>
        ...
        <div className='text-center shrink-0'>
          ...
          <div className='flex items-center justify-center gap-0.5 my-1'>
            {[1, 2, 3, 4, 5].map(s => <StarIcon key={s} size={16} filled={s <= Math.round(avg)} className={s <= Math.round(avg) ? 'text-amber-400' : 'text-gray-200'} />)}
          </div>
        ...
        <div className='flex-1 w-full space-y-1.5'>
          ...
          pct
              ...
              <div className='flex-1 bg-gray-200 rounded-full h-2 overflow-hidden'>
                <div className='bg-amber-400 h-2 rounded-full transition-all duration-500' style={{
              width: `${pct}%`
            }} />
              </div>
      ...
      {}
      {reviews.length === 0 ? <div className='text-center py-10'>
          ...
          {reviews.map(review => <div key={review.id} className='bg-white border border-gray-100 rounded-2xl p-5'>
              {}
              <div className='flex items-start justify-between gap-3 mb-3'>
                ...
                <div className='text-right shrink-0'>
                  <div className='flex items-center gap-0.5 justify-end'>
                    {[1, 2, 3, 4, 5].map(s => <StarIcon key={s} size={13} filled={s <= review.rating} className={s <= review.rating ? 'text-amber-400' : 'text-gray-200'} />)}
                  </div>
              ...
              {}
              {review.title && <p className='font-montserrat font-bold text-sm text-[#0A1F44] mb-1'>{review.title}</p>}
