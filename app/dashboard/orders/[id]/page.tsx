...
// This page didn't exist at all before — every order row in the orders
// list already linked to /dashboard/orders/[id], so clicking any order
// 404'd. This also finally surfaces per-item metadata (like the POS
// "String Upgrade" choice) which was being stored on orders but never
// shown anywhere.
...
// Fields we already show elsewhere on the item row, or that are purely
// internal — everything else on an item's metadata gets displayed as a
// "Notes" chip (this is how the POS String Upgrade choice surfaces here).
...
export default function OrderDetailPage({
  ...
  // Replaces the browser's native window.confirm() popup with an in-app
  // modal (see the confirm modal rendered near the bottom of this
  // component) — holds which action is pending confirmation, or null when
  ...
  // Actually performs the status change. All actions are routed through
  // the confirm modal below instead of calling this directly.
  ...
  // Entry point for buttons — shows the in-app confirm modal for every
  // action instead of the browser's native window.confirm() popup.
  const requestAction = (
    action:
      | 'confirm'
      | 'cancel'
  ...
  // Remaining returnable qty per line item — total qty minus whatever's
  // already refunded or sitting in a pending ('requested') record.
  // Mirrors lib/api/medusa-returns.ts's getRemainingReturnableQty, kept
  // as a small client-side copy since that file is server-only.
    ...
    return (
      <div className='p-6'>
        <Link
          ...
          className='text-[13px] text-[#008060] hover:underline'
        >
          ← Back to Orders
        </Link>
  ...
  // Store-pickup orders (POS or website "Store Pickup") skip the
  // dispatch/courier leg entirely — the customer collects in person, so
  // there's nothing to "dispatch". Only home-delivery orders get the
  // Dispatch button between Fulfilled and Delivered.
    ...
    <div className='p-6 max-w-5xl mx-auto'>
      ...
      <div className='flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-6'>
        <div>
          <Link
            href='/dashboard/orders'
            className='text-[13px] text-[#008060] hover:underline mb-2 inline-block'
          >
            ← Back to Orders
          </Link>
          <div className='flex flex-wrap items-center gap-2'>
            ...
            <span
              className={`px-2.5 py-1 rounded-full text-[11px] font-semibold capitalize whitespace-nowrap ${STATUS_STYLES[order.status] ?? 'bg-gray-100 text-gray-600'}`}
            >
              {order.status}
            </span>
            <span
              className={`px-2.5 py-1 rounded-full text-[11px] font-semibold capitalize whitespace-nowrap ${PAYMENT_STYLES[order.payment_status] ?? 'bg-gray-100 text-gray-600'}`}
            >
              {order.payment_status?.replace('_', ' ')}
            </span>
            <span
              className={`px-2.5 py-1 rounded-full text-[11px] font-semibold capitalize whitespace-nowrap ${FULFILLMENT_STYLES[order.fulfillment_status] ?? 'bg-gray-100 text-gray-600'}`}
            >
              {(order.fulfillment_status ?? 'not_fulfilled').replace(/_/g, ' ')}
            </span>
            {isPOS && (
              <span className='px-2.5 py-1 rounded-full text-[11px] font-semibold bg-[#2C6ECB]/10 text-[#2C6ECB] whitespace-nowrap'>
                🖥️ POS
              </span>
            )}
          </div>
        ...
        <div className='flex flex-wrap items-center gap-2 lg:justify-end shrink-0'>
          {order.status === 'pending' && (
            <button
              ...
              className='px-3.5 py-2 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-medium rounded-lg transition-colors disabled:opacity-50 whitespace-nowrap'
            >
          ...
          {(order.fulfillment_status === 'not_fulfilled' ||
            ...
            order.status !== 'canceled' && (
              <button
                onClick={() => requestAction('fulfill')}
                disabled={!!actionLoading}
                className='px-3.5 py-2 bg-[#2C6ECB] hover:bg-[#1a4f9e] text-white text-[13px] font-medium rounded-lg transition-colors disabled:opacity-50 whitespace-nowrap'
              >
          ...
          {!isPickup && isReadyToDispatch && order.status !== 'canceled' && (
            <button
              onClick={() => requestAction('ship')}
              disabled={!!actionLoading}
              className='px-3.5 py-2 bg-purple-600 hover:bg-purple-700 text-white text-[13px] font-medium rounded-lg transition-colors disabled:opacity-50 whitespace-nowrap'
            >
              {actionLoading === 'ship' ? 'Dispatching…' : 'Dispatch Order'}
            </button>
          )}
          {((isPickup && isReadyToDispatch) || isShipped) && (
            <button
          ...
          {order.status !== 'canceled' && order.status !== 'archived' && (
            <button
              onClick={() => requestAction('cancel')}
              disabled={!!actionLoading}
              className='px-3.5 py-2 border border-[#D82C0D] text-[#D82C0D] hover:bg-[#FFF4F4] text-[13px] font-medium rounded-lg transition-colors disabled:opacity-50 whitespace-nowrap'
            >
          ...
          {order.status !== 'archived' && (
            <button
              ...
              disabled={!!actionLoading}
              className='px-3.5 py-2 border border-[#E1E3E5] text-[#6D7175] hover:bg-[#F6F6F7] text-[13px] font-medium rounded-lg transition-colors disabled:opacity-50 whitespace-nowrap'
            >
          ...
          {order.payment_status !== 'not_paid' &&
            Object.values(remainingReturnQty).some((qty) => qty > 0) && (
              <button
                ...
                className='px-3.5 py-2 border border-[#008060] text-[#008060] hover:bg-[#F2F7F5] text-[13px] font-medium rounded-lg transition-colors whitespace-nowrap'
              >
                Process Return
              </button>
            )}
      ...
      {/* Confirm modal — replaces window.confirm() for confirm/cancel/
          archive/capture actions. */}
      {confirmAction && (
        <div
              ...
              {confirmAction === 'capture'
                ...
                : confirmAction === 'ship'
                  ...
                  : confirmAction === 'deliver'
                    ...
                    : `${actionLabels[confirmAction][0].toUpperCase()}${actionLabels[confirmAction].slice(1)} order?`}
            </h3>
            <p className='text-[13px] text-[#6D7175] mb-5'>
              {confirmAction === 'capture'
                ? 'Confirm the cash/card payment for this order has been received.'
                : confirmAction === 'ship'
                  ? "Confirm this order has been handed to the courier/left the store. This marks it 'Dispatched' for the customer."
                  : confirmAction === 'deliver'
                    ? isPickup
                      ? 'Confirm the customer has collected this order in-store.'
                      : 'Confirm the customer has actually received this order.'
                    : `Are you sure you want to ${actionLabels[confirmAction]} this order? This action can't be undone from here.`}
            </p>
            <div className='flex justify-end gap-2'>
              <button
                onClick={() => setConfirmAction(null)}
                className='px-3.5 py-2 border border-[#E1E3E5] text-[#6D7175] hover:bg-[#F6F6F7] text-[13px] font-medium rounded-lg transition-colors'
              >
                Go back
              </button>
              <button
                ...
                className={`px-3.5 py-2 text-white text-[13px] font-medium rounded-lg transition-colors ${
                  confirmAction === 'cancel'
                    ? 'bg-[#D82C0D] hover:bg-[#b8250b]'
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-5'>
        ...
        <div className='lg:col-span-2 space-y-5'>
          <div className='bg-white border border-[#E1E3E5] rounded-xl overflow-hidden'>
                ...
                return (
                  ...
                  >
                    <div className='w-14 h-14 rounded-lg bg-[#F6F6F7] border border-[#E1E3E5] flex items-center justify-center overflow-hidden shrink-0'>
                      ...
                      <span
                        className={`text-[9.5px] text-[#8C9196] text-center leading-tight px-1 ${item.thumbnail ? 'hidden' : ''}`}
                      >
                        No image
                      </span>
                    </div>
                        ...
                        {notes.length > 0 && (
                          <div className='flex flex-wrap gap-1.5 mt-2'>
                            {notes.map(([k, v]) => (
                              <span
                                key={k}
                                className='text-[10.5px] px-2 py-1 rounded-md bg-[#F2F7F5] text-[#008060] font-medium'
                              >
            ...
            <div className='px-5 py-4 bg-[#FAFAFA] border-t border-[#E1E3E5] space-y-1.5'>
              ...
              {order.discount_total > 0 && (
                <div className='flex justify-between text-[13px] text-[#D82C0D]'>
                  <span>Discount</span>
          ...
          {/* Order-level metadata (e.g. POS cashier / payment method / note) */}
          ...
          {/* Returns — staff-processed and customer-requested, newest first */}
                ...
                {[...order.metadata.returns].reverse().map((r: any) => (
                  ...
                  >
                    <div className='flex items-center justify-between mb-1'>
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10.5px] font-semibold capitalize ${
                          r.status === 'refunded'
          ...
          {order.payments?.length > 0 && (
            <div className='bg-white border border-[#E1E3E5] rounded-xl p-5'>
              ...
              {order.payments.map((p: any) => (
                <div
                  key={p.id}
                  className='flex items-center justify-between text-[13px] text-[#202223] mb-1.5'
                >
                  <span>
                  ...
                  <span
                    className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${p.captured_at ? 'bg-[#008060]/10 text-[#008060]' : 'bg-[#FFC453]/20 text-[#916A00]'}`}
                  >
                    {p.captured_at ? 'Captured' : 'Authorized'}
                  </span>
                </div>
              ))}
              {/* Manual capture — for POS/cash sales where money is
                  collected at the counter but capture is a deliberate
                  dashboard step rather than automatic. Website COD orders
                  capture automatically when marked Fulfilled instead. */}
              {order.payments.some((p: any) => !p.captured_at) && (
                <button
                  onClick={() => handleAction('capture')}
                  disabled={!!actionLoading}
                  className='mt-3 w-full px-3.5 py-2 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-medium rounded-lg transition-colors disabled:opacity-50'
                >
