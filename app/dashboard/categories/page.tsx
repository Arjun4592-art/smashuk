...
function SearchIcon() {
  return <svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='#8C9196' strokeWidth='1.5' strokeLinecap='round' strokeLinejoin='round'>
...
export default function CategoriesPage() {
  ...
  const autoHandle = (name: string) => name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
  const handleSave = async () => {
    if (!form.name || !form.sport) return;
    setSaving(true);
    setSaveError(null);
    const sportParent = categories.find(c => c.parentId === null && c.name === form.sport);
    ...
    try {
      if (editingId) {
        ...
        setCategories(prev => deriveSports(prev.map(c => c.id === editingId ? updated : c)));
  ...
  return <div className='space-y-5'>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-4 gap-4'>
        <StatCard label='Total Categories' value={total} icon={<svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1.5' strokeLinecap='round' strokeLinejoin='round'>
              <path d='M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z' />
            </svg>} colorClasses='bg-[#2C6ECB]/8 text-[#2C6ECB]' />
        <StatCard label='Active' value={categories.filter(c => c.status === 'active').length} icon={<svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1.5' strokeLinecap='round' strokeLinejoin='round'>
              <circle cx='12' cy='12' r='10' />
              <path d='M8 12l3 3 5-6' />
            </svg>} colorClasses='bg-[#008060]/8 text-[#008060]' />
        <StatCard label='Inactive' value={categories.filter(c => c.status === 'inactive').length} icon={<svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1.5' strokeLinecap='round' strokeLinejoin='round'>
              <circle cx='12' cy='12' r='10' />
              <path d='M10 15V9M14 15V9' />
            </svg>} colorClasses='bg-[#6D7175]/8 text-[#6D7175]' />
        <StatCard label='Total Products' value={categories.reduce((s, c) => s + c.productCount, 0)} icon={<svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1.5' strokeLinecap='round' strokeLinejoin='round'>
              <path d='M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z' />
              <polyline points='3.27 6.96 12 12.01 20.73 6.96' />
              <line x1='12' y1='22.08' x2='12' y2='12' />
            </svg>} colorClasses='bg-amber-50 text-amber-600' />
      </div>
      ...
      <div className='bg-white border border-[#E1E3E5] rounded-2xl overflow-hidden shadow-sm'>
        ...
        <div className='flex items-center gap-3 px-4 py-3 border-b border-[#E1E3E5] flex-wrap'>
          <div className='flex items-center gap-2 flex-1 min-w-50 px-3 py-2 border border-[#E1E3E5] rounded-lg bg-[#F8F9FA] focus-within:border-[#008060] focus-within:bg-white focus-within:ring-2 focus-within:ring-[#008060]/10 transition-all duration-150'>
            <SearchIcon />
            <input type='text' placeholder='Search categories...' value={search} onChange={e => setSearch(e.target.value)} className='flex-1 bg-transparent text-[13px] text-[#202223] placeholder-[#B0B5BA] outline-none' />
          ...
          <select value={sportFilter} onChange={e => setSportFilter(e.target.value)} className='px-3 py-2 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] bg-white outline-none cursor-pointer hover:border-[#8C9196] transition-colors shadow-sm'>
        ...
        {loading ? <div className='animate-pulse'>
            {[...Array(5)].map((_, i) => <div key={i} className='flex items-center gap-4 px-5 py-4 border-b border-[#F1F1F1]'>
          ...
          </div> : error ? <div className='flex flex-col items-center justify-center py-20 gap-3'>
            <p className='text-[13px] text-[#D82C0D]'>{error}</p>
            <button onClick={fetchCategories} className='px-4 py-2 bg-[#008060] text-white text-[13px] rounded-lg border-none cursor-pointer hover:bg-[#006e52]'>
              Retry
            </button>
            ...
            <table className='w-full'>
              <thead>
                <tr className='border-b border-[#E1E3E5] bg-[#FAFAFA]'>
                  {['Category', 'Sport', 'Handle', 'Products', 'Status', 'Created', ''].map((h, i) => <th key={i} className={`px-4 py-3 text-[11px] font-semibold text-[#9CA3AF] uppercase tracking-wider ${i === 6 ? 'text-right' : 'text-left'}`}>
                      {h}
                    </th>)}
                </tr>
              </thead>
                ...
                {filtered.length === 0 ? <tr>
                    <td colSpan={7} className='px-4 py-16 text-center'>
                      <div className='flex flex-col items-center gap-3'>
                        ...
                        <button onClick={openAdd} className='mt-1 inline-flex items-center gap-1.5 px-4 py-2 bg-[#008060] text-white text-[13px] font-medium rounded-lg hover:bg-[#006e52] transition-colors border-none cursor-pointer'>
                          <PlusIcon /> Add Category
                        </button>
                      </div>
                    </td>
                  </tr> : filtered.map(cat => <tr key={cat.id} className='hover:bg-[#FAFAFA] transition-colors duration-100 group'>
                      ...
                      <td className='px-4 py-3'>
                        <Toggle checked={cat.status === 'active'} onChange={() => toggleStatus(cat)} title={cat.status === 'active' ? 'Click to deactivate' : 'Click to activate'} />
                      </td>
                      <td className='px-4 py-3'>
                      ...
                      <td className='px-4 py-3'>
                        <div className='flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150'>
                          <button onClick={() => openEdit(cat)} className='w-7 h-7 flex items-center justify-center text-[#8C9196] hover:text-[#202223] hover:bg-[#F0F0F0] rounded-lg transition-all duration-150 bg-transparent border-none cursor-pointer' title='Edit'>
                            <EditIcon />
                          </button>
                          <button onClick={() => setDeleteId(cat.id)} className='w-7 h-7 flex items-center justify-center text-[#8C9196] hover:text-[#D82C0D] hover:bg-[#D82C0D]/5 rounded-lg transition-all duration-150 bg-transparent border-none cursor-pointer' title='Delete'>
                            <TrashIcon />
                          </button>
                        </div>
                      </td>
        ...
        <div className='flex items-center justify-between px-4 py-3 border-t border-[#F1F1F1]'>
          ...
          <div className='flex items-center gap-1'>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className='w-8 h-8 flex items-center justify-center border border-[#E1E3E5] rounded-lg text-[#6D7175] bg-white cursor-pointer hover:bg-[#F6F6F7] disabled:opacity-40 disabled:cursor-not-allowed transition-colors'>
              <ChevronLeftIcon />
            </button>
            {Array.from({
            length: totalPages
          }, (_, i) => i + 1).slice(Math.max(0, page - 2), Math.min(totalPages, page + 1)).map(p => <button key={p} onClick={() => setPage(p)} className={`w-8 h-8 flex items-center justify-center rounded-lg text-[12.5px] font-semibold border transition-colors cursor-pointer ${p === page ? 'bg-[#008060] text-white border-[#008060] shadow-sm shadow-[#008060]/20' : 'border-[#E1E3E5] text-[#6D7175] bg-white hover:bg-[#F6F6F7]'}`}>
                  {p}
                </button>)}
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className='w-8 h-8 flex items-center justify-center border border-[#E1E3E5] rounded-lg text-[#6D7175] bg-white cursor-pointer hover:bg-[#F6F6F7] disabled:opacity-40 disabled:cursor-not-allowed transition-colors'>
              <ChevronRightIcon />
            </button>
          </div>
        </div>
      ...
      {showModal && <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
          ...
          <div className='relative bg-white rounded-2xl shadow-[0_24px_64px_rgba(0,0,0,0.15)] w-full max-w-lg overflow-hidden'>
            <div className='flex items-center justify-between px-6 py-4 border-b border-[#E1E3E5]'>
              ...
              <button onClick={() => setShowModal(false)} className='w-7 h-7 flex items-center justify-center text-[#8C9196] hover:text-[#202223] hover:bg-[#F6F6F7] rounded-lg transition-all bg-transparent border-none cursor-pointer'>
                <CloseIcon />
              </button>
            </div>
            <div className='px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto'>
              ...
              <div>
                ...
                <div className='flex flex-wrap gap-2'>
                  {['📦', '⚽', '🏏', '🏀', '🎾', '🥊', '🏊', '🚴', '🏃', '👟', '🦺', '🧤', '⛑️', '🥅'].map(icon => <button key={icon} onClick={() => setForm(f => ({
                ...f,
                icon
              ...
              setForm(f => ({
                ...f,
                name,
                handle: autoHandle(name)
              }));
            }} placeholder='e.g. Football Boots' className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] placeholder-[#C4C8CC] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/12 transition-all duration-150' />
              </div>
              {}
              <div>
                <label className='block text-[12px] font-semibold text-[#6D7175] uppercase tracking-wider mb-1.5'>
                ...
                <div className='flex items-center border border-[#E1E3E5] rounded-lg overflow-hidden focus-within:border-[#008060] focus-within:ring-2 focus-within:ring-[#008060]/12 transition-all duration-150'>
                  ...
                  <input type='text' value={form.handle} onChange={e => setForm(f => ({
                ...f,
              ...
              <div>
                ...
                <select value={form.sport} onChange={e => setForm(f => ({
              ...f,
              sport: e.target.value
                  ...
                  {SPORTS.filter(s => s !== 'All').map(s => <option key={s} value={s}>
                      {SPORT_ICONS[s]} {s}
                    </option>)}
                </select>
              </div>
            ...
            }))} placeholder='Brief description of this category...' rows={3} className='w-full px-3.5 py-2.5 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] placeholder-[#C4C8CC] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/12 transition-all duration-150 resize-none' />
              </div>
              {}
              <div className='flex items-center justify-between p-3.5 border border-[#E1E3E5] rounded-xl bg-[#FAFAFA]'>
                ...
                <Toggle checked={form.status === 'active'} onChange={() => setForm(f => ({
              ...f,
            ...
            <div className='flex items-center justify-end gap-2.5 px-6 py-4 border-t border-[#E1E3E5] bg-[#FAFAFA]'>
              <button onClick={() => setShowModal(false)} className='px-4 py-2 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[13px] font-medium text-[#202223] rounded-lg transition-colors duration-150 cursor-pointer'>
                Cancel
              </button>
              <button onClick={handleSave} disabled={saving || !form.name || !form.sport} className='inline-flex items-center gap-2 px-5 py-2 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-semibold rounded-lg transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer border-none shadow-sm shadow-[#008060]/20'>
                {saving ? <>
                    <SpinnerIcon /> Saving...
