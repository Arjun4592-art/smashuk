...
// Real Stripe-backed saved cards (previously this only stored a locally
// typed-in label — brand/last4/expiry the person typed themselves, with no
// actual card behind it, in a Zustand store that lived only in the
// browser). Now cards are saved via Stripe SetupIntent + Elements: Stripe
// collects and stores the real card, we only ever see brand/last4/expiry,
// and the card can be reused for a future purchase without re-entering it.
    ...
    >
      ...
      <div className='flex gap-3'>
        ...
        <button
          type='button'
          onClick={onCancel}
          className='text-sm font-semibold text-gray-500 hover:text-gray-700 font-lato'
        >
          Cancel
        </button>
      </div>
  ...
  return (
    <div className='bg-white rounded-2xl border border-gray-100 p-6'>
      <div className='flex items-center justify-between mb-3'>
        ...
        <button
          onClick={() => (showForm ? setShowForm(false) : openAddCard())}
          disabled={loadingIntent}
          className='flex items-center gap-1.5 bg-[#E8553A] hover:bg-[#D4441F] disabled:opacity-50 text-white font-montserrat font-bold px-4 py-2 rounded-full text-xs transition-colors'
        >
      ...
      {loading ? (
        ...
        <p className='text-sm text-gray-400 font-lato py-6 text-center'>
          No saved payment methods yet.
        </p>
      ) : (
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
          {methods.map((m) => (
            <div key={m.id} className='border rounded-xl p-4 relative border-gray-100'>
              ...
              <div className='flex gap-3 mt-3'>
                <button
                  ...
                  className='text-xs font-semibold text-red-500 hover:underline font-lato'
                >
                  Remove
                </button>
              </div>
