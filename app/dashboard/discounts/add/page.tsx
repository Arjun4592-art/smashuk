...
// ─── Build Medusa Promotion Payload (Medusa 2.15.5) ───────────────
function buildPromotionPayload(form: any, autoRules: AutoRule[]) {
  // Quantity Discount ("Buy X, get Y% off") always auto-applies once the
  // cart hits the quantity threshold — a customer typing a code defeats
  // the point of a quantity deal, which should just kick in at checkout.
  const isAutomatic = autoRules.length > 0 || form.type === 'buy_x_get_y'

  // BUG FIX: Free Shipping used to be built as a `target_type: 'order'`,
  // `value: 0` promotion — i.e. "take £0 off the order", which does
  // nothing. A promotion only actually waives shipping cost in Medusa v2
  // when it targets the shipping method itself: `target_type:
  // 'shipping_methods'` with a 100% discount on it. This is separate from
  // the automatic £-threshold free shipping on the storefront (see
  // FREE_SHIPPING_THRESHOLD in lib/constants.ts, applied in cart/checkout)
  // — this is the promo-code path, e.g. a "FREESHIP" code a customer types
  // in that should waive shipping on that order regardless of subtotal.
  const applicationMethod: any = {
    type: form.type === 'fixed' ? 'fixed' : 'percentage', // percentage | buy_x_get_y | free_shipping all use percentage-off
    ...
    value:
      form.type === 'fixed'
        ...
        : form.type === 'free_shipping'
          ? 100
          : Number(form.value || 0), // percentage | buy_x_get_y — plain % number
    currency_code: 'gbp',
  }
  ...
  if (form.minOrderAmount)
    rules.push({
      attribute: 'subtotal',
      operator: 'gte',
      values: [{ value: String(Number(form.minOrderAmount)) }],
    })
  if (form.minQuantity)
    rules.push({
      attribute: 'quantity',
      operator: 'gte',
      values: [{ value: form.minQuantity }],
    })

  // Quantity Discount's own threshold — "buy at least N items" — same
  // rule shape as the generic "Minimum item quantity" auto rule below,
  // just always attached for this discount type instead of opt-in.
  if (form.type === 'buy_x_get_y' && form.buyQuantity)
    rules.push({
      attribute: 'quantity',
      operator: 'gte',
      values: [{ value: form.buyQuantity }],
    })
  ...
  autoRules.forEach((rule) => {
    if (rule.type === 'min_amount' && rule.value)
      rules.push({
        attribute: 'subtotal',
        operator: 'gte',
        values: [{ value: String(Number(rule.value)) }],
      })
  ...
  if (form.customerEligibility === 'new_customers')
    rules.push({
      attribute: 'customer_order_count',
      operator: 'eq',
      values: [{ value: '0' }],
  ...
  // Campaign — description, dates, usage limit all go into the Campaign
  const hasCampaign =
    form.description ||
    form.startsAt ||
  ...
  const campaign = hasCampaign
    ? {
        ...
        ...(form.hasEndDate && form.expiresAt
          ...
          : {}),
        ...(form.maxUses
          ? { budget: { type: 'usage', limit: Number(form.maxUses) } }
          : {}),
      }
    : undefined
...
function AddDiscountPageContent() {
  ...
  // had dead modal state that never rendered, and this page only handled
  // creation. Now /dashboard/discounts/add?id=<id> loads the promotion from
  // Medusa and prefills every field below.
  useEffect(() => {
    if (!editId) return
    let cancelled = false
    ;(async () => {
      try {
        ...
        // Quantity Discount is just "percentage + automatic + a quantity
        // rule attached directly (not via the opt-in Auto Rules list)" —
        // recognize that shape so editing one doesn't silently turn it
        // into a generic percentage discount and drop the Qty threshold.
        ...
        setForm((f) => ({
          ...
          minOrderAmount: subtotalRule
            ...
            : '',
          // For a Quantity Discount the quantity rule IS buyQuantity above,
          // not the separate opt-in "min item quantity" field.
          minQuantity:
                ...
                { label: 'Max Uses', value: form.maxUses || 'Unlimited' },
                {
                  ...
                  value:
                    form.customerEligibility === 'all'
                      ? 'All'
                      : form.customerEligibility === 'new_customers'
                        ? 'New only'
                        : 'Specific',
                },
                {
