...
interface CartState {
  ...
  addItem: (product: Product, quantity?: number, variant?: CartItem['variant'], metadata?: Record<string, any>, discountPerUnit?: number) => void;
...
function computeTotals(items: CartItem[], discountAmount: number, giftCardTotal: number = 0, taxRate: number = DEFAULT_TAX_RATE) {
  const subtotal = items.reduce((s, i) => s + (i.product.price - (i.discount ?? 0)) * i.quantity, 0);
  const physicalItems = items.filter(i => i.product.slug !== GIFT_CARD_PRODUCT_HANDLE);
  const physicalSubtotal = physicalItems.reduce((s, i) => s + (i.product.price - (i.discount ?? 0)) * i.quantity, 0);
  const shipping = physicalItems.length === 0 || physicalSubtotal >= FREE_SHIPPING_THRESHOLD ? 0 : SHIPPING_COST;
...
export const useCartStore = create<CartState>()(persist((set, get) => ({
  ...
  setTaxRate: rate => set(state => ({
    taxRate: rate,
    ...computeTotals(state.items, state.discountAmount, state.giftCardTotal, rate)
  })),
  addItem: (product, quantity = 1, variant, metadata, discountPerUnit) => {
    set(state => {
      const existingIndex = state.items.findIndex(i => i.product.id === product.id && i.variant?.id === variant?.id);
      ...
      return {
        items: newItems,
        ...computeTotals(newItems, state.discountAmount, state.giftCardTotal, state.taxRate)
      };
    });
    trackAddToCart({
  ...
  removeItem: (productId, variantId) => {
    const state0 = get();
    const removedItem = state0.items.find(i => i.product.id === productId && i.variant?.id === variantId);
    const effectiveVariantId = variantId ?? (removedItem?.product as any)?.variants?.[0]?.id;
    set(state => {
      const newItems = state.items.filter(i => !(i.product.id === productId && i.variant?.id === variantId));
      return {
        items: newItems,
        ...computeTotals(newItems, state.discountAmount, state.giftCardTotal, state.taxRate)
      };
    ...
    if (cartId && effectiveVariantId) {
      ...
      (async () => {
        try {
          ...
          const lineItem = (medusaCart?.items ?? []).find((li: any) => li.variant_id === effectiveVariantId);
          if (lineItem) {
            await removeFromCart(cartId, lineItem.id);
          }
        } catch (err) {
          console.error('Failed to sync item removal with backend cart:', err);
        }
      })();
    }
  },
  updateQuantity: (productId, quantity, variantId) => {
    ...
    const state0 = get();
    const targetItem = state0.items.find(i => i.product.id === productId && i.variant?.id === variantId);
    const effectiveVariantId = variantId ?? (targetItem?.product as any)?.variants?.[0]?.id;
    set(state => {
      const newItems = state.items.map(i => i.product.id === productId && i.variant?.id === variantId ? {
        ...i,
        quantity
      } : i);
      return {
    ...
    if (cartId && effectiveVariantId) {
      ...
      (async () => {
        try {
          ...
          console.error('Failed to sync quantity change with backend cart:', err);
        }
      })();
    }
  },
  applyCoupon: async code => {
    ...
    try {
      ...
      set(state => ({
        ...
        discountAmount,
        ...computeTotals(state.items, discountAmount, state.giftCardTotal, state.taxRate)
      }));
      return {
        success: true
      };
  ...
  applyGiftCard: async code => {
    ...
    try {
      ...
      const giftCards = creditLines.filter(cl => !cl.reference || cl.reference === 'gift-card').map(cl => ({
      ...
      set(state => ({
        giftCards,
        giftCardTotal,
        ...computeTotals(state.items, state.discountAmount, giftCardTotal, state.taxRate)
      }));
      return {
        success: true
      };
  ...
  onRehydrateStorage: () => state => {
    if (!state) return;
    const totals = computeTotals(state.items, state.discountAmount, state.giftCardTotal, state.taxRate);
