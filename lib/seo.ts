...
export async function generateCategoryMetadata(handle: string): Promise<Metadata> {
  try {
    ...
    const category = categories.find((c: any) => c.handle === handle || c.name.toLowerCase() === handle.toLowerCase());
    ...
    return buildMetadata({
      ...
      fallbackDescription: category.description ?? `Shop premium ${category.name} equipment at ${SITE_NAME}.`,
...
export async function generateStaticMetadata(page: 'home' | 'shop' | string): Promise<Metadata> {
  try {
    ...
    const fallbacks: Record<string, {
      ...
      home: {
        ...
        description: 'Shop premium badminton, tennis, padel and squash equipment with fast UK-wide delivery.',
        canonical: SITE_URL
      },
      shop: {
      ...
      'local-store': {
        ...
        description: 'Visit our Manchester racket specialist store for badminton, tennis, squash and padel — racket restringing, expert advice from club-level players, and a demo service to try before you buy.',
