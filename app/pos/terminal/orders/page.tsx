...
export default function OrdersPage() {
  ...
  return (
    <>
      ...
      {/* Order detail modal — rendered OUTSIDE #pos-orders-scroll-area on
          purpose. That container has an inline `transform` (entrance
          animation), and a `transform` on an ancestor turns it into the
          containing block for any descendant `position: fixed` element.
          That silently broke these modals: instead of being fixed to the
          viewport, they were fixed relative to the scrollable list, so they
          scrolled/shifted with it and got clipped at its edges. */}
      {selectedOrder && (
        <OrderDetailModal
