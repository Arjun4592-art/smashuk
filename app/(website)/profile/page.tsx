...
function ProfilePageContent() {
  const { user, logout, authChecked, isLoggingOut } = useAuthStore()
  ...
  // Supports deep-linking straight into a tab, e.g. /profile?tab=orders —
  // used after checkout and by old /orders links instead of a separate route.
  ...
  const handleLogout = async () => {
    clearCart()
    logout('website')
    // Go straight to login — previously this pushed to '/' while the
    // effect below *also* fired a competing router.replace('/login...')
    // the instant `user` went null, and the two navigations racing each
    // other left the page stuck showing the skeleton instead of landing
    // anywhere. isLoggingOut (checked below) stops that effect from firing
    // during this manual navigation.
    router.replace('/login')
  }

  useEffect(() => {
    // Server-side session check has finished and there's genuinely no user
    // (e.g. an expired/invalid token) — redirect instead of showing a
    // skeleton forever. Skip while a manual logout navigation is already
    // in flight (see handleLogout) so the two don't race.
              ...
              <div className='bg-white rounded-2xl border border-gray-100 p-2'>
                {TABS.map((tab) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-lato font-semibold transition-colors flex items-center justify-between ${
                      activeTab === tab.key
                        ? 'bg-[#E8553A]/8 text-[#E8553A]'
                        : 'text-[#0A1F44] hover:bg-gray-50'
                    }`}
                  >
                    {tab.label}
                    {tab.key === 'wishlist' && wishlistItems.length > 0 && (
                      <span className='text-[10px] font-black bg-[#0A1F44] text-white rounded-full px-1.5 py-0.5'>
...
function OrderRow({
  ...
  return (
    <div className='p-4 border border-gray-100 rounded-xl hover:border-[#E8553A]/20 transition-colors'>
      ...
      <div className='flex items-center justify-between mt-3 pt-3 border-t border-gray-100'>
        ...
        <div className='flex items-center gap-3'>
          <button
            onClick={onTrack}
            className='text-xs font-semibold text-[#0A1F44] hover:text-[#E8553A] font-lato flex items-center gap-1 transition-colors'
          >
            🚚 Track
          </button>
          {order.payment_status === 'captured' &&
            order.status !== 'canceled' && (
              <button
                onClick={onReturn}
                className='text-xs font-semibold text-[#0A1F44] hover:text-[#E8553A] font-lato transition-colors'
              >
                ↩️ Request Return
              </button>
            )}
