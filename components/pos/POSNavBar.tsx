...
export default function POSNavBar({
  ...
  const soundOnScan = usePOSStore((s) => s.soundOnScan)
  const autoPrintReceipt = usePOSStore((s) => s.autoPrintReceipt)
  const showStockCount = usePOSStore((s) => s.showStockCount)
  const taxInclusivePricing = usePOSStore((s) => s.taxInclusivePricing)
  const updateTerminalSetting = usePOSStore((s) => s.updateTerminalSetting)
  const settings: Record<TerminalSettingKey, boolean> = {
    soundOnScan,
    autoPrintReceipt,
    showStockCount,
    taxInclusivePricing,
  ...
  const toggleSetting = (key: TerminalSettingKey) =>
          ...
          <div ref={settingsRef} className='relative'>
            ...
            {showSettings && (
              ...
              >
                ...
                {(
                  Object.entries(settings) as [TerminalSettingKey, boolean][]
                ).map(([key, val], i, arr) => (
                  <div
                    key={key}
                    className='flex items-center justify-between px-4 py-3'
