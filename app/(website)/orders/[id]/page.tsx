...
// app/(website)/orders/[id]/page.tsx
...
export default function OrderDetailPage({
  ...
  // Track which items have been reviewed in this session
  ...
  if (error || !order) {
    return (
      <div className='min-h-screen bg-[#F2F4F7] flex flex-col items-center justify-center gap-4 px-4'>
    ...
    <>
        ...
        <div className='bg-[#0A1F44] py-10'>
          <div className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8'>
            ...
            <div className='flex items-center gap-3 flex-wrap'>
              ...
              <span
                className={`px-3 py-1.5 rounded-full text-xs font-bold font-montserrat capitalize ${STATUS_COLORS[displayStatus] ?? 'bg-gray-100 text-gray-600'}`}
              >
                {displayStatus}
              </span>
              {isPickup && (
                <span className='px-3 py-1.5 rounded-full text-xs font-bold font-montserrat bg-white/10 text-white'>
                  🏬 Store Pickup
                </span>
              )}
            </div>
                  ...
                  return (
                    <div key={item.id} className='px-6 py-4'>
                      ...
                      {item.metadata?.service_type === 'stringing' && (
                        <div className='mt-2 ml-[68px] inline-flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-[#B54708] bg-[#FFFAEB] border border-[#FEDF89] rounded-md px-2.5 py-1.5 font-lato'>
                      ...
                      {/* Review CTA — only for delivered orders with a known productId */}
          ...
          <div className='space-y-4'>
            {isPickup ? (
              ...
              <div className='bg-white rounded-2xl border border-gray-100 p-5'>
                ...
                {/* Only show Track button for non-delivered orders */}
                {!isDelivered && (
                  <button
                    onClick={() => setShowTracking(true)}
                    className='mt-3 text-sm font-semibold text-[#E8553A] hover:underline font-lato'
                  >
                    Track this order →
                  </button>
                )}
                {isDelivered && (
                  <p className='mt-3 text-xs text-green-600 font-lato font-semibold flex items-center gap-1'>
      ...
      {reviewItem && (
        <WriteReviewModal
          ...
          onSubmitted={() => {
            // Mark this order-item as reviewed (using orderId+productId combo)
            setReviewedItems((prev) => {
              const next = new Set(prev)
              // Find the item id from order items matching this productId
              const matchedItem = (order.items ?? []).find(
                (i: any) =>
                  i.variant?.product_id === reviewItem.productId ||
                  i.product_id === reviewItem.productId,
              )
