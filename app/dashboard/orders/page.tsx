...
function OrdersPageContent() {
  ...
  const handleBulkStatusChange = async (action: 'deliver' | 'cancel') => {
    ...
    if (failCount === 0) {
      toast.success(
        `Marked ${successCount} order${successCount !== 1 ? 's' : ''} as ${verb}`,
      )
    } else {
      toast.error(
        `${successCount} updated, ${failCount} failed — check those orders individually`,
      )
    }
  }
  ...
  const totalRevenue = orders.reduce(
    (s: number, o: any) =>
      ...
      o.paymentStatus === 'partially_captured'
        ? o.amount
        : 0),
    0,
  )
  ...
  return (
    <div className='space-y-5'>
      ...
      <div className='bg-white border border-[#E1E3E5] rounded-xl overflow-hidden'>
                  ...
                  {loading ? (
                    ...
                    paginated.map((order: any) => (
                      ...
                      >
                        ...
                        <td className='px-4 py-3'>
                          <span
                            className='inline-flex items-center gap-1.5 text-[12.5px] text-[#202223]'
                            title={order.source}
                          >
                            <span>{SOURCE_ICONS[order.source] ?? '🌐'}</span>
                            <span>
                              {SOURCE_LABELS[order.source] ?? 'Website'}
                            </span>
                          </span>
                        </td>
                        <td className='px-4 py-3'>
        ...
        {view === 'abandoned' && (
          <>
            {!abandonedAvailable && !abandonedLoading && (
              <div className='px-5 py-16 text-center'>
                <p className='text-[14px] font-medium text-[#202223]'>
                  Abandoned Checkouts
                </p>
                <p className='text-[13px] text-[#6D7175] mt-1 max-w-md mx-auto'>
                  This needs a small custom endpoint on the Medusa backend
                  (carts aren't exposed as "abandoned checkouts" out of the
                  box). See{' '}
                  <code className='bg-[#F6F6F7] px-1 py-0.5 rounded'>
                    BACKEND_SNIPPET.md
                  </code>{' '}
                  for the route to add — once it's deployed this tab will fill
                  in automatically.
                </p>
              </div>
            )}
            ...
            {abandonedAvailable &&
              ...
              abandonedCheckouts.length === 0 && (
                <div className='px-5 py-16 text-center'>
                  ...
                  <p className='text-[13px] text-[#6D7175] mt-1'>
                    Nothing to recover right now — carts with items that have
                    sat inactive for a while will show up here.
                  </p>
                </div>
              )}

            {abandonedAvailable &&
              (abandonedLoading || abandonedCheckouts.length > 0) && (
                <div className='overflow-x-auto'>
                  <table className='w-full text-[13px]'>
                    <thead>
                      <tr className='border-b border-[#E1E3E5] text-left text-[#6D7175]'>
                    ...
                    <tbody>
                      {abandonedLoading
                        ? Array.from({
                            ...
                            <tr
                              key={i}
                              className='border-b border-[#F1F2F3] animate-pulse'
                            >
                              <td className='px-5 py-4'>
                                <div className='h-3 w-32 bg-[#E1E3E5] rounded' />
                              </td>
                              <td className='px-5 py-4'>
                                <div className='h-3 w-10 bg-[#E1E3E5] rounded' />
                              </td>
                              <td className='px-5 py-4'>
                                <div className='h-3 w-16 bg-[#E1E3E5] rounded' />
                              </td>
                              <td className='px-5 py-4'>
                                <div className='h-3 w-24 bg-[#E1E3E5] rounded' />
                              </td>
                              <td className='px-5 py-4' />
                            </tr>
                          ))
