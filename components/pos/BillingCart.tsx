...
interface Props {
  ...
  total: number
  subtotal: number
  // Gift card redeemed against this sale (a tender against the
  // already-taxed total, not a subtotal discount — see posStore.ts).
  giftCardCode?: string | null
  giftCardAmount?: number
  // total minus giftCardAmount — what's actually left for cash/card/split
  // to collect. Falls back to `total` when omitted.
  amountDue?: number
  onIncrease: (id: string) => void
  onDecrease: (id: string) => void
  onRemove: (id: string) => void
  // BUG FIX: this used to be called with the raw 0-100 number typed into
  // the "Discount %" box, but the caller (setCustomDiscount) stored that
  // number directly as a flat currency amount — so typing "20" gave a flat
  // -£20 off while the UI still labelled it "20%". This callback now always
  // receives a real percentage (0-100); the caller must convert it to an
  // amount itself (see applyPercentageDiscount in store/posStore.ts, the
  // same conversion the "Add discount" modal's percent tab already uses).
...
export default function BillingCart({
  ...
  // Raw text the cashier is typing into the "Discount %" box. Kept local
  // (not derived from `discount`, which is a currency AMOUNT that may also
  // include a coupon) so the box always reflects the % actually typed
  // instead of trying to reverse-engineer a percentage from an amount.
        ...
        <div className='space-y-1.5'>
          ...
          {discountAmount > 0 && (
            <div
              ...
              style={{ color: '#008060' }}
            >
              {/* BUG FIX: this used to show "Discount ({discount}%)", but
                  `discount`/`discountAmount` are currency amounts that may
                  combine a percentage discount AND a coupon — reconstructing
                  a single "%" label from that total was misleading (and was
                  actively wrong while the quick box bug above existed).
                  Just label it plainly; the £ amount alongside it is the
                  real, unambiguous number. */}
              <span>Discount</span>
