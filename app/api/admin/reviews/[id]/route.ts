...
//
// DELETE removes a review from the dashboard; PATCH toggles `published`
// (so a review can be hidden from the website slider without deleting it).
// Both read-modify-write `store.metadata.reviews`, same as the parent
// route — see app/api/admin/reviews/route.ts for the storage rationale.
...
async function saveReviews(
  ...
  const res = await fetch(`${MEDUSA_URL}/admin/stores/${storeId}`, {
    ...
    body: JSON.stringify({ metadata: { ...metadata, reviews } }),
  })
  const data = await safeJson(res)
  ...
  try {
    ...
    const updated = existing.map((r) => {
      ...
      return {
        ...r,
        published:
          typeof body.published === 'boolean' ? body.published : r.published,
      }
    })
