...
import { BADMINTON_TENSIONS as TENSIONS, BADMINTON_STRING_BRANDS as STRING_BRANDS } from '@/lib/stringing-options';
...
export const metadata = {
  ...
  description: 'Professional badminton racket stringing from £16. Tension advice for beginner to advanced players, popular string types in stock, 24-hour or 40-minute express turnaround.',
  keywords: 'badminton stringing service, badminton racket restring, badminton string tension guide, badminton restring manchester, badminton string types'
};
const Icon = ({
  glyph,
  ...p
...
const IconDroplet = (p: React.SVGProps<SVGSVGElement>) => <Icon {...p} glyph={<path d='M12 2.69s6 7.03 6 11.31a6 6 0 0 1-12 0c0-4.28 6-11.31 6-11.31z' />} />;
          ...
          <div className='grid md:grid-cols-2 gap-12 items-end'>
            <div>
              ...
              <div className='flex flex-wrap gap-3'>
                <Link href='/local-store/stringing#book' className='bg-[#E8553A] hover:bg-[#D4441F] text-white px-7 py-3.5 rounded-full text-sm font-montserrat font-bold transition-colors'>
                  Book Your Slot →
                </Link>
                <a href='#tension' className='border border-white/20 hover:border-white/40 text-white/70 hover:text-white px-7 py-3.5 rounded-full text-sm font-montserrat transition-colors'>
                  Find My Tension
                </a>
              </div>
            </div>
      ...
      <div className='max-w-4xl mx-auto px-4 md:px-6 py-16 space-y-20'>
        ...
        <section>
          <div className='bg-white border border-[#0A1F44]/8 rounded-3xl p-8 md:p-10'>
            ...
            <div className='grid sm:grid-cols-2 gap-4'>
              {RESTRING_SIGNS.map(s => <div key={s.title} className='flex gap-3 bg-[#F5F3EF] rounded-xl border border-[#0A1F44]/8 px-5 py-4'>
        ...
        <section id='tension'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-5'>
            {TENSIONS.map((t, i) => <div key={t.level} className={`rounded-2xl p-6 border ${i === 1 ? 'bg-[#0A1F44] border-[#0A1F44]' : 'bg-white border-[#0A1F44]/8'}`}>
                <p className={`text-xs font-montserrat font-bold uppercase tracking-wider mb-2 ${i === 1 ? 'text-[#E8553A]' : 'text-[#E8553A]'}`}>
                  {t.level}
                </p>
                <p className={`font-montserrat font-black text-2xl mb-3 ${i === 1 ? 'text-white' : 'text-[#0A1F44]'}`}>
                  {t.range}
                </p>
                <p className={`text-sm leading-relaxed ${i === 1 ? 'text-white/70' : 'text-gray-400'}`}>
                  {t.note}
                </p>
              </div>)}
          </div>
        ...
        <section>
          ...
          <div className='space-y-4'>
            {STRING_TYPES.map(s => <div key={s.name} className='bg-white border border-[#0A1F44]/8 rounded-2xl p-6 grid md:grid-cols-[1fr_1.3fr] gap-6'>
                <div>
        ...
        <section>
          ...
          <Accordion defaultOpenId={STRING_BRANDS[0]?.brand} containerClassName='bg-white rounded-2xl border border-[#0A1F44]/8 divide-y divide-[#0A1F44]/8' triggerClassName='font-montserrat font-bold text-base text-[#0A1F44] px-6 py-5' icon='chevron' iconWrapClassName='w-7 h-7 text-xs' contentClassName='px-6 pb-5' items={STRING_BRANDS.map(b => ({
          id: b.brand,
          title: b.brand,
          content: <ol className='grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2'>
                  {(b.items ?? []).map((item, idx) => <li key={item} className='text-sm text-gray-500 flex items-center gap-2'>
        ...
        <section>
          ...
          <div className='grid sm:grid-cols-3 gap-5'>
            {CARE_TIPS.map(c => {
            const IconEl = c.icon;
            return <div key={c.title} className='bg-white border border-[#0A1F44]/8 rounded-2xl p-6 text-center'>
        ...
        <section>
          ...
          <Link href='/blog/how-to-choose-a-badminton-racket' className='group flex flex-col sm:flex-row max-w-xl mx-auto bg-white rounded-2xl border border-[#0A1F44]/8 overflow-hidden hover:border-[#E8553A]/30 transition-all'>
            <div className='sm:w-48 shrink-0 aspect-[4/3] sm:aspect-auto bg-gray-100'>
              <img src='https://images.unsplash.com/photo-1626224583764-f87db24ac4ea?w=800&q=80' alt='Badminton guide' className='w-full h-full object-cover' />
            </div>
