...
export default function ProductCard({
  ...
  // BUG FIX: the SALE ribbon only ever rendered when a product had an
  // explicit `metadata.badge = 'SALE'` set in the dashboard — which almost
  // no product has, since it's a separate optional field from the price
  // fields. So even after the price-display bug was fixed (strikethrough +
  // "Save £X" now shows correctly whenever a real discount exists), the
  // small SALE tag on the card itself stayed invisible. Falling back to
  // "genuinely discounted" here keeps it in sync with the same logic
  // /shop?badge=SALE and the sidebar's "On Sale" filter already use
  // (see isOnSale() in lib/api/store.ts) — an explicit badge still wins.
      ...
      <Link href={`/shop/${product.slug}`} className='group block'>
        <div className='flex gap-4 bg-white border border-[#E5E7EB] rounded-2xl p-4 hover:border-[#E8553A]/30 hover:shadow-[0_4px_20px_rgba(232,85,58,0.08)] transition-all duration-300'>
          ...
          <div className='relative w-28 h-28 sm:w-36 sm:h-36 rounded-xl overflow-hidden bg-[#F2F4F7] pt-3 shrink-0'>
            ...
            {displayBadge && (
              <span
                className={`absolute top-2 left-2 text-[9px] font-black px-2 py-0.5 rounded-full font-montserrat tracking-wide ${BADGE_STYLES[displayBadge]}`}
              >
                {displayBadge}
              </span>
            )}
          ...
          <div className='flex-1 min-w-0'>
            <div className='flex items-start justify-between gap-2'>
              ...
              <button
                ...
                className='p-1.5 shrink-0 rounded-lg hover:bg-[#E8553A]/8 transition-colors'
                aria-label='Wishlist'
              >
                <HeartIcon
                  size={18}
                  filled={wishlisted}
                  className={wishlisted ? 'text-[#E8553A]' : 'text-[#D1D5DB]'}
                />
              </button>
            </div>
            ...
            <div className='flex items-center justify-between gap-3 flex-wrap'>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-black font-montserrat transition-all duration-200 ${
                  !product.inStock
                    ? 'bg-[#F2F4F7] text-[#9CA3AF] cursor-not-allowed'
                    : added
                      ? 'bg-[#10B981] text-white'
                      : 'bg-[#0A1F44] hover:bg-[#E8553A] text-white shadow-sm'
                }`}
              >
