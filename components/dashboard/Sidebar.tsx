  ...
  return (
    <div ref={containerRef} className='relative'>
      <div
        className={
          variant === 'plain'
            ? 'flex items-center gap-2 px-2.5 py-1.5 bg-[#F6F6F7] border border-[#E1E3E5] rounded-lg'
            : 'flex items-center gap-2 px-2.5 py-1.5 bg-[#F6F6F7] border border-[#E1E3E5] rounded-lg hover:border-[#8C9196] transition-colors cursor-text'
        }
      >
        {Icons.search}
        <input
          ...
          placeholder='Search...'
          className='flex-1 bg-transparent text-[12.5px] text-[#202223] placeholder-[#8C9196] outline-none min-w-0'
        />
        {variant === 'default' && !query && (
          <span className='text-[10px] text-[#8C9196] bg-white border border-[#E1E3E5] px-1 py-0.5 rounded font-mono shrink-0'>
            ⌘K
          </span>
        )}
      </div>
      ...
      {open && query && (
        <div className='absolute left-0 right-0 mt-1.5 bg-white border border-[#E1E3E5] rounded-lg shadow-lg z-50 overflow-hidden max-h-72 overflow-y-auto'>
          {results.length === 0 ? (
            <div className='px-3 py-3 text-[12.5px] text-[#8C9196]'>
            ...
            results.map((item, i) => (
              <button
                ...
                className={`w-full flex items-center justify-between gap-2 px-3 py-2 text-left text-[12.5px] bg-transparent border-none cursor-pointer ${i === activeIndex ? 'bg-[#F6F6F7] text-[#202223]' : 'text-[#202223]'}`}
              >
                <span className='truncate'>{item.label}</span>
                {item.group && (
                  <span className='text-[10.5px] text-[#8C9196] shrink-0'>
                    {item.group}
                  </span>
                )}
              </button>
