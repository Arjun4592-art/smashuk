...
async function getProductWithMeta(productId: string) {
  const res = await medusaServiceFetch(`/admin/products/${productId}?fields=id,metadata`);
...
export async function GET(_req: NextRequest, {
  ...
  try {
    ...
    const published = reviews.filter(r => r.published).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
...
export async function POST(req: NextRequest, {
  ...
  const orderRes = await fetch(`${MEDUSA_URL}/store/orders/${orderId}?fields=*items,*items.variant,status,fulfillment_status,customer_id`, {
    headers: {
  ...
  const {
    order
  } = await safeJson(orderRes, 'app/api/store/products/[id]/reviews/route.ts');
  const deliveredStatuses = ['delivered', 'partially_delivered'];
  if (order?.status !== 'completed' && !deliveredStatuses.includes(order?.fulfillment_status)) {
    return NextResponse.json({
  ...
  const hasProduct = (order?.items ?? []).some((item: any) => item.variant?.product_id === productId || item.product_id === productId);
  if (!hasProduct) {
    return NextResponse.json({
  ...
  const existingReviews: ProductReview[] = product.metadata?.productReviews ?? [];
  const alreadyReviewed = existingReviews.some(r => r.orderId === orderId && r.customerId === order.customer_id);
  if (alreadyReviewed) {
    return NextResponse.json({
  ...
  const customerName = `${order.shipping_address?.first_name ?? ''} ${order.shipping_address?.last_name ?? ''}`.trim() || 'Verified Buyer';
  ...
  const avgRating = publishedReviews.length > 0 ? Math.round(publishedReviews.reduce((sum, r) => sum + r.rating, 0) / publishedReviews.length * 10) / 10 : 0;
