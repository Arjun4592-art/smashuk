  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (!result.valid || result.balance == null) {
        setError(result.reason === 'already_redeemed' ? 'This gift card has already been fully redeemed' : result.reason === 'expired' ? 'This gift card has expired' : 'Invalid gift card code');
        return;
      }
      if (result.balance <= 0) {
    ...
    background: 'rgba(0,0,0,0.4)'
      ...
      boxShadow: '0 8px 32px rgba(0,0,0,0.12)'
        ...
        <div className='p-5 space-y-4'>
          {}
          {giftCardCode && <div className='flex items-center justify-between p-3 rounded-lg' style={{
          background: '#F2F7F5',
          border: '1px solid #008060'
              ...
              <button onClick={handleRemove} className='text-xs px-2 py-1 rounded hover:bg-[#FFF4F4] transition-colors' style={{
            color: '#D82C0D'
          }}>
                Remove
              </button>
          ...
          <div>
            ...
            <input autoFocus value={code} onChange={e => {
            setCode(e.target.value);
            setError('');
          }} onKeyDown={e => {
            if (e.key === 'Enter') handleApply();
          }} placeholder='e.g. GC-XXXXXXXX' className='w-full px-3 py-2.5 rounded-lg border text-sm outline-none uppercase' style={{
            borderColor: error ? '#D82C0D' : '#E1E3E5',
            color: '#202223'
          }} />
            {error && <p className='text-xs mt-1' style={{
            color: '#D82C0D'
          }}>
                {error}
              </p>}
        ...
        <div className='px-5 pb-5 flex gap-2'>
          ...
          <button onClick={handleApply} disabled={checking} className='py-2.5 px-6 rounded-lg text-sm font-semibold transition-colors' style={{
          background: '#008060',
          color: '#FFFFFF',
          flex: 2,
