...
function CustomTooltip({
  ...
  return <div className='bg-white border border-[#E1E3E5] rounded-xl px-3.5 py-2.5 shadow-lg shadow-black/5'>
      <p className='text-[11px] text-[#8C9196] mb-1.5'>{label}</p>
      {payload.map((p: any) => <p key={p.dataKey} className='text-[13px] font-semibold text-[#202223]'>
...
function StatCard({
  ...
  if (loading) {
    return <div className='bg-white border border-[#E1E3E5] rounded-2xl p-5 animate-pulse'>
  ...
  return <div className='group bg-white border border-[#E1E3E5] rounded-2xl p-5 hover:shadow-[0_4px_24px_rgba(0,0,0,0.07)] hover:-translate-y-0.5 transition-all duration-200 ease-out'>
      <div className='flex items-start justify-between mb-4'>
        <div className={`w-10 h-10 ${colors.bg} rounded-xl flex items-center justify-center ${colors.text} transition-transform duration-200 group-hover:scale-105`}>
          {icon}
        </div>
        <span className={`inline-flex items-center gap-1 text-[11.5px] font-medium px-2.5 py-1 rounded-full ${positive ? 'bg-[#008060]/8 text-[#008060]' : 'bg-[#D82C0D]/8 text-[#D82C0D]'}`}>
      ...
      {subtitle && <p className='text-[11.5px] text-[#B0B5BA] mt-1'>{subtitle}</p>}
    </div>;
}
function SectionHeader({
  ...
  return <div className='flex items-center justify-between w-full'>
      <div>
        ...
        {subtitle && <p className='text-[12px] text-[#8C9196] mt-0.5'>{subtitle}</p>}
      </div>
      {link && <Link href={link} className='inline-flex items-center gap-1.5 text-[12.5px] text-[#008060] hover:text-[#006e52] no-underline font-medium transition-colors group shrink-0'>
          {linkLabel}
  ...
  return <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='flex items-center gap-2.5'>
          <select value={dateRange} onChange={e => setDateRange(e.target.value)} className='px-3 py-2 bg-white border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] outline-none cursor-pointer hover:border-[#8C9196] transition-colors shadow-sm'>
      ...
      {}
      {statsError && <div className='px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-[13px] text-red-600 flex items-center justify-between'>
          <span>Failed to load stats: {statsError}</span>
          <button onClick={refetchStats} className='ml-3 underline cursor-pointer bg-transparent border-none text-red-600 text-[13px]'>
            Retry
          </button>
        </div>}
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4'>
        <StatCard title='Total Revenue' value={stats ? formatCurrency(stats.totalRevenue) : '—'} change={stats?.revenueChange ?? 0} icon={<RevenueIcon />} subtitle={comparisonLabel} loading={statsLoading} colorIndex={0} />
        <StatCard title='Total Orders' value={stats ? stats.totalOrders.toLocaleString() : '—'} change={stats?.ordersChange ?? 0} icon={<OrdersIcon />} subtitle={comparisonLabel} loading={statsLoading} colorIndex={1} />
        <StatCard title='Total Customers' value={stats ? stats.totalCustomers.toLocaleString() : '—'} change={stats?.customersChange ?? 0} icon={<CustomersIcon />} subtitle={comparisonLabel} loading={statsLoading} colorIndex={2} />
        <StatCard title='Total Products' value={stats ? stats.totalProducts.toLocaleString() : '—'} change={stats?.productsChange ?? 0} icon={<ProductsIcon />} subtitle='active products' loading={statsLoading} colorIndex={3} />
      </div>
      ...
      <div className='grid grid-cols-1 xl:grid-cols-3 gap-4'>
        ...
        <div className='xl:col-span-2 bg-white border border-[#E1E3E5] rounded-2xl p-5 shadow-sm'>
          <SectionHeader title='Revenue Overview' subtitle='Daily revenue for the selected period' />
          {salesData.length === 0 && !statsLoading ? <div className='h-[220px] flex flex-col items-center justify-center gap-2 text-center'>
            ...
            bottom: 0
              ...
              <Area type='monotone' dataKey='revenue' stroke='#008060' strokeWidth={2} fill='url(#revenueGrad)' dot={false} activeDot={{
              r: 4,
              fill: '#008060',
              strokeWidth: 0
            }} />
        ...
        <div className='bg-white border border-[#E1E3E5] rounded-2xl p-5 shadow-sm flex flex-col'>
          ...
          <div className='mt-3 max-h-[160px] overflow-y-auto pr-1 [scrollbar-width:thin]'>
            <ResponsiveContainer width='100%' height={Math.max(160, categoryBreakdown.length * 28)}>
              <BarChart data={categoryBreakdown} layout='vertical' margin={{
              top: 0,
              right: 8,
              left: 0,
              bottom: 0
                ...
                <YAxis type='category' dataKey='sport' tick={{
                fontSize: 12,
                fill: '#6D7175'
                ...
                <Bar dataKey='orders' fill='#008060' radius={[0, 5, 5, 0]} barSize={12} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className='mt-4 space-y-2.5 pt-4 border-t border-[#F1F1F1]'>
            {categoryBreakdown.map(s => <div key={s.sport} className='flex items-center justify-between group'>
                <div className='flex items-center gap-2'>
                  <span className='w-2 h-2 rounded-full shrink-0 transition-transform duration-150 group-hover:scale-125' style={{
                background: s.color
              }} />
      ...
      <div className='grid grid-cols-1 xl:grid-cols-3 gap-4'>
        ...
        <div className='xl:col-span-2 bg-white border border-[#E1E3E5] rounded-2xl overflow-hidden shadow-sm'>
          <div className='px-5 py-4 border-b border-[#F1F1F1]'>
            <SectionHeader title='Recent Orders' link='/dashboard/orders' linkLabel='View all' />
          </div>
          <div className='divide-y divide-[#F6F6F7]'>
            {ordersLoading ? [...Array(5)].map((_, i) => <div key={i} className='flex items-center gap-4 px-5 py-3.5 animate-pulse'>
                    ...
                    <div className='w-16 h-5 bg-[#F1F1F1] rounded-full' />
                  </div>) : recentOrders.map(order => <div key={order.id} className='flex items-center gap-4 px-5 py-3.5 hover:bg-[#FAFAFA] transition-colors duration-150 cursor-default'>
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-[11px] font-medium capitalize shrink-0 ${STATUS_STYLES[order.status] ?? 'bg-gray-100 text-gray-600'}`}>
                      {order.status}
                    </span>
                  </div>)}
          </div>
        ...
        <div className='space-y-4'>
          ...
          <div className='bg-white border border-[#E1E3E5] rounded-2xl p-5 shadow-sm'>
            ...
            }].map(action => <Link key={action.label} href={action.href} className={`flex flex-col items-center gap-2 p-3.5 border border-[#E1E3E5] rounded-xl ${action.bg} transition-all duration-150 no-underline text-center group`}>
                  <span className={`${action.color} transition-transform duration-150 group-hover:scale-110`}>
                    {action.icon}
                  </span>
          ...
          {}
          {topProducts.length > 0 && <div className='bg-white border border-[#E1E3E5] rounded-2xl overflow-hidden shadow-sm'>
              <div className='px-5 py-4 border-b border-[#F1F1F1]'>
                <SectionHeader title='Top Products' link='/dashboard/products' linkLabel='View all' />
              </div>
              <div className='divide-y divide-[#F6F6F7]'>
              ...
              stock: number;
              price: number;
            }, i: number) => <div key={product.id} className='flex items-center gap-3 px-5 py-3 hover:bg-[#FAFAFA] transition-colors duration-150'>
