...
// Same price-extraction logic as normalizeProduct() in lib/api/store.ts,
// applied per-variant instead of just variants[0] — normalizeProduct
// collapses a multi-variant product down to a single price, which is right
// for a normal product card but wrong here: each denomination variant has
// its own price and the customer needs to pick one.
function getVariantAmount(v: ProductVariant): number {
  const calcAmount = v.calculated_price?.calculated_amount
  const gbpPrices = (v.prices ?? [])
    .filter((p) => p.currency_code === 'gbp')
    .map((p) => p.amount)
...
export default function GiftCardPurchaseClient({ product, variants }: Props) {
  const addItem = useCartStore((s) => s.addItem)

  const sortedVariants = [...variants].sort(
    (a, b) => getVariantAmount(a) - getVariantAmount(b),
  )
  ...
  // ── Send as Gift ──────────────────────────────────────────────────────
  // Captured as a cart-item metadata object (same mechanism the stringing
  // booking flow already uses for per-item extras — see store/cartStore.ts)
  // so it's attached to the real Medusa line item and visible on the order
  // in the dashboard, not just held in local UI state and lost after
  // checkout.
  ...
  const canAdd =
    !!selectedVariant && !adding && (!isGift || isValidEmail(recipientEmail))
  ...
  const handleAddToCart = () => {
    ...
    addItem(
      ...
      quantity,
      { id: selectedVariant.id, size: selectedVariant.title },
      isGift
        ? {
            isGift: true,
    ...
    toast.success(
      isGift
        ? `£${selectedAmount} gift card queued for ${recipientEmail.trim()}!`
        : `£${selectedAmount} gift card added to cart!`,
    )
      ...
      <div className='grid md:grid-cols-2 gap-12'>
        ...
        <div>
          <div className='sticky top-28'>
              ...
              <div className='relative h-full flex flex-col justify-between p-6 sm:p-8 text-white'>
                ...
                <div>
                  ...
                  {isGift && recipientName.trim() && (
                    <p className='mt-2 font-lato text-sm text-white/80'>
            ...
            {isGift && message.trim() && (
              <div className='mt-4 p-4 rounded-xl bg-[#F2F4F7] border border-[#E5E7EB]'>
            ...
            <div className='mt-6 grid grid-cols-2 gap-3'>
              {[
                { icon: <SparkleIcon size={16} />, label: 'Instant delivery' },
                {
                ...
                { icon: <MailIcon size={16} />, label: 'Emailed at checkout' },
              ].map((f) => (
                <div
                  key={f.label}
                  className='flex items-center gap-2 text-xs font-lato text-gray-500'
                >
        ...
        <div>
            ...
            {sortedVariants.map((v) => {
              ...
              return (
                <button
                  ...
                  className={`py-3.5 rounded-xl border-2 font-montserrat font-bold text-sm transition-all duration-150 ${
                    active
                      ? 'border-[#E8553A] bg-[#E8553A]/6 text-[#E8553A] shadow-sm scale-[1.03]'
                      : 'border-gray-200 text-[#0A1F44] hover:border-[#0A1F44]/30 hover:scale-[1.02]'
                  }`}
                >
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl border transition-all duration-200 mb-3 ${
              isGift
          ...
          >
            <span className='flex items-center gap-2.5'>
              <GiftIcon
                size={18}
                className={isGift ? 'text-[#E8553A]' : 'text-gray-400'}
              />
              <span
                className={`text-[14px] font-lato font-semibold ${isGift ? 'text-[#0A1F44]' : 'text-[#4B5563]'}`}
              >
                I want to send this as a gift
              </span>
            </span>
            <div
              className={`w-10 h-5 rounded-full relative shrink-0 transition-colors duration-300 ${isGift ? 'bg-[#E8553A]' : 'bg-[#E5E7EB]'}`}
            >
              <div
                className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-all duration-300 ${isGift ? 'left-5' : 'left-0.5'}`}
              />
            </div>
          </button>
              ...
              <div className='space-y-4 p-4 rounded-xl bg-[#F8F9FB] border border-[#E5E7EB]'>
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3.5 py-2.5 rounded-lg border text-sm font-lato focus:outline-none transition-colors ${
                      emailError
                        ? 'border-red-400 focus:border-red-500'
                        : 'border-gray-200 focus:border-[#E8553A]'
                    }`}
                  />
                  {emailError && (
                    <p className='mt-1 text-xs text-red-500 font-lato'>
                ...
                <div>
                  ...
                  <input
                    ...
                    className='w-full px-3.5 py-2.5 rounded-lg border border-gray-200 text-sm font-lato focus:outline-none focus:border-[#E8553A] transition-colors'
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder='A short note for them goes here.'
                    className='w-full px-3.5 py-2.5 rounded-lg border border-gray-200 text-sm font-lato focus:outline-none focus:border-[#E8553A] transition-colors resize-none'
                  />
                </div>
              </div>
            </div>
          ...
          <div className='flex items-center border border-gray-200 rounded-xl overflow-hidden w-fit mb-8'>
            <button
              ...
              className='w-10 h-10 flex items-center justify-center text-[#0A1F44] hover:bg-gray-50 transition-colors'
            >
              −
            </button>
          ...
          <button
            ...
            disabled={!canAdd}
            className='w-full bg-[#0A1F44] hover:bg-[#E8553A] text-white font-montserrat font-bold py-3.5 rounded-xl transition-colors disabled:opacity-60'
          >
            {adding
              ? 'Adding…'
              : `Add to Cart — ${formatCurrency(selectedAmount * quantity)}`}
          </button>
