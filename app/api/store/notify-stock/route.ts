// app/api/store/notify-stock/route.ts
//
// "Notify me when back in stock" — smashuk.co (and most ecommerce sites)
// let customers leave their email against an out-of-stock product and get
// pinged when it's available again. This didn't exist anywhere in the app.
//
// Follows the same pattern as app/api/store/newsletter/route.ts: requests
// are persisted in Medusa's own store.metadata (genuine Postgres
// persistence via the admin API) rather than a local JSON/file write,
// which would either leak publicly from /public or fail outright on a
// read-only-filesystem host like Vercel.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const updated: StockRequest[] = [
      ...result.requests,
      { email: normalized, productId, productName, createdAt: new Date().toISOString() },
    ]

    const saveRes = await medusaServiceFetch(`/admin/stores/${result.storeId}`, {
      method: 'POST',
      body: JSON.stringify({ metadata: { stockNotifyRequests: updated } }),
    })
    if (!saveRes.ok) {
    ...
    // Confirmation to the customer + heads-up to the store — both
    // non-fatal, the signup is saved either way.
    sendMail({
      to: normalized,
