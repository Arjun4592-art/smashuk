...
export interface SplitPayment {
  ...
  // Exact pence amount the PaymentIntent above was created for — lets the
  // server confirm with Stripe itself that this much was actually charged,
...
interface Props {
  // This is amountDue from the store — the total AFTER any gift card
  // applied via the cart's gift-card icon has already been deducted (see
  // <PaymentModal total={amountDue} .../> in billing/page.tsx). There is
  // deliberately no separate "Gift Card" option in here: that's the one
  // place a gift card gets redeemed, so a partial-balance card just means
  // this modal shows a smaller total and the cashier picks Cash/Stripe/
  // Split for whatever's left — no need to juggle it twice.
  total: number
  // Returns a Promise now — the modal awaits it and keeps its "Processing…"
  // state up until the caller (handleConfirmPayment in billing/page.tsx)
  // has actually finished creating the Medusa order, verifying payment, and
  // marking fulfillment. Before this, onConfirm fired-and-forgot: for a cash
  // sale specifically there was no card-collection step to show a spinner
  // for, so the modal just sat there with no feedback for however long the
  // cart→line-items→payment→complete→capture→fulfill chain took server-side,
  // looking frozen to the cashier.
...
const SPLIT_METHODS: { id: 'cash' | 'card'; label: string }[] = [
  { id: 'cash', label: 'Cash' },
  { id: 'card', label: 'Stripe' },
]

export default function PaymentModal({ total, onConfirm, onClose }: Props) {
  const [selected, setSelected] = useState<PayMethod>('cash')

  // ── Stripe Terminal (card_present) state ────────────────────────────────
  const [processing, setProcessing] = useState(false)
  const [statusMsg, setStatusMsg] = useState('')
  const [terminalError, setTerminalError] = useState('')
  // Default true so this works without a physical reader. Flip off once a
  // real reader is set up in the Stripe Dashboard (Terminal → Locations).
  const [useSimulatedReader, setUseSimulatedReader] = useState(true)

  // ── Split state ──────────────────────────────────────────────────────────
  // Which methods are active in the split (min 2, max 3)
  const [activeSplits, setActiveSplits] = useState<('cash' | 'card')[]>([
    'cash',
    'card',
  ])
  // Raw input strings per method (so user can type freely)
  ...
  // ── Toggle a method in/out of the split ─────────────────────────────────
  ...
  // ── Rebalance amounts equally across active methods ─────────────────────
  ...
  // ── Confirm ──────────────────────────────────────────────────────────────
  // `processing` now stays true for the FULL round trip, not just the Stripe
  // card-collection step: collect card (if any) → then keep spinning through
  // onConfirm, which is where the actual Medusa order gets created, the
  // payment gets verified/captured, and (for a cash pickup) fulfillment gets
  // marked — see handleConfirmPayment in app/pos/terminal/billing/page.tsx.
  // That chain is several sequential network calls and can genuinely take a
  // few seconds, so the cashier needs to see it's still working the whole
  // time, not just during card collection.
  const handleConfirm = async () => {
    ...
    if (selected === 'split') {
      ...
      await onConfirm({ method: 'split', splits })
      setProcessing(false)
      return
    }
    ...
    // Cash — no card to collect, but the sale still has to be created in
    // Medusa (and, for a cash pickup, marked paid + fulfilled) before this
    // is really "done". Show the same spinner so it doesn't look frozen.
    setProcessing(true)
    setStatusMsg('Recording sale…')
    await onConfirm({ method: selected })
    setProcessing(false)
  }
    ...
    >
      ...
      >
        ...
        <div className='overflow-y-auto flex-1'>
          ...
          {/* ── Stripe Terminal (card) settings/error — card contexts only ── */}
          {(selected === 'card' ||
          ...
          {/* ── Processing spinner — ALL methods (cash included). Covers not
              just Stripe card collection but also the time the order takes
              to actually be created/verified/fulfilled in Medusa after that,
              so the modal never just sits there looking frozen. ──────────── */}
          {processing && (
            <div className='px-4 pb-2'>
              <div
                className='flex items-center gap-2 px-3 py-2 rounded-lg text-xs'
                style={{ background: '#F2F7F5', color: '#008060' }}
              >
