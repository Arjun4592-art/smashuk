  ...
  return (
    <PaymentElement
      ...
      onLoadError={(e) =>
        onLoadError?.(
          e.error?.message ??
            'Could not load the card form. Please disable ad blockers or try a different browser.',
        )
      }
          ...
          <div className='lg:col-span-2 space-y-5'>
            ...
            <div className='grid grid-cols-2 gap-0 border border-gray-200 rounded-2xl overflow-hidden bg-white'>
              <button
                ...
                className={`flex items-center justify-center gap-2 py-3.5 font-montserrat font-bold text-sm transition-colors ${deliveryMode === 'ship' ? 'bg-[#0A1F44] text-white' : 'text-gray-400 hover:bg-gray-50'}`}
              >
                🚚 Ship
              </button>
              <button
                ...
                className={`flex items-center justify-center gap-2 py-3.5 font-montserrat font-bold text-sm transition-colors border-l border-gray-200 ${deliveryMode === 'pickup' ? 'bg-[#0A1F44] text-white' : 'text-gray-400 hover:bg-gray-50 disabled:opacity-40 disabled:hover:bg-transparent'}`}
              >
                🏬 Pickup
              </button>
            </div>
            ...
            <div className='bg-white rounded-2xl p-5 border border-gray-100 sticky top-28'>
              ...
              <div className='space-y-2.5 text-sm font-lato border-t border-gray-100 pt-4'>
                ...
                <div className='flex justify-between'>
                  <span className='text-gray-500 flex items-center gap-1.5'>
                    ...
                    <button
                      ...
                      className='w-4 h-4 rounded-full border border-gray-300 text-gray-400 text-[10px] leading-none flex items-center justify-center hover:border-gray-400 hover:text-gray-600 transition-colors'
                    >
                      ?
                    </button>
                  </span>
                ...
                <p className='text-xs text-gray-400 font-lato text-right -mt-1'>
                  Including {formatCurrency(displayTax)} in taxes
                </p>
              </div>
                    ...
                    <div className={paymentElementReady ? '' : 'hidden'}>
                      ...
                      >
                        <StripeCardBlock
                          ...
                          onLoadError={(msg) => setCardLoadError(msg)}
        ...
        >
          ...
          >
            ...
            <div className='px-6 py-5 space-y-4 text-sm font-lato text-gray-600'>
              {DELIVERY_INFO_SECTIONS.map((section, i) => (
                <div key={i}>
                  {section.heading && (
                    <p className='font-semibold text-[#0A1F44] mb-1.5'>
                      {section.heading}
                    </p>
                  )}
                  {section.body.split('\n\n').map((para, j) => (
