...
function LoginForm() {
  ...
  const redirect = rawRedirect && rawRedirect.startsWith('/') && !rawRedirect.startsWith('//') ? rawRedirect : '/profile';
  const {
    data: session
  } = useSession();
      ...
      <div className='w-full max-w-md'>
        ...
        <div className='bg-white rounded-2xl shadow-sm border border-gray-100 p-8'>
          ...
          <form onSubmit={handleSubmit} className='space-y-5'>
            ...
            }))} placeholder='you@example.com' className='w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#E8553A] transition-colors font-lato' required />
            </div>
            ...
            <div>
              ...
              <div className='relative'>
                <input type={showPassword ? 'text' : 'password'} value={form.password} onChange={e => setForm(f => ({
                ...f,
                password: e.target.value
              }))} placeholder='••••••••' className='w-full border border-gray-200 rounded-xl px-4 py-3 pr-11 text-sm outline-none focus:border-[#E8553A] transition-colors font-lato' required autoComplete='current-password' />
                <button type='button' onClick={() => setShowPassword(v => !v)} className='absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors' aria-label={showPassword ? 'Hide password' : 'Show password'} tabIndex={-1}>
                  {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
              </div>
              <div className='text-right mt-1.5'>
                <Link href='/forgot-password' className='text-xs text-[#E8553A] hover:underline font-lato'>
                  Forgot password?
                </Link>
              </div>
            </div>

            {error && <div className='bg-red-50 border border-red-200 rounded-xl p-3'>
                <p className='text-xs text-red-600 font-lato'>{error}</p>
              </div>}

            <button type='submit' disabled={loading} className={`w-full py-3.5 rounded-xl font-montserrat font-black text-white transition-all ${loading ? 'bg-gray-300 cursor-not-allowed' : 'bg-[#E8553A] hover:bg-[#D4441F] shadow-lg hover:-translate-y-0.5'}`}>
          ...
          <div className='mt-6 text-center'>
            <p className='text-sm text-gray-500 font-lato'>
              ...
              <Link href='/register' className='text-[#E8553A] font-semibold hover:underline'>
                Create one free
              </Link>
            </p>
          </div>
