...
type CustomerEligibility = 'all' | 'specific'
interface AutoRule {
  id: string
  type:
    | 'min_amount'
...
function SectionCard({
  ...
  return (
    <div
      className={`bg-white border border-[#E1E3E5] rounded-2xl p-6 shadow-sm ${className}`}
    >
      {children}
    </div>
  )
...
function Toggle({
  ...
  return (
    <button
      onClick={onChange}
      className={`relative w-10 h-6 rounded-full transition-colors duration-200 border-none cursor-pointer ${checked ? 'bg-[#008060]' : 'bg-[#D1D5DB]'}`}
    >
      <span
        className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-transform duration-200 ${checked ? 'translate-x-4' : 'translate-x-0.5'}`}
      />
    </button>
  )
}
function Input({
  ...
  return (
    <div className='relative'>
      {prefix && (
        <span className='absolute left-3.5 top-1/2 -translate-y-1/2 text-[#8C9196] text-[13px] font-medium select-none'>
          {prefix}
        </span>
      )}
      <input
        {...props}
        className={`w-full ${prefix ? 'pl-8' : 'px-3.5'} pr-3.5 py-2.5 border border-[#E1E3E5] rounded-xl text-[13px] text-[#202223] placeholder-[#C4C8CC] outline-none focus:border-[#008060] focus:ring-2 focus:ring-[#008060]/10 transition-all duration-150 ${props.className ?? ''}`}
      />
    </div>
  )
}
function buildPromotionPayload(
  ...
  existingCampaignId: string | null = null,
) {
  ...
  if (form.minOrderAmount)
    rules.push({
      attribute: 'subtotal',
      operator: 'gte',
      values: [String(Number(form.minOrderAmount))],
    })
  if (form.minQuantity)
    rules.push({
      attribute: 'quantity',
      operator: 'gte',
      values: [String(form.minQuantity)],
    })
  if (form.type === 'buy_x_get_y' && form.buyQuantity)
    rules.push({
      attribute: 'quantity',
      operator: 'gte',
      values: [String(form.buyQuantity)],
    })
  autoRules.forEach((rule) => {
    if (rule.type === 'min_amount' && rule.value)
      rules.push({
        attribute: 'subtotal',
        operator: 'gte',
        values: [String(Number(rule.value))],
      })
    if (rule.type === 'min_quantity' && rule.value)
      rules.push({
        attribute: 'quantity',
        operator: 'gte',
    ...
    if (rule.type === 'specific_product' && rule.selectedProducts?.length)
      rules.push({
        ...
        operator: 'in',
        values: rule.selectedProducts.map((p) => p.id),
      })
  ...
  if (form.customerEligibility === 'specific' && selectedGroupIds.length > 0)
    rules.push({
      attribute: 'customer.groups.id',
      operator: 'in',
      values: selectedGroupIds,
    })
  const dedupedRules = rules.filter((rule, index) => {
...
function AddDiscountPageContent() {
  ...
  const loadGroups = () => {
    ...
    fetch('/api/admin/customer-groups?limit=100')
      .then((r) => r.json())
      .then((d) =>
        setAvailableGroups(
          (d.customer_groups ?? []).map((g: any) => ({
            id: g.id,
            name: g.name,
            customerCount: g.customers?.length ?? 0,
          })),
        ),
      )
      .catch(() => {})
  ...
  const [segmentGroupBusyId, setSegmentGroupBusyId] = useState<string | null>(
    null,
  )
  const loadSegments = () => {
    ...
    fetch('/api/admin/customers/segments')
      .then((r) => r.json())
      .then((d) => setAvailableSegments(d.segments ?? []))
      .catch(() => {})
  ...
  const handleUseSegment = async (segment: {
    id: string
    label: string
    customerCount: number
    customers: { id: string; name: string; email: string }[]
    ...
    setSegmentGroupBusyId(segment.id)
    try {
      const res = await fetch('/api/admin/customer-groups', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: segment.label }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error('Failed to create group')
      const group = data.customer_group ?? data
      await fetch(`/api/admin/customer-groups/${group.id}/customers`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ add: segment.customers.map((c) => c.id) }),
      })
      const newGroup = {
        id: group.id,
        name: group.name,
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/admin/customers?q=${encodeURIComponent(customerSearch)}&limit=10`,
        )
        const data = await res.json()
        setCustomerSearchResults(
    ...
    ;(async () => {
      try {
        ...
        const subtotalRule = (p.rules ?? []).find(
          (r: any) => r.attribute === 'subtotal',
        )
        const quantityRule = (p.rules ?? []).find(
          (r: any) => r.attribute === 'quantity',
        )
        const customerGroupRule = (p.rules ?? []).find(
          (r: any) => r.attribute === 'customer.groups.id',
        )
        const isQuantityDiscount =
          am.type === 'percentage' && !!p.is_automatic && !!quantityRule
        const isFreeShipping = am.target_type === 'shipping_methods'
        const type: DiscountType = isFreeShipping
          ? 'free_shipping'
          : isQuantityDiscount
            ? 'buy_x_get_y'
            : am.type === 'fixed' && am.value > 0
              ? 'fixed'
              : 'percentage'
        setForm((f) => ({
          ...f,
          ...
          value:
            type === 'percentage' || type === 'buy_x_get_y'
              ? String(am.value ?? '')
              : String(am.value ?? 0),
          buyQuantity: isQuantityDiscount
            ? String(quantityRule.values?.[0]?.value ?? '')
            : f.buyQuantity,
          minOrderAmount: subtotalRule
            ? String(Number(subtotalRule.values?.[0]?.value ?? 0))
            : '',
          minQuantity:
            quantityRule && !isQuantityDiscount
              ? String(quantityRule.values?.[0]?.value ?? '')
              : '',
          maxUses: p.campaign?.budget?.limit
            ? String(p.campaign.budget.limit)
            : '',
          startsAt: p.campaign?.starts_at
            ? new Date(p.campaign.starts_at).toISOString().slice(0, 10)
            : '',
          expiresAt: p.campaign?.ends_at
            ? new Date(p.campaign.ends_at).toISOString().slice(0, 10)
            : '',
          ...
          try {
            const results = await Promise.all(
              ids.map(async (id: string) => {
                const r = await fetch(`/api/admin/customer-groups/${id}`)
                if (!r.ok)
                  return {
                    id,
                    name: id,
            ...
            if (!cancelled) setSelectedGroups(results)
          } catch {}
        }
      } catch (err: any) {
  ...
  const togglePendingGroupMember = (customer: {
    id: string
    email: string
    name?: string
  }) => {
    setPendingGroupMembers((prev) => {
      const already = prev.find((c) => c.id === customer.id)
      return already
        ? prev.filter((c) => c.id !== customer.id)
        : [...prev, customer]
    })
  }
  const toggleSelectedGroup = (group: { id: string; name: string }) => {
    setSelectedGroups((prev) => {
      const already = prev.find((g) => g.id === group.id)
      return already ? prev.filter((g) => g.id !== group.id) : [...prev, group]
    })
  }
    ...
    try {
      ...
      if (pendingGroupMembers.length > 0) {
        await fetch(`/api/admin/customer-groups/${group.id}/customers`, {
          ...
          body: JSON.stringify({
            add: pendingGroupMembers.map((c) => c.id),
          }),
        })
      }
      const newGroup = {
      ...
      setAvailableGroups((prev) => [
        ...
        {
          ...newGroup,
          customerCount: pendingGroupMembers.length,
        },
      ])
      ...
      setSaveError('Failed to create customer group')
    } finally {
  ...
  const previewValue = () => {
    ...
    return form.buyQuantity && form.value
      ? `Buy ${form.buyQuantity}+, get ${form.value}% off`
      : '—'
  }
  ...
  const checklist = [
    ...
    },
    {
      label: 'Customer eligibility',
        ...
        form.customerEligibility !== 'specific' || selectedGroups.length > 0,
    },
    ...(autoEnabled
      ? [
          {
  ...
  return (
    <div className='max-w-5xl mx-auto space-y-5'>
        ...
        <div className='flex items-center gap-2.5'>
          {!isEditing && (
            <button
              ...
              className='px-4 py-2 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[13px] font-medium text-[#202223] rounded-lg transition-colors disabled:opacity-50 cursor-pointer shadow-sm'
            >
              Save as Inactive
            </button>
          )}
          <button
            onClick={() => handleSave(form.isActive)}
            disabled={saving || !form.code || loadingExisting}
            className='inline-flex items-center gap-2 px-4 py-2 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-semibold rounded-lg transition-all disabled:opacity-50 cursor-pointer border-none shadow-sm shadow-[#008060]/20'
          >
      ...
      <div className='grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-5'>
              ...
              {DISCOUNT_TYPES.map((t) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 p-4 border-2 rounded-xl text-left transition-all duration-150 cursor-pointer group ${isActive ? `${t.activeBorder} ${t.activeBg}` : 'border-[#E1E3E5] bg-white hover:border-[#C4C8CC] hover:bg-[#FAFAFA]'}`}
                  >
                    <div
                      className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-transform duration-150 group-hover:scale-105 ${isActive ? `${t.iconActiveBg} ${t.color}` : 'bg-[#F6F6F7] text-[#8C9196]'}`}
                    >
                      {t.icon}
                    </div>
                    <div className='flex-1 min-w-0'>
                      <p
                        className={`text-[13px] font-semibold ${isActive ? t.color : 'text-[#202223]'}`}
                      >
                        {t.label}
                      </p>
                    ...
                    {isActive && (
                      <div
                        className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${t.iconActiveBg} ${t.color}`}
                      >
                        {Icons.check}
                      </div>
                    )}
              ...
              <div>
                ...
                <div className='flex gap-2'>
                  ...
                  <button
                    ...
                    className='inline-flex items-center gap-1.5 px-3.5 py-2.5 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[12.5px] font-medium text-[#202223] rounded-xl transition-colors cursor-pointer whitespace-nowrap shadow-sm'
                  >
                ...
                <p className='text-[11.5px] text-[#B0B5BA] mt-1.5 flex items-center gap-1'>
                  {Icons.info}{' '}
                  {form.type === 'buy_x_get_y'
                    ? 'Applies automatically — this code is just an internal reference.'
                ...
                <div>
                  ...
                  <div className='max-w-[200px]'>
                    <Input
                      ...
                      value={form.value}
                      onChange={(e) =>
                        update('value', (e.target as HTMLInputElement).value)
                      }
          ...
          <SectionCard>
            ...
            {autoEnabled && (
              <div className='space-y-4'>
                <div className='grid grid-cols-2 gap-2'>
                  {AUTO_RULE_OPTIONS.map((opt) => {
                    ...
                    return (
                      <button
                        key={opt.type}
                        onClick={() => addAutoRule(opt.type)}
                        disabled={added}
                        className={`flex items-start gap-2.5 p-3 border rounded-xl text-left transition-all duration-150 cursor-pointer ${added ? 'border-[#008060]/30 bg-[#F2F7F5] opacity-60 cursor-not-allowed' : 'border-[#E1E3E5] bg-white hover:border-[#008060]/40 hover:bg-[#F2F7F5]'}`}
                      >
                        <div
                          className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${added ? 'bg-[#008060] text-white' : 'bg-[#F6F6F7] text-[#8C9196]'}`}
                        >
                ...
                {autoRules.length > 0 && (
                  <div className='space-y-3 pt-2 border-t border-[#F1F1F1]'>
                    <p className='text-[11.5px] font-semibold text-[#6D7175] uppercase tracking-wider'>
                      Active Rules
                    </p>
                    {autoRules.map((rule) => {
                      const opt = AUTO_RULE_OPTIONS.find(
                        (o) => o.type === rule.type,
                      )!
                      return (
                        <div
                          key={rule.id}
                          className='flex items-center gap-3 p-3.5 bg-[#F8F9FA] border border-[#E1E3E5] rounded-xl'
                        >
                          ...
                          <button
                            onClick={() => removeAutoRule(rule.id)}
                            className='w-7 h-7 flex items-center justify-center text-[#8C9196] hover:text-[#D82C0D] hover:bg-[#D82C0D]/5 rounded-lg transition-all bg-transparent border-none cursor-pointer shrink-0'
                          >
                            {Icons.trash}
                          </button>
                        </div>
          ...
          <SectionCard>
            ...
            <div className='space-y-2'>
              {[
                ...
                return (
                  <label
                    key={opt.value}
                    className={`flex items-center gap-3 cursor-pointer p-3.5 border rounded-xl transition-all duration-150 ${isChecked ? 'border-[#008060]/30 bg-[#F2F7F5]' : 'border-[#E1E3E5] hover:bg-[#FAFAFA]'}`}
                  >
                    <input
                      type='radio'
                      name='eligibility'
            ...
            {form.customerEligibility === 'specific' && (
              <div className='mt-3 pt-3 border-t border-[#F1F1F1]'>
                <p className='text-[11.5px] text-[#8C9196] mb-2'>
                  Discount applies only to customers in the selected group(s)
                </p>

                <p className='text-[11px] font-semibold text-[#6D7175] uppercase tracking-wider mb-1.5'>
                  Quick add from segment
                </p>
                {segmentsLoading && (
                  <p className='text-[12px] text-[#8C9196] py-1 mb-2'>
                    Loading segments...
                  </p>
                )}
                {!segmentsLoading && (
                  <div className='flex flex-wrap gap-1.5 mb-3'>
                    {availableSegments.map((seg) => {
                      ...
                      const isSelected = !!(
                        matchingGroup &&
                        selectedGroups.find((s) => s.id === matchingGroup.id)
                      )
                      const isBusy = segmentGroupBusyId === seg.id
                      const disabled = seg.customerCount === 0 || isBusy
                      return (
                        <button
                          key={seg.id}
                          type='button'
                          ...
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 border rounded-full text-[12px] font-medium transition-colors cursor-pointer disabled:cursor-not-allowed disabled:opacity-40 ${isSelected ? 'border-[#008060]/30 bg-[#F2F7F5] text-[#008060]' : 'border-[#E1E3E5] bg-white text-[#202223] hover:bg-[#FAFAFA]'}`}
                        >
                          {isBusy ? Icons.spinner : null}
                          {seg.label}
                          <span
                            className={`text-[11px] ${isSelected ? 'text-[#008060]/70' : 'text-[#8C9196]'}`}
                          >
                            {seg.customerCount}
                          </span>
                        </button>
                  ...
                  <div className='space-y-1.5 mb-2'>
                    {availableGroups.map((g) => {
                      ...
                      return (
                        <label
                          key={g.id}
                          className={`flex items-center gap-2.5 p-2.5 border rounded-lg cursor-pointer transition-all ${isSelected ? 'border-[#008060]/30 bg-[#F2F7F5]' : 'border-[#E1E3E5] hover:bg-[#FAFAFA]'}`}
                        >
                          <input
                            type='checkbox'
                            checked={isSelected}
                            ...
                            className='accent-[#008060] w-4 h-4 shrink-0'
                          />
                          <span className='flex-1 text-[12.5px] text-[#202223] font-medium'>
                            {g.name}
                          </span>
                          <span className='text-[11px] text-[#8C9196]'>
                            {g.customerCount} customer
                            {g.customerCount !== 1 ? 's' : ''}
                          </span>
                        </label>
                      )
                ...
                {!groupsLoading && availableGroups.length === 0 && (
                  <p className='text-[12px] text-[#8C9196] text-center py-3'>
                    No customer groups yet
                  </p>
                )}
                <button
                  onClick={() => setNewGroupModalOpen(true)}
                  className='w-full flex items-center justify-center gap-1.5 px-3 py-2 border border-dashed border-[#C9CDD2] rounded-lg text-[12.5px] font-medium text-[#008060] bg-white hover:bg-[#F2F7F5] hover:border-[#008060]/40 transition-colors cursor-pointer'
                >
            ...
            <div className='space-y-4'>
              <div className='grid grid-cols-2 gap-4'>
                <div>
                  ...
                  <Input
                    ...
                    value={form.maxUses}
                    onChange={(e) =>
                      update('maxUses', (e.target as HTMLInputElement).value)
                    }
              ...
              <div className='flex items-center justify-between p-3.5 border border-[#E1E3E5] rounded-xl bg-[#FAFAFA]'>
                ...
                <Toggle
                  ...
                  onChange={() =>
                    update('combineWithOther', !form.combineWithOther)
                  }
                />
              </div>
            </div>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <div>
                  ...
                  <Input
                    ...
                    value={form.startsAt}
                    onChange={(e) =>
                      update('startsAt', (e.target as HTMLInputElement).value)
                    }
                  />
                </div>
                <div>
        ...
        {}
        <div className='space-y-4'>
          {autoEnabled && autoRules.length > 0 && (
            <div className='flex items-center gap-2.5 px-4 py-3 bg-[#008060] text-white rounded-2xl shadow-sm shadow-[#008060]/20'>
              <span>{Icons.zap}</span>
              <div>
            ...
            <div className='space-y-3 divide-y divide-[#F5F5F5]'>
              {[
                ...
                },
                {
                  label: 'Eligibility',
                  value:
                    form.customerEligibility === 'all'
                      ? 'All'
                      : `Specific (${selectedGroups.length} group${selectedGroups.length !== 1 ? 's' : ''})`,
                },
                {
                  label: 'Status',
                ...
                >
                  ...
                  <span
                    className={`text-[12.5px] font-medium text-[#202223] text-right max-w-[140px] truncate ${item.mono ? 'font-mono tracking-wider text-[11.5px]' : ''}`}
                  >
                    {item.value}
                  </span>
                </div>
            ...
            <div className='space-y-3'>
              {checklist.map((item) => (
                <div key={item.label} className='flex items-center gap-2.5'>
                  <div
                    className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 transition-colors duration-200 ${item.done ? 'bg-[#008060]' : 'bg-[#E1E3E5]'}`}
                  >
                    {item.done && (
                  ...
                  <span
                    className={`text-[12.5px] transition-colors duration-150 ${item.done ? 'text-[#202223]' : 'text-[#B0B5BA]'}`}
                  >
                    {item.label}
                  </span>
                </div>
          ...
          <div className='space-y-2'>
            <button
              onClick={() => handleSave(isEditing ? form.isActive : true)}
              disabled={saving || !form.code || loadingExisting}
              className='w-full py-2.5 bg-[#008060] hover:bg-[#006e52] text-white text-[13px] font-semibold rounded-xl transition-all disabled:opacity-50 cursor-pointer border-none inline-flex items-center justify-center gap-2 shadow-sm shadow-[#008060]/20'
            >
              {saving ? Icons.spinner : null}
              {saving
                ? 'Saving...'
            ...
            {!isEditing && (
              <button
                onClick={() => handleSave(false)}
                disabled={saving || !form.code}
                className='w-full py-2.5 border border-[#E1E3E5] bg-white hover:bg-[#F6F6F7] text-[13px] font-medium text-[#202223] rounded-xl transition-colors disabled:opacity-50 cursor-pointer shadow-sm'
              >
                Save as Inactive
              </button>
            )}
            <Link
              href='/dashboard/discounts'
              className='block text-center py-2.5 text-[13px] text-[#8C9196] hover:text-[#202223] no-underline transition-colors'
            >
              Discard
            </Link>
          </div>
          ...
          >
            ...
            <div className='flex items-center justify-between px-5 py-4 border-b border-[#E1E3E5]'>
              <div>
                ...
                <p className='text-[11.5px] text-[#8C9196] mt-0.5'>
                  Search customers to add to this group
                </p>
              </div>
              <button
            ...
            <div className='flex-1 overflow-y-auto px-5 py-3 space-y-1.5'>
              ...
              {customerSearchResults.map((c) => {
                const isSelected = !!pendingGroupMembers.find(
                  (s) => s.id === c.id,
                )
                return (
                  <button
                    key={c.id}
                    onClick={() => togglePendingGroupMember(c)}
                    className={`w-full flex items-center gap-3 p-2.5 rounded-xl border transition-all text-left cursor-pointer ${isSelected ? 'border-[#008060]/30 bg-[#F2F7F5]' : 'border-[#E1E3E5] bg-white hover:border-[#008060]/20 hover:bg-[#F9FAF9]'}`}
                  >
                    <div className='w-9 h-9 rounded-full bg-[#F6F6F7] shrink-0 flex items-center justify-center text-[#8C9196] text-[12px] font-semibold uppercase'>
            ...
            <div className='px-5 py-3 border-t border-[#E1E3E5] flex items-center justify-between'>
              <p className='text-[12px] text-[#6D7175]'>
                {pendingGroupMembers.length} member
                {pendingGroupMembers.length !== 1 ? 's' : ''} selected
              </p>
              <button
                onClick={() => setCustomerModalOpen(false)}
                className='px-4 py-2 bg-[#008060] text-white text-[13px] font-medium rounded-lg hover:bg-[#006B4F] transition-colors border-none cursor-pointer'
          ...
          >
            ...
            <div className='flex items-center justify-between px-5 py-4 border-b border-[#E1E3E5]'>
              <div>
                ...
                <p className='text-[11.5px] text-[#8C9196] mt-0.5'>
                  Group customers to target them with this discount
                </p>
              </div>
              <button
                onClick={() => {
                  ...
                  <div className='flex flex-wrap gap-1.5 mb-2'>
                    {pendingGroupMembers.map((c) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => togglePendingGroupMember(c)}
                          className='w-3 h-3 flex items-center justify-center bg-transparent border-none cursor-pointer text-[#008060] hover:text-[#006B4F] p-0'
                        >
                          <svg
                            width='9'
                            height='9'
            ...
            {}
            <div className='px-5 py-3 border-t border-[#E1E3E5] flex items-center justify-end gap-2'>
              <button
                onClick={() => {
                  setNewGroupModalOpen(false)
                  setNewGroupName('')
                ...
                className='px-4 py-2 border border-[#E1E3E5] bg-white text-[13px] font-medium text-[#202223] rounded-lg hover:bg-[#F6F6F7] transition-colors cursor-pointer'
              >
                Cancel
              </button>
              <button
                ...
                className='px-4 py-2 bg-[#008060] text-white text-[13px] font-medium rounded-lg hover:bg-[#006B4F] transition-colors border-none cursor-pointer disabled:opacity-50 inline-flex items-center gap-2'
              >
                {savingGroup ? Icons.spinner : null}
