...
const TYPE_CONFIG: Record<DiscountType, {
  label: string;
  color: string;
  bg: string;
}> = {
...
function CheckIcon() {
  return <svg width='13' height='13' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round'>
...
function Toggle({
  ...
  return <div className={`relative w-9 h-5 rounded-full transition-colors duration-200 ${checked ? 'bg-[#008060]' : 'bg-[#D1D5DB]'}`}>
  ...
  return <div className='bg-white border border-[#E1E3E5] rounded-2xl p-5 flex items-center gap-4 hover:shadow-[0_4px_20px_rgba(0,0,0,0.06)] hover:-translate-y-0.5 transition-all duration-200'>
      ...
      <div>
        <p className='font-sora text-[22px] font-bold text-[#202223] leading-tight tracking-tight'>
          {loading ? <span className='inline-block w-10 h-6 bg-[#F1F1F1] rounded animate-pulse' /> : value}
        </p>
...
export default function DiscountsPage() {
  ...
  const filtered = discounts.filter(d => {
    ...
    const matchStatus = statusFilter === 'All' || statusFilter === 'Active' && d.isActive && !expired || statusFilter === 'Inactive' && !d.isActive || statusFilter === 'Expired' && expired;
  ...
  const TYPE_FILTERS = ['All', 'percentage', 'fixed', 'free_shipping', 'buy_x_get_y'];
  ...
  return <div className='space-y-5'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <Link href='/dashboard/discounts/add' className='inline-flex items-center gap-2 px-4 py-2 bg-[#008060] hover:bg-[#006e52] active:bg-[#005c45] text-white text-[13px] font-medium rounded-lg transition-all duration-150 border-none cursor-pointer shadow-sm shadow-[#008060]/20 no-underline'>
          <PlusIcon />
          Create Discount
        </Link>
      </div>
      ...
      <div className='grid grid-cols-2 xl:grid-cols-4 gap-4'>
        <StatCard label='Total Discounts' value={discounts.length} icon={<TotalDiscountsIcon />} iconBg='bg-[#2C6ECB]/8' iconColor='text-[#2C6ECB]' loading={loading} />
        <StatCard label='Active' value={activeCount} icon={<ActiveDiscountIcon />} iconBg='bg-[#008060]/8' iconColor='text-[#008060]' loading={loading} />
        <StatCard label='Total Used' value={totalUsed} icon={<UsedIcon />} iconBg='bg-amber-50' iconColor='text-amber-600' loading={loading} />
        <StatCard label='Expired' value={expiredCount} icon={<ExpiredIcon />} iconBg='bg-[#D82C0D]/8' iconColor='text-[#D82C0D]' loading={loading} />
      </div>
      ...
      <div className='bg-white border border-[#E1E3E5] rounded-2xl overflow-hidden shadow-sm'>
        ...
        <div className='flex items-center gap-3 px-4 py-3 border-b border-[#E1E3E5] flex-wrap'>
          <div className='flex items-center gap-2 flex-1 min-w-[200px] px-3 py-2 border border-[#E1E3E5] rounded-lg bg-[#F8F9FA] focus-within:border-[#008060] focus-within:bg-white focus-within:ring-2 focus-within:ring-[#008060]/10 transition-all duration-150'>
            <SearchIcon />
            <input type='text' placeholder='Search discounts...' value={search} onChange={e => setSearch(e.target.value)} className='flex-1 bg-transparent text-[13px] text-[#202223] placeholder-[#B0B5BA] outline-none' />
          ...
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className='px-3 py-2 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] bg-white outline-none cursor-pointer hover:border-[#8C9196] transition-colors shadow-sm'>
            {TYPE_FILTERS.map(t => <option key={t} value={t}>
                {typeLabel(t)}
              </option>)}
          </select>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className='px-3 py-2 border border-[#E1E3E5] rounded-lg text-[13px] text-[#202223] bg-white outline-none cursor-pointer hover:border-[#8C9196] transition-colors shadow-sm'>
        ...
        <div className='flex items-center border-b border-[#E1E3E5] overflow-x-auto scrollbar-none px-4 gap-0.5'>
          {STATUS_FILTERS.map(s => <button key={s} onClick={() => setStatusFilter(s)} className={`px-4 py-2.5 text-[12.5px] font-medium whitespace-nowrap border-b-2 transition-all duration-150 bg-transparent border-l-0 border-r-0 border-t-0 cursor-pointer ${statusFilter === s ? 'border-b-[#008060] text-[#008060]' : 'border-b-transparent text-[#8C9196] hover:text-[#202223]'}`}>
              {s}
              <span className='ml-1.5 text-[10.5px] text-[#B0B5BA]'>
                {s === 'All' ? discounts.length : s === 'Active' ? activeCount : s === 'Inactive' ? discounts.filter(d => !d.isActive).length : expiredCount}
              </span>
            </button>)}
        </div>
          ...
          <table className='w-full'>
            <thead>
              <tr className='border-b border-[#E1E3E5] bg-[#FAFAFA]'>
                {['Code', 'Type', 'Value', 'Usage', 'Min Order', 'Expires', 'Status', ''].map((h, i) => <th key={i} className={`px-4 py-3 text-[11px] font-semibold text-[#9CA3AF] uppercase tracking-wider ${i === 7 ? 'text-right' : 'text-left'}`}>
                    {h}
                  </th>)}
              </tr>
            </thead>
            <tbody className='divide-y divide-[#F5F5F5]'>
              {loading ? [...Array(6)].map((_, i) => <tr key={i} className='animate-pulse'>
                ...
                </tr> : filtered.map(discount => {
              const config = TYPE_CONFIG[discount.type] ?? TYPE_CONFIG.percentage;
              const pct = usagePct(discount.usedCount, discount.maxUses);
              const expired = isExpired(discount.expiresAt);
              return <tr key={discount.id} className='hover:bg-[#FAFAFA] transition-colors duration-100 group'>
                      {}
                      <td className='px-4 py-3.5'>
                        <div className='flex items-center gap-2'>
                          ...
                          <button onClick={() => copyCode(discount.id, discount.code)} className={`w-6 h-6 flex items-center justify-center rounded transition-all duration-150 bg-transparent border-none cursor-pointer ${copiedId === discount.id ? 'text-[#008060] bg-[#F2F7F5]' : 'text-[#B0B5BA] hover:text-[#008060] hover:bg-[#F2F7F5]'}`} title='Copy code'>
                            {copiedId === discount.id ? <CheckIcon /> : <CopyIcon />}
                          </button>
                        </div>
                      ...
                      {}
                      <td className='px-4 py-3.5'>
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold ${config.bg} ${config.color}`}>
                          {config.label}
                        </span>
                      </td>
                      ...
                      <td className='px-4 py-3.5'>
                        <div className='space-y-1.5'>
                          <div className='flex items-center gap-1.5'>
                            ...
                            {discount.maxUses && <span className='text-[11.5px] text-[#B0B5BA]'>
                                / {discount.maxUses}
                              </span>}
                          </div>
                          {pct !== null && <div className='w-24 h-1.5 bg-[#E1E3E5] rounded-full overflow-hidden'>
                              <div className={`h-full rounded-full transition-all duration-300 ${pct >= 90 ? 'bg-[#D82C0D]' : pct >= 70 ? 'bg-[#FFC453]' : 'bg-[#008060]'}`} style={{
                        width: `${pct}%`
                      }} />
                            </div>}
                        </div>
                      ...
                      <td className='px-4 py-3.5'>
                        <span className='text-[12.5px] text-[#202223]'>
                          {discount.minOrderAmount > 0 ? `£${discount.minOrderAmount}` : '—'}
                        </span>
                      </td>

                      {}
                      <td className='px-4 py-3.5'>
                        {discount.expiresAt ? <div>
                            <span className={`text-[12.5px] ${expired ? 'text-[#D82C0D]' : 'text-[#202223]'}`}>
                        ...
                        year: 'numeric'
                            ...
                            {expired && <p className='text-[10.5px] text-[#D82C0D] font-medium mt-0.5'>
                                Expired
                              </p>}
                          </div> : <span className='text-[12.5px] text-[#B0B5BA]'>
                            No expiry
                          </span>}
                      </td>
                      ...
                      <td className='px-4 py-3.5'>
                        <div className='flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150'>
                          <Link href={`/dashboard/discounts/add?id=${discount.id}`} className='w-7 h-7 flex items-center justify-center text-[#8C9196] hover:text-[#008060] hover:bg-[#008060]/5 rounded-lg transition-all duration-150 no-underline' title='Edit'>
                            <EditIcon />
                          </Link>
                          <button onClick={() => setDeleteId(discount.id)} className='w-7 h-7 flex items-center justify-center text-[#8C9196] hover:text-[#D82C0D] hover:bg-[#D82C0D]/5 rounded-lg transition-all duration-150 bg-transparent border-none cursor-pointer' title='Delete'>
                            <TrashIcon />
                          </button>
                        </div>
                      </td>
      ...
      {deleteId && <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
          ...
          <div className='relative bg-white rounded-2xl shadow-[0_24px_64px_rgba(0,0,0,0.15)] w-full max-w-sm p-6'>
            ...
            <div className='flex gap-2.5'>
              ...
              <button onClick={() => handleDelete(deleteId)} className='flex-1 py-2.5 bg-[#D82C0D] hover:bg-[#be2209] active:bg-[#a01d07] text-white text-[13px] font-semibold rounded-lg transition-all duration-150 border-none cursor-pointer shadow-sm shadow-[#D82C0D]/20'>
                Delete
              </button>
            </div>
          </div>
