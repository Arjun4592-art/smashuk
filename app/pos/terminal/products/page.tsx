...
export default function ProductsPage() {
  ...
  const filtered = products.filter((p) => {
    ...
    const matchSize = size === 'All sizes' || p.size === size
    const matchSearch =
      !search ||
  ...
  return (
    ...
    >
      ...
      >
        {}
        <div
          className='hidden sm:grid grid-cols-7 px-4 py-2.5 text-[11px] font-medium uppercase tracking-wide'
          style={{
            background: '#F6F6F7',
            color: '#8C9196',
        ...
        {medusaLoading ? (
          <div className='animate-pulse'>
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className='flex items-center gap-4 px-4 py-3 border-b border-[#F1F1F1]'
              >
          ...
          filtered.map((p, i) => {
            ...
            return (
              <div key={p.variantId ?? p.id}>
                {}
                <div
                  className='hidden sm:grid grid-cols-7 px-4 py-3 text-sm items-center'
                  style={{
                    borderBottom:
