    ...
    if (!inventoryItemId) {
      if (!foundProductId) {
        return NextResponse.json(
          {
            error:
              'Variant not found in any product — cannot create/link an inventory item for it.',
          },
          {
            status: 404,
          },
      ...
      const createItemData = await safeJson(
        createItemRes,
        ...
        return NextResponse.json(
          {
            error:
              linkData.message ??
              'Inventory item created but could not be linked to the variant.',
          },
          {
