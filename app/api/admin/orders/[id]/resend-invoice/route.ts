// app/api/admin/orders/[id]/resend-invoice/route.ts
...
export async function POST(
  ...
  // SECURITY: regenerates and re-links a customer's invoice — must
  // require a logged-in admin session, same as every other admin order action.
  ...
  try {
    ...
    url.searchParams.set(
      'fields',
      'id,currency_code,metadata,*items,*shipping_methods,customer.first_name,customer.last_name,' +
    ...
    const { invoiceNumber, url: pdfUrl } = await generateInvoiceForOrder(
      { ...orderData.order, channel },
      { regenerate: true },
    )

    return NextResponse.json({ invoiceNumber, url: pdfUrl })
  } catch (err: any) {
