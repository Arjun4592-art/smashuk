  ...
  return (
    <div className='flex-1 min-h-0 overflow-y-auto p-3 sm:p-4 space-y-3'>
      <div className='flex items-center justify-between'>
        <h1 className='text-xl font-semibold' style={{ color: '#202223' }}>Customers</h1>
        <button
      ...
      {/* Search */}
      <div className='flex items-center gap-2 px-3 py-2 rounded-lg border bg-white' style={{ borderColor: '#E1E3E5' }}>
      ...
      <div className='rounded-xl overflow-hidden' style={{ background: '#FFFFFF', border: '1px solid #E1E3E5' }}>
        {loading ? (
          [...Array(6)].map((_, i) => <POSCustomerRowSkeleton key={i} />)
        ) : customers.length === 0 ? (
          ...
          customers.map((c, i) => (
            <div
              key={c.id}
              className='flex items-center gap-3 px-4 py-3 transition-colors hover:bg-[#F6F6F7]'
              style={{ borderBottom: i < customers.length - 1 ? '1px solid #F6F6F7' : 'none' }}
            >
              <div className='w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0' style={{ background: '#E3F1EB', color: '#008060' }}>
                {c.name.charAt(0).toUpperCase()}
              </div>
              <div className='flex-1 min-w-0'>
                <p className='text-sm font-medium truncate' style={{ color: '#202223' }}>{c.name}</p>
      ...
      {showAddCustomer && (
        <CustomerSearch onClose={() => { setShowAddCustomer(false); loadCustomers() }} />
      )}
    </div>
  )
}
