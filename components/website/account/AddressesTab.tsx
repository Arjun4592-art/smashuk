...
export default function AddressesTab() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Fail quietly with an empty list — user can still add a new address
      setAddresses([])
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className='bg-white rounded-2xl border border-gray-100 p-6'>
      <div className='flex items-center justify-between mb-5'>
        ...
        <button
          onClick={openAdd}
          className='flex items-center gap-1.5 bg-[#E8553A] hover:bg-[#D4441F] text-white font-montserrat font-bold px-4 py-2 rounded-full text-xs transition-colors'
        >
      ...
      {loading ? (
        <p className='text-sm text-gray-400 font-lato'>Loading addresses...</p>
            ...
            >
              ...
              <div className='flex gap-3 mt-3'>
                <button
                  ...
                  className='text-xs font-semibold text-[#E8553A] hover:underline font-lato'
                >
                  Edit
                </button>
                <button
      ...
      {showForm && (
        <form
          onSubmit={handleSave}
          className='mt-5 border-t border-gray-100 pt-5 grid grid-cols-1 sm:grid-cols-2 gap-4'
        >
          {[
            { name: 'first_name', label: 'First Name', required: true },
            { name: 'last_name', label: 'Last Name', required: true },
            { name: 'address_1', label: 'Address Line 1', required: true, colSpan: true },
            { name: 'address_2', label: 'Address Line 2 (Optional)', colSpan: true },
            { name: 'city', label: 'City', required: true },
            { name: 'province', label: 'County' },
