...
async function saveSessions(storeId: string, sessions: CashDrawerSession[]): Promise<{
  ok: boolean;
  error?: string;
}> {
  const res = await medusaServiceFetch(`/admin/stores/${storeId}`, {
    ...
    body: JSON.stringify({
      metadata: {
        posCashDrawerSessions: sessions.slice(0, MAX_HISTORY)
      }
    })
  });
  const data = await safeJson(res);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'movement') {
      ...
      if (typeof amount !== 'number' || !['in', 'out'].includes(type) || !reason) {
        return NextResponse.json({
