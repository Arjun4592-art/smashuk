...
export async function GET(req: NextRequest) {
  ...
  params.set('fields', 'id,title,status,thumbnail,metadata,*images,*categories,*variants,variants.id,variants.title,variants.sku,variants.barcode,*variants.prices,*variants.images,*variants.inventory_items,*variants.inventory_items.inventory.location_levels');
  try {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!body.metadata.metaTitle && body.title) {
      body.metadata.metaTitle = body.title.length <= 60 ? body.title : body.title.slice(0, 57).trim() + '...';
    }
    if (!body.metadata.metaDescription) {
      const clean = (body.description ?? '').replace(/\s+/g, ' ').trim();
      const fallback = clean || `Buy ${body.title ?? 'this product'} online at Smash Racket Pro. Fast UK delivery, genuine products, easy returns.`;
      body.metadata.metaDescription = fallback.length <= 155 ? fallback : fallback.slice(0, 152).trim() + '...';
    }
    if (!body.metadata.metaKeywords) {
      const parts = [body.metadata.brand, body.metadata.sport, ...(Array.isArray(body.tags) ? body.tags.map((t: any) => t.value) : [])].filter(Boolean);
      if (parts.length) body.metadata.metaKeywords = Array.from(new Set(parts)).join(', ');
    }
    ...
    if (!body.sales_channels) {
      const channels = await resolveSalesChannels(sellingChannel, authorization, MEDUSA_URL);
      if (channels) body.sales_channels = channels;
    }
    if (!body.shipping_profile_id) {
      try {
        const spRes = await fetch(`${MEDUSA_URL}/admin/shipping-profiles?limit=50`, {
          headers: {
            Authorization: authorization
          }
        });
        const spData = await safeJson(spRes, 'app/api/admin/products/route.ts');
        const defaultProfileId = (spData.shipping_profiles ?? []).find((p: any) => p.type === 'default')?.id ?? (spData.shipping_profiles ?? []).find((p: any) => /default/i.test(p.name ?? ''))?.id;
        if (defaultProfileId) {
    ...
    if (data.product?.id) {
      try {
        const skuProductRes = await fetch(`${MEDUSA_URL}/admin/products/${data.product.id}?fields=*variants,*variants.inventory_items`, {
          headers: {
            Authorization: authorization
          }
        });
        ...
        for (const variant of skuVariants) {
          const invItemId: string | undefined = variant.inventory_items?.[0]?.inventory_item_id ?? variant.inventory_items?.[0]?.inventory?.id ?? undefined;
          ...
          const skuPatchRes = await fetch(`${MEDUSA_URL}/admin/inventory-items/${invItemId}`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              Authorization: authorization
    ...
    if (hasVariantsToLink && data.product?.id) {
      ...
      if (!locationId) {
        ...
        const productRes = await fetch(`${MEDUSA_URL}/admin/products/${data.product.id}?fields=*variants,*variants.inventory_items`, {
          headers: {
            Authorization: authorization
          }
        });
        ...
        for (const variant of variants) {
          ...
          if (!inventoryItemId) {
            try {
              ...
              const linkRes = await fetch(`${MEDUSA_URL}/admin/products/${data.product.id}/variants/${variant.id}/inventory-items`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  Authorization: authorization
          ...
          const qty = variantStocks[variantTitle] ?? variantStocks[variant.sku ?? ''] ?? variantStocks[variantTitle.trim()] ?? stockQty;
          const safeQty = qty > 0 ? qty : 0;
          const levelsRes = await fetch(`${MEDUSA_URL}/admin/inventory-items/${inventoryItemId}/location-levels?location_id[]=${locationId}`, {
            headers: {
              Authorization: authorization
            }
          });
          ...
          if (existingLevel) {
            const upRes = await fetch(`${MEDUSA_URL}/admin/inventory-items/${inventoryItemId}/location-levels/${locationId}`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                Authorization: authorization
            ...
            if (!createRes.ok) console.warn('[products POST] Level create failed:', createData.message);else void 0;
          }
        }
      }
    }
