...
interface Props {
  order: any
  // Remaining returnable quantity per line item id (already accounts for
  // past returns) — passed in so this component doesn't need its own
  // copy of the server-side calculation.
  remainingQty: Record<string, number>
  onSubmit: (items: { item_id: string; quantity: number }[], reason: string) => Promise<void>
  onClose: () => void
}

export default function ReturnOrderModal({ order, remainingQty, onSubmit, onClose }: Props) {
  ...
  const refundTotal = returnableItems.reduce(
    ...
    >
      ...
      >
        ...
        {returnableItems.length === 0 ? (
          <p className='text-[13px] text-[#6D7175]'>Every item on this order has already been returned.</p>
        ) : (
          <div className='space-y-3 mb-4'>
            {returnableItems.map((item: any) => {
              ...
              return (
                <div key={item.id} className='p-3 rounded-lg border border-[#E1E3E5]'>
                  <div className='flex items-center justify-between mb-2'>
                    ...
                    <div className='flex items-center rounded border border-[#E1E3E5] overflow-hidden'>
                      <button
                        onClick={() =>
                          setQtys((p) => ({ ...p, [item.id]: Math.max(0, qty - 1) }))
                        }
                      ...
                      <button
                        onClick={() =>
                          setQtys((p) => ({ ...p, [item.id]: Math.min(max, qty + 1) }))
                        }
                        className='w-7 h-7 flex items-center justify-center hover:bg-[#F6F6F7] text-[#6D7175]'
                      >
                        +
            ...
            {refundTotal > 0 && (
              <div className='p-3 rounded-lg bg-[#F2F7F5] flex justify-between text-[13px] font-medium text-[#202223]'>
