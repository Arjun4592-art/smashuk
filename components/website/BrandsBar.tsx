...
function BrandLogo({
  ...
  if (failed) {
    return <span className='font-montserrat font-black text-[11px] tracking-wide text-[#0A1F44] group-hover:text-[#E8553A] transition-colors duration-300'>
        {initial}
      </span>;
  }
  return (<img src={logo} alt={`${name} logo`} className={`${dim} object-contain`} onError={() => setFailed(true)} />
  );
}
export default function BrandsBar() {
  ...
  const BRANDS = useMemo(() => {
    ...
    return list.map(b => {
      ...
      return {
        name: b.name,
        slug: slugify(b.name),
        description: info?.description ?? `${b.count} product${b.count === 1 ? '' : 's'}`,
  ...
  return <section className='py-16 bg-white border-y border-[#E5E7EB] overflow-hidden'>
      {}
      ...
      <div className='relative mb-3'>
        ...
        <div className='overflow-hidden'>
          <div className='flex items-center gap-3.5 py-1.5 w-max animate-scroll'>
            {BRANDS_DOUBLED.map((brand, i) => <div key={`${brand.name}-${i}`} className='shrink-0 group cursor-pointer relative flex flex-col items-center gap-2 px-3.5 py-3 min-w-[92px] rounded-[18px] border border-[#E5E7EB] bg-white hover:border-[#E8553A]/40 hover:shadow-[0_8px_24px_rgba(232,85,58,0.10)] hover:-translate-y-[3px] transition-all duration-300'>
                {}
                ...
                <div className='w-16 h-16 rounded-xl bg-[#F2F4F7] group-hover:bg-[#E8553A]/[0.08] border border-transparent group-hover:border-[#E8553A]/20 flex items-center justify-center transition-all duration-300 overflow-hidden'>
                  <BrandLogo logo={brand.logo} initial={brand.initial} name={brand.name} size='md' />
                </div>
      ...
      <div className='relative'>
        ...
        <div className='overflow-hidden'>
          <div className='flex items-center gap-3.5 w-max animate-scroll-reverse'>
            {[...BRANDS_DOUBLED].reverse().map((brand, i) => <div key={`${brand.name}-rev-${i}`} className='shrink-0 group cursor-pointer flex items-center gap-2 px-3.5 py-2 pr-4 rounded-full border border-[#E5E7EB] bg-white hover:border-[#E8553A]/40 hover:bg-[#E8553A]/[0.04] hover:-translate-y-px transition-all duration-200'>
                <div className='w-10 h-10 rounded-lg bg-[#0A1F44]/[0.06] group-hover:bg-[#E8553A]/10 flex items-center justify-center shrink-0 transition-colors duration-200 overflow-hidden'>
                  <BrandLogo logo={brand.logo} initial={brand.initial} name={brand.name} size='sm' />
                </div>
      ...
      <div className='max-w-[520px] mx-auto mt-9 px-4'>
        <div className='flex items-center bg-[#F2F4F7] border border-[#E5E7EB] rounded-[20px] px-6 py-5'>
          {STATS.map((stat, idx) => <div key={stat.label} className={`flex-1 text-center px-4 ${idx > 0 ? 'border-l border-[#E5E7EB]' : ''}`}>
      ...
      <div className='text-center mt-7'>
        <a href='/shop' className='inline-flex items-center gap-1.5 text-sm font-bold text-[#E8553A] font-montserrat px-[22px] py-2.5 rounded-full border border-[#E8553A]/25 hover:bg-[#E8553A]/[0.06] hover:border-[#E8553A]/50 hover:gap-2.5 transition-all duration-200 group'>
          Shop all brands
