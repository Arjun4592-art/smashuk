...
export const BADMINTON_STRING_BRANDS: StringBrand[] = [{
  brand: 'Yonex Strings',
  items: ['BG 65', 'BG 65 Titanium', 'BG 80', 'BG 80 Power', 'BG 66 Ultimax', 'Exbolt 68', 'Exbolt 65', 'Exbolt 63', 'Aerobite', 'Aerobite Boost', 'Nanogy 98', 'Nanogy 95']
}, {
  ...
  items: ['Ashaway Zymax 69', 'Ashaway Zymax 66 Fire Power', 'Ashaway Rogue Duo Hybrid']
}];
...
export const TENNIS_STRING_BRANDS: StringBrand[] = [{
  brand: 'Babolat',
  intro: 'Best known for co-polyester ("poly") strings built for durability, spin and control.',
  groups: [{
    label: 'Natural Gut Strings',
    items: ['VS Touch', 'VS Team', 'Tonic+ Natural Gut']
  }, {
    label: 'Multifilament & Premium Nylon Strings',
    items: ['Xcel', 'Xcel French Open Edition', 'Addiction', 'Origin', 'N.Vy', 'Synthetic Gut', 'M7']
  }, {
    label: 'Polyester / Co-Poly Strings (RPM & Pro Hurricane)',
    items: ['RPM Blast', 'RPM Blast Rough', 'RPM Team', 'RPM Power', 'RPM Hurricane', 'Pro Hurricane', 'Pro Hurricane Tour']
  }]
}, {
  brand: 'Luxilon',
  items: ['4G Series', 'Element Series', 'Big Banger Alu Power Series', 'Big Banger Original Series', 'Adrenaline Series', 'Savage Series', 'Smart']
}, {
  brand: 'Solinco',
  items: ['Hyper-G', 'Tour-Bite', 'Outlast', 'Vanquish', 'Confidential']
}, {
  brand: 'Head',
  items: ['Lynx Tour', 'Velocity', 'RIP Control', 'Sonic Pro', 'Hawk', 'Hawk Touch']
}, {
  brand: 'Wilson',
...
export const SQUASH_BRANDS: string[] = ['Tecnifibre', 'Ashaway', 'Prince', 'Head', 'Wilson'];
export function getStringGroupsForSport(sport: string): {
  group: string;
  items: string[];
}[] {
  switch (sport) {
    ...
    case 'squash':
      return [{
        group: 'String Type',
        items: SQUASH_STRINGS.map(s => s.name)
      }, {
