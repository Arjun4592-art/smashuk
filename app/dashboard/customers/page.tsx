...
function CustomersContent() {
  ...
  // BUG FIX: the per-row "Edit" button used to have no onClick at all —
  // there was no way to edit a customer's details anywhere in the app.
  // Mirrors handleAddCustomer above, but PATCHes the existing customer via
  // the new app/api/admin/customers/[id]/route.ts instead of creating one.
      ...
      <div className='bg-white border border-[#E1E3E5] rounded-2xl overflow-hidden shadow-sm'>
        ...
        <div className='flex items-center border-b border-[#E1E3E5] px-4 gap-0.5'>
          {[
            ...
            { id: 'segments', label: 'Segments', count: 0 },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setTab(tab.id)}
              className={`px-4 py-3 text-[13px] font-medium whitespace-nowrap border-b-2 transition-all duration-150 bg-transparent border-l-0 border-r-0 border-t-0 cursor-pointer ${
                view === tab.id
              ...
              <div className='flex items-center gap-1'>
                ...
                {Array.from({ length: Math.min(totalPages, 5) }).map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setPage(i + 1)}
                    className={`w-8 h-8 flex items-center justify-center rounded-lg text-[12.5px] font-medium cursor-pointer transition-all duration-150 ${
                      page === i + 1
                        ? 'bg-[#008060] text-white border-none shadow-sm shadow-[#008060]/20'
