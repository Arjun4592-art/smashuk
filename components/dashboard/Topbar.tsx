...
export default function Topbar() {
  ...
  // Persist which notification IDs have been read across refreshes/polls.
  // Notification IDs are stable (order-{id}, stock-{variantId}, etc.), so a
  // read stays read even though the backend recomputes the notification
  // list fresh from live data on every poll.
  ...
  const saveReadIds = (ids: Set<string>) => {
    try {
      // Cap stored size so this doesn't grow forever
      const arr = Array.from(ids).slice(-500)
      window.localStorage.setItem(READ_IDS_KEY, JSON.stringify(arr))
    } catch {
      // localStorage unavailable — non-fatal, read state just won't persist
    }
  }
      ...
      <div className='flex items-center gap-1 shrink-0'>
        ...
        <div className='relative'>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className='absolute top-1 right-1 w-3.5 h-3.5 bg-[#D82C0D] text-white text-[8px] font-bold rounded-full flex items-center justify-center'>
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {showNotifications && (
            <div className='absolute right-0 top-full mt-2 w-[340px] bg-white border border-[#E1E3E5] rounded-xl shadow-lg overflow-hidden z-50'>
              <div className='flex items-center justify-between px-4 py-3 border-b border-[#E1E3E5]'>
                <div className='flex items-center gap-2'>
                  ...
                  {unreadCount > 0 && (
                    <span className='px-1.5 py-0.5 bg-[#D82C0D] text-white text-[10px] font-bold rounded-full'>
                      {unreadCount}
                    </span>
                  )}
                </div>
                <button
                  ...
                  className='text-[12px] text-[#008060] hover:text-[#006e52] bg-transparent border-none cursor-pointer'
                >
                  Mark all read
                </button>
              </div>
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                    ...
                    >
                      ...
                      {n.unread && (
                        <span className='w-2 h-2 rounded-full bg-[#008060] shrink-0 mt-1.5' />
                      )}
                    </div>
                  ))
                )}
