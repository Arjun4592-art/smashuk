...
async function findParcel2GoProviderId(token: string): Promise<string> {
  ...
  const parcel2goProvider = providers.find((p: any) =>
    /parcel ?2 ?go/i.test(p.id ?? ''),
  )
  if (!parcel2goProvider) {
    throw new Error(
      'No fulfillment provider matching "parcel2go" is registered on the backend.\n' +
        'Found these instead: ' +
        (providers.map((p: any) => p.id).join(', ') || '(none)') +
        '\nCheck that the parcel2go module is added to medusa-config.ts and the backend has ' +
        'been restarted, or run scripts/fix-parcel2go-provider.ts to diagnose an existing store.',
    )
  }
  return parcel2goProvider.id
}
...
async function main() {
  ...
  const existingParcel2Go = options.find(
