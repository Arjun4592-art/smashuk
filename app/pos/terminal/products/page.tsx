...
export default function ProductsPage() {
  ...
  return <div className='flex-1 min-h-0 overflow-y-auto p-3 sm:p-4 space-y-3' style={{
    opacity: mounted ? 1 : 0,
    ...
    transition: 'opacity 0.2s ease, transform 0.2s ease'
      ...
      border: '1px solid #E1E3E5'
        ...
        {}
        <div className='hidden sm:grid grid-cols-6 px-4 py-2.5 text-[11px] font-medium uppercase tracking-wide' style={{
        background: '#F6F6F7',
        color: '#8C9196',
        ...
        {medusaLoading ? <div className='animate-pulse'>
            {[...Array(8)].map((_, i) => <div key={i} className='flex items-center gap-4 px-4 py-3 border-b border-[#F1F1F1]'>
        ...
        return <div key={p.id}>
                {}
                <div className='hidden sm:grid grid-cols-6 px-4 py-3 text-sm items-center' style={{
            borderBottom: i < filtered.length - 1 ? '1px solid #F6F6F7' : 'none'
              ...
              color: stockColor
                    ...
                    {isOut ? 'Out' : !showStockCount ? '—' : isLow ? `${p.stock} left` : `${p.stock}`}
                  </span>
                  <span className='text-[10px] px-1.5 py-0.5 rounded font-medium w-fit' style={{
              ...
              color: p.channel === 'pos_only' ? '#B7791F' : '#6D7175'
                ...
                {}
                <div className='sm:hidden flex items-center justify-between px-4 py-3 gap-3' style={{
            borderBottom: i < filtered.length - 1 ? '1px solid #F6F6F7' : 'none'
          }}>
                ...
                color: stockColor
                      ...
                      {isOut ? 'Out of stock' : !showStockCount ? '' : isLow ? `${p.stock} left` : `${p.stock} in stock`}
                    </p>
                  </div>
                </div>
              </div>;
